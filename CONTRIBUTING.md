...
- **Building and running the test suite** requires a **Python 3** interpreter on `PATH` (used only by the independent Domain Store D1-A vector oracle: `tools/domain_store_vector_gen.py generate|check`). Production libraries do not link or invoke Python.
