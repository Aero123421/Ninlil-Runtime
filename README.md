...
- Domain Store D1-B2 / D1-B3a..l body codec（SCHEDULER_OWNER / ORDERED_INGRESS / BLOB / ATTEMPT / ATTEMPT_ID_INDEX / CANCEL_STATE / EVIDENCE_CELL / DELIVERY / RESULT_CACHE / REVERSE_REPLY / EVENT_SPOOL / RETRY_SUMMARY）。**D1-B3b controller-ingress retrofit implemented**（ORDERED_INGRESS `controller_ingress_*` 32-byte local durable-copy block）。**D1-B3g EVIDENCE_CELL production implemented**。**D1-B3h DELIVERY production implemented**。**D1-B3i RESULT_CACHE production implemented**（kind9/10 `digest||token_generation||phase` exact 42 bytes）。**D1-B3j REVERSE_REPLY production implemented**（exact 330 / raw80+raw86 / key+header bijection / counter-state-timer-I/availability closed matrix）。**D1-B3k EVENT_SPOOL production implemented**（exact 300 / ID128=tx / revision=spool_revision / state×cause / resume 0..8 / discard iff DISCARDED / reservation KEY_DIGEST）。**D1-B3l RETRY_SUMMARY production implemented**（vector format `ninlil-domain-store-v1-d1b3l`; CUMULATIVE exact 84 / RECENT exact 80 / composite key / kind-slot-fold / bools; B3m–B3o / D2-S3 / Stage 5 / public Runtime still pending）
