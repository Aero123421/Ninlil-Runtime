...
- **R7 T1c Authenticated Hop Fresh-Install Owner（Proposed docs-only）:** M4 authenticated Hop install token one-shot consume、accepted authority stamp adapter、node-id KDF closed mapping、T1b verified derive → 既存 N6 Hop fresh 4-key FULL、FULL_OK only handle publish を **docs-only Proposed** として固定（[docs/34](docs/34-r7-t1c-authenticated-hop-fresh-install-owner.md) / [ADR-0014](docs/adr/0014-r7-t1c-authenticated-hop-fresh-install-owner.md) Proposed）。**implementation candidate / vectors / CTest / ESP target crypto KAT / Accepted ではない。** E2E install、resume/M5、W1/L1、counter/AEAD/replay、LINK/FRAG/CELL/HA、RF/USB/LoRa HIL、Japan legal、production radio、R7 fullは非claim。public ABI非変更。compile/link ≠ target execution。
...
- Domain Store D3相互validation の残 slice（EVIDENCE live L/cardinality multi-row / witness chain / capacity / health 等; **D3-S0 architecture freeze is docs-only complete**; **D3-S1a closed-mode/context freeze is docs-only complete**; **D3-S1 exact-1 implementation complete**; **D3-S2a declared multi-count freeze is docs-only complete**; **D3-S3a BLOB lifecycle freeze is docs-only complete**; **D3-S4a DSW1_ALL_OLD_NEW freeze is docs-only complete**; **D3-S2 implementation Accepted; D3-S3 implementation は R28 oracle 283 vectors + production bridge green + ADR-0015 Accepted（2026-07-21）; D3-S4 implementation / D3-S5..S12 / D3 overall / Stage 5 D3 bind still pending**）、D4 business operation別commit-unknown convergence（empty metadata init は private candidate のみ）
