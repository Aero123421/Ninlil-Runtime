...
| Identity / Attachment / session install | **PROPOSED / private Host candidate** | precondition contractとNIAF S5 amendmentはAcceptedです。固定容量・fail-closedなprivate consumer kernelと、caller指定専用namespaceのprivate NIAF standalone ownerはHost候補です。NIAFはCore/Foundation Domain Store durability authority配下でcanonical Domain schema-1 catalogとは独立し、availabilityを公開しません。PA-S0 software gateとPA-S1aのexact libedhoc/zcbor default-OFF依存・test-only custom-memory候補は独立reviewでGOです。PA-S2aもexact callbackの4-byte opaque key slot、SHA-256/HKDF、suite 2 AES-CCM、suite 3 ChaCha20-Poly1305をHost KATとESP32-S3 compile/link/stackまで独立reviewでGOです。PA-S2b1のephemeral P-256と2-use opaque-token bridge、PA-S3aのprivate NAS1 direct-stream parser、PA-S3b1のstrict NAR1 post-admission single-record reassemblerもHost通常・SanitizerとESP32-S3 compile/link/stackまで独立reviewでGOです。ESP target KATとHost/ESP equality、credential authority、local static-DH、EDHOC message owner、accepted carrier/socket、live locator/cookie/RF ingress、NAC1 handoff、Composition注入、PA-S1全体、PA-S2..S6、direct/cold Join、物理HILは未完です。 |
