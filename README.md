...
| Identity / Attachment / session install | **PROPOSED / private kernel Host candidate** | precondition contractとNIAF S5 amendmentはAcceptedです。固定容量・fail-closedなprivate consumer kernelはHost候補で、NIAFはCore/Foundation Domain Store durability authority配下のcaller指定専用namespaceとしてcanonical Domain schema-1 catalogとは独立します。standalone owner、Compositionへの注入、PA-S1..S6、direct/cold Join、ESP target実行、物理HILは未完です。kernelは常にfencedで、availabilityを公開しません。 |
