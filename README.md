...
- **U0 USB / physical radio boundary freeze (docs only):** compile/runtime 依存分離 [ADR-0003](docs/adr/0003-radio-usb-dependency-direction.md)、NCL1 HELLO（Controller-only）/ session ownership / Compliance Permit sole TX edge（immutable plan 後）/ Required HIL / U1–U7・R1–R10（[docs/23](docs/23-usb-radio-boundary.md)）。**USB production / SX1262 production 未実装。wire unallocated。compile ≠ HIL。Attachment/Join・relay・multi-parent は後続。**
...
- ESP-IDF storage の実機 FULL attestation / power-cut HIL、USB transport 実装、LoRa bearer/radio MAC、完全 logical control / assignment / custody protocol、public Runtime owner wiring（**M3-prep packaging + M3-basic adapters + control framing + owner/cell/loopback skeleton + storage candidate + U0 boundary docs freeze まで。storage は ESP_UNPROVEN; USB/SX1262 code と M3 complete / port completeではない**）
...
ESP-IDFはV1のtargetです。**M3-prep** のcomponent packagingとpinned `v5.5.3` / ESP32-S3 compile smokeに加え、**M3-basic** のclock / entropy / execution adapters、**M3-slice** のowner-task / Cell Agent skeleton / loopback TxPermit、および dual-slot durable-storage candidate（PR #80）まで実装済みです。**U0** で USB/radio の依存方向と境界を docs freeze 済み（[ADR-0003](docs/adr/0003-radio-usb-dependency-direction.md)、[docs/23](docs/23-usb-radio-boundary.md)）。storageの実機power-cut HIL / FULL attestation、USB / radio production 実装とpublic Runtime owner wiringは未完了で、port completeではありません。それ以外のplatformもPort ABIによる移植候補になり得ますが、現時点では検証、互換性、supportを約束しません。詳細は[M3-prep](docs/18-m3-prep-esp-idf-component.md)、[M3-basic](docs/20-m3-basic-esp-idf-platform-adapters.md)、[M3 durable storage](docs/21-m3-esp-idf-durable-storage.md)、[M3 owner/cell skeleton](docs/22-m3-owner-cell-agent-skeleton.md)、[USB/radio boundary](docs/23-usb-radio-boundary.md)を参照してください。
