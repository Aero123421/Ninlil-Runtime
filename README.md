...
peer側のApplication Runtime・複数Service統合、実機起動、物理HIL、
...
| 区分 | 内容 |
| --- | --- |
| **Host software evidenceあり** | Portable Core、POSIX SQLite、service、durable retry/dedupe、公開Fabric、公開POSIX TCP/TLSの2プロセスrestart E2E、公開POSIX USB serialのPTY E2E、private V1 USB bridgeの永続binding・双方向packet・再接続PTY、固定2 peer／4 pathのPC USB→Fabric adapter、Controller接続プローブの別process＋PTY縦断、固定board ownerによるUSB→NRA1/NRW1/R7/R9/SX1262 spy→peer→Receipt→USB縦断、private relay/multi-parent、公開submitからApplication Receiptまでのprivate multi-frame Host経路、`composition_v1` base owner。ESP実行と実機経路は未完であり、機能ごとの正式状態は上表を優先します |
| **ESP compile/link evidence** | ESP-IDF v5.5.3 compile/link/map、PSRAM/stack/resource gate、Wi-Fi/SX1262/USBのtarget adapter、公開Fabric/Compositionとprivate V1 radio packet-link・固定board ownerのcomponent packaging。全private機能の同時ONと、USB CDC・SX1262・clock・session Storageを接続した診断用board imageの最終ELFまで合格。target実行と物理経路は未確認なので、platform状態は`SPEC_ACCEPTED`のままです |
