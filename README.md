...
| Identity / Attachment / session install | **PROPOSED / private kernel Host candidate** | precondition contractはAcceptedで、固定容量・fail-closedなprivate consumer kernelはHost候補です。ただしNIAF single-writer、Composition activation/injection、PA-S1..S6、direct/cold Join、ESP target実行、物理HILは未完です。kernelは常にfencedで、availabilityを公開しません。 |
