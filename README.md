...
| Identity / Attachment / session install | **PROPOSED / private Host candidate** | precondition contractとNIAF S5 amendmentはAcceptedです。固定容量・fail-closedなprivate consumer kernelと、caller指定専用namespaceのprivate NIAF standalone ownerはHost候補です。NIAFはCore/Foundation Domain Store durability authority配下でcanonical Domain schema-1 catalogとは独立し、availabilityを公開しません。ownerはESP32-S3 compile/linkとframe evidenceまでで、target実行は未実施です。Compositionへの注入、PA-S1..S6、direct/cold Join、物理HILは未完です。 |
