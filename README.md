...
Acceptedです。ただしRF mappingとUSB framingが未確定なため、縦断機能の状態は
`PROPOSED`のままです。仕様承認を実装済みや物理HIL済みとは扱いません。
...
| V1 functional LAB vertical slice | **PROPOSED** | ADR-0034でLinux/macOS→USB親機→単一hop SX1262→指定peer/Service→APPLIED ReceiptをV1の実機完成線としてAccepted。小容量ApplicationData、複数Service、3台・10件/10秒の単一channel受入を定義し、20件/10秒はV2の二channel等で検証。RF mapping、USB framing、実装、物理HILは未完 |
...
4. USB control framingと単一hop ApplicationData↔NRW1 SINGLE mappingを固定 — 未完
