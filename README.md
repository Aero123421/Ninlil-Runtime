...
- **M3-slice durable storage candidate:** format 4 dual-slot durable-storage 候補（[docs/21](docs/21-m3-esp-idf-durable-storage.md)）。host conformance 済みだが ESP FULL は `COMMIT_UNKNOWN`、実機 power-cut HIL 未実施。**M3 complete / field-ready ではない**
...
- ESP-IDF storage の実機 FULL attestation / power-cut HIL、USB transport、LoRa bearer/radio MAC、logical control messages、public Runtime owner wiring（**M3-prep packaging + M3-basic adapters + control framing + owner/cell/loopback skeleton + storage candidate まで。storage は ESP_UNPROVEN; M3 complete / port completeではない**）
...
ESP-IDFはV1のtargetです。**M3-prep** のcomponent packagingとpinned `v5.5.3` / ESP32-S3 compile smokeに加え、**M3-basic** のclock / entropy / execution adapters、**M3-slice** のowner-task / Cell Agent skeleton / loopback TxPermit、および dual-slot durable-storage candidateまで実装済みです。storageの実機power-cut HIL / FULL attestation、USB / radio bearerとpublic Runtime owner wiringは未完了で、port completeではありません。それ以外のplatformもPort ABIによる移植候補になり得ますが、現時点では検証、互換性、supportを約束しません。詳細は[M3-prep](docs/18-m3-prep-esp-idf-component.md)、[M3-basic](docs/20-m3-basic-esp-idf-platform-adapters.md)、[M3 durable storage](docs/21-m3-esp-idf-durable-storage.md)、[M3 owner/cell skeleton](docs/22-m3-owner-cell-agent-skeleton.md)を参照してください。
