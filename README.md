...
- Domain Store D1-B2 / D1-B3a..j body codec（SCHEDULER_OWNER / ORDERED_INGRESS / BLOB / ATTEMPT / ATTEMPT_ID_INDEX / CANCEL_STATE / EVIDENCE_CELL / DELIVERY / RESULT_CACHE / REVERSE_REPLY）。**D1-B3b controller-ingress retrofit implemented**（ORDERED_INGRESS `controller_ingress_*` 32-byte local durable-copy block）。**D1-B3g EVIDENCE_CELL production implemented**。**D1-B3h DELIVERY production implemented**。**D1-B3i RESULT_CACHE production implemented**（kind9/10 `digest||token_generation||phase` exact 42 bytes）。**D1-B3j REVERSE_REPLY production implemented**（vector format `ninlil-domain-store-v1-d1b3j`; exact 330 / raw80+raw86 / key+header bijection / counter-state-timer-I/availability closed matrix）
...
- 現在の開発branchで54件のCTestが成功することを、通常buildとASan/UBSan buildで検査
