...
lost/unknownをHost通常・ASan/UBSan試験で確認しました。ただし、利用者が起動する
Controller接続プローブは実装済みで、実PTY上の別processとの`BOARD_INFO`照合、binding
適用、SQLite-backed Composition生成、Fabric登録をHost通常・ASan/UBSanで確認しました。
現時点のプローブは接続診断後に終了し、Application送信CLI／常駐loopではありません。
...
残るController RuntimeからのApplication送信・結果表示、実機起動、物理HIL、
flash-FULL耐久経路は未完であるため、
...
5. private SX1262 packet-linkをESP32-S3へ載せる — USB bridge、provisioner、packet-linkを束ねる固定board owner、接続世代ごとのclock anchor、bindingからのController/peer ID採用をHost模擬E2Eで実装。同じsourceのUSB親機／汎用peer診断imageもtarget compile/linkに合格。PC側private USB→Fabric adapterは固定2 peer／4 path、Controller接続プローブは実process＋PTYのclock照合・binding適用・SQLite Composition・Fabric登録までHost合格。peer固定Compositionは初回bindingから最大3 Serviceを同一Runtimeへ登録し、実radio packet-link上のcallback Receiptと公開submit EventFactまでHost合格。残件はController Runtime公開submitのUSB縦断とflash-FULL耐久経路
...
| 区分 | 内容 |
| --- | --- |
| **Host software evidenceあり** | Portable Core、POSIX SQLite、service、durable retry/dedupe、公開Fabric、公開POSIX TCP/TLSの2プロセスrestart E2E、公開POSIX USB serialのPTY E2E、private V1 USB bridgeの永続binding・双方向packet・再接続PTY、固定2 peer／4 pathのPC USB→Fabric adapter、Controller接続プローブの別process＋PTY縦断、固定board ownerによるUSB→NRA1/NRW1/R7/R9/SX1262 spy→peer→Receipt→USB縦断、初回bindingから最大3 Serviceを登録し実radio packet-linkでcallback Receiptと公開submit EventFactを通すpeer固定Composition、private relay/multi-parent、公開submitからApplication Receiptまでのprivate multi-frame Host経路、`composition_v1` base owner。ESP実行と実機経路は未完であり、機能ごとの正式状態は上表を優先します |
| **ESP compile/link evidence** | ESP-IDF v5.5.3 compile/link/map、PSRAM/stack/resource gate、Wi-Fi/SX1262/USBのtarget adapter、公開Fabric/Compositionとprivate V1 radio packet-link・固定board ownerのcomponent packaging。全private機能の同時ONと、USB CDC・SX1262・clock・session Storage、RF開始前のboot entropy→DRBG切替、peer固定Compositionを接続した診断用board imageの最終ELFまで合格。target実行と物理経路は未確認なので、platform状態は`SPEC_ACCEPTED`のままです |
...
Composition生成、最大4 pathのFabric登録までを確認できます。既定ではbuildされず、
Domain schema 1候補との同時利用も意図的に拒否します。
