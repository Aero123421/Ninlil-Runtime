...
- **R7 T0 private crypto provider candidate（local acceptance GO）:** portable validation/alias/zeroization wrapper、Host OpenSSL **exact 3.x**、ESP-IDF v5.5.3 mbedTLS、AES-128-GCM / HKDF-SHA-256 / SHA-256、37-vector独立oracle bridgeをproduction-private境界へ追加（[docs/31](docs/31-r7-crypto-provider-and-aead.md) / [ADR-0011](docs/adr/0011-r7-crypto-provider-boundary.md) Proposed）。tests-OFF member/symbol/install leakage、exact CTest set、GCC `-O2` stack evidence、ESP最終ELF linkをfail-closed検査。**T0 Acceptedはremote CI確認待ち。R7 full wire codec・counter/storage・FRAG/LINK/CELL/HA、実機KAT、RF/USB HIL、Japan legal、production radioは未実装・未検証。public ABI非変更。compile/link ≠ HIL。**
