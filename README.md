...
Serviceを登録するHost起動・close試験とESP peer target buildまで合格しました。
残るControllerからのApplication送信、peer callbackと上りEventを一続きにした
Host E2E、実機起動、物理HIL、flash-FULL耐久経路は未完であるため、
...
| 区分 | 内容 |
| --- | --- |
| **Host software evidenceあり** | Portable Core、POSIX SQLite、service、durable retry/dedupe、公開Fabric、公開POSIX TCP/TLSの2プロセスrestart E2E、公開POSIX USB serialのPTY E2E、private V1 USB bridgeの永続binding・双方向packet・再接続PTY、固定2 peer／4 pathのPC USB→Fabric adapter、Controller接続プローブの別process＋PTY縦断、固定board ownerによるUSB→NRA1/NRW1/R7/R9/SX1262 spy→peer→Receipt→USB縦断、初回bindingから最大3 Serviceを登録するpeer固定Composition lifecycle、private relay/multi-parent、公開submitからApplication Receiptまでのprivate multi-frame Host経路、`composition_v1` base owner。ESP実行と実機経路は未完であり、機能ごとの正式状態は上表を優先します |
