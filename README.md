...
| Identity / Attachment / session install | **PROPOSED / precondition contract accepted / implementation open** | 利用開始前のidentity/attachment preconditionとNIAF restart floorの仕様は固まりました。実際に使えるsession installは未実装です。PA-S1..S6、private consumer seam、direct/cold Join、ESP実行、物理HILは未完です。 |
