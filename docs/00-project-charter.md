...
KGuard はこれらの reference application ですが、Ninlil の public model は KGuard の業務語彙に依存しません。
...
- KGuard を持っていなくても simulator と generic example で貢献できる。
...
- leak、clog、occupancy、display の schema
- 「漏水なら使用不可」の rule
- 表示文言と色
- site / toilet / work order の業務 model
- KGuard service ごとの policy と SLO 要求
