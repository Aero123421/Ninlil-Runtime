...
| R1 host (`radio_hal_r1` + `radio_hal_r1_gate`) | sole `transmit_with_permit`; production default-deny; null/zero/oversize; validator/consume deny+error; success exactly-once order (**digest→validate→consume→edge×1**); R2-seam one-shot replay deny; callback reentry BUSY zero second edge; frame mutation TOCTOU fail-closed; each §9.3 live bind field mismatch independent; permit seq reuse; not-before/expiry boundaries; edge error 後 reuse 不可; counter saturation helper; spy trace overflow; no alternate TX symbol (nm); tests-off spy absent; gate mutation self-test | R2 authority implementation; real SX1262; Japan profile; legal certification; RF/HIL; production radio complete |
| R2 docs freeze (`pcp_r2_docs_gate` + [24章 §14](24-r2-physical-compliance-permit-authority.md) / [ADR-0004](adr/0004-r2-durable-permit-authority.md) / `src/radio/pcp_authority.h`) | complete private header; E entry RO snapshot; UNKNOWN 収束; recover empty-prefix; stage7; epoch!=; SEM mutations; **re-review GO 禁止** | R2 body implementation; legal; RF/HIL; re-review PASS |
