...
- **SQLite linkage / install / test backend matrix（normative host profile）:**
  ...
  **Installed package config mapping:** single-config `install(EXPORT)` は producer の1 config 分だけ `IMPORTED_LOCATION_<CONFIG>` を出す。`NinlilConfig.cmake` は (1) `Ninlil::ninlil_posix_sqlite_storage` に config 非依存 `IMPORTED_LOCATION` と不足 consumer config 向け `MAP_IMPORTED_CONFIG_*`、(2) FindSQLite3 の config-less location の standard config 名への **additive 複製のみ**（`INTERFACE_*`/link 非改変）を行い、空 `CMAKE_BUILD_TYPE` / Debug / Release / multi-config / 厳格 identity `CMAKE_MAP_IMPORTED_CONFIG_*` でも `find_package`+link+run できる。install-consumer smoke は1 install 後に match / no-build-type / Debug↔Release / strict-map を回帰する。
