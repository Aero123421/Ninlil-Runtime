...
- **SQLite linkage / install / test backend matrix（normative host profile）:**
  ...
  **Sanitizer artifact は非出荷:** `NINLIL_ENABLE_SANITIZERS=ON` の ASan/UBSan build は coverage を下げない。installed consumer は package/install/`find_package`/link/run を維持する。ASan global descriptor が absolute `-c` source path を `.rodata` に埋めるのは prefix-map では除去不能なため、**明示 `NINLIL_ENABLE_SANITIZERS` のときだけ** archive/object hygiene scan を skip する（compiler 推測・暗黙 fallback 不可。検査コード削除や ASan globals 無効化は禁止）。出荷 hygiene は非 sanitizer gate が保証する。
