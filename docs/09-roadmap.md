...
- **R7 T1 NRW1 SINGLE private pure wire codec implementation candidate Proposed**（[32章](32-r7-t1-nrw1-single-wire-codec.md) / [ADR-0012](adr/0012-r7-t1-nrw1-single-wire-codec.md) Proposed; DATA/SINGLE dual-envelopeの8 private API、bounded C11 codec、独立subset vectors、Host bridge、ESP final-ELF link smoke、fail-closed acceptance gate; local PRE-CI acceptance **P0=P1=P2=0 GO**）。**push/PR/ESP CIと独立POST-CI acceptance完了前のためAcceptedではない。full §18 artifact/state、counter/storage/replay/durable admission、FRAG/LINK/CELL/HA、W1/L1、実機KAT、RF/USB HIL、Japan legal、production radioを完成扱いにしない。**
