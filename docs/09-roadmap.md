...
- **R7 T1 NRW1 SINGLE private pure wire codec implementation candidate Accepted**（[32章](32-r7-t1-nrw1-single-wire-codec.md) / [ADR-0012](adr/0012-r7-t1-nrw1-single-wire-codec.md) Accepted / [review](reviews/2026-07-19-r7-t1-single-wire-codec-accepted.md); DATA/SINGLE dual-envelopeの8 private API、bounded C11 codec、独立subset vectors、Host bridge、ESP final-ELF link、fail-closed acceptance gate; push/PR/ESP CI全成功; independent POST-CI **P0=P1=P2=0 GO**）。**AcceptedはT1 candidateのみ。full §18 artifact/state、counter/storage/replay/durable admission、FRAG/LINK/CELL/HA、W1/L1、実機KAT、RF/USB HIL、Japan legal、production radioを完成扱いにしない。**
- **R7 T1b context binding / verified HKDF schedule implementation candidate Accepted**（[33章](33-r7-t1b-context-binding-hkdf.md) / [ADR-0013](adr/0013-r7-t1b-context-binding-hkdf.md) Accepted / [review](reviews/2026-07-19-r7-t1b-context-binding-hkdf-accepted.md); Hop/E2E canonical encode/digest、expected-digest必須のtyped key bundle、exact 24-vector oracle、Host/ESP final-link、fail-closed acceptance gate; push/PR CI・ESP CI全成功; independent POST-CI **P0=P1=P2=0 GO**）。**AcceptedはT1b private stateless候補のみ。expected digest/traffic secret provenance、context install、state/W1、実機HIL、legal、production radio、R7 fullを完成扱いにしない。**
- **R7 T1c Authenticated Hop Fresh-Install Owner（Proposed docs-only）**（[34章](34-r7-t1c-authenticated-hop-fresh-install-owner.md) / [ADR-0014](adr/0014-r7-t1c-authenticated-hop-fresh-install-owner.md) Proposed）。Hop一方向のみ: T1c-A accepted authority stamp adapter、T1c-B M4 incomplete Hop install token one-shot consume + copy-owned claim、node-id KDF closed mapping、sole `install_hop_accepted` owner、T1b verified derive → N6 Hop fresh 4-key FULL、FULL_OK only handle publish、CU handle0/`recover_cu` only、ALL_PROPOSED後 secret破棄+DORMANT・再注入禁止。**implementation / vectors / CTest / Accepted 未。** nonclaims: E2E install、resume/M5、W1/L1、counter/nonce/AEAD/replay、T1 composite、LINK/FRAG/CELL/HA、M4 handshake parser、R2 L1 full、provider統一、RF/USB HIL、Japan legal、production radio、R7 full。compile/link ≠ HIL。local-identity exact ABI 不変。public ABI 非変更。
