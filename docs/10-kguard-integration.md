# 10. KGuard Integration Profile

状態: Normative integration baseline<br>
対象: KGuardがNinlil Runtimeを利用する境界

## 原則

```text
KGuard application / management
        |
        | Ninlil public API
        v
Ninlil Runtime
...
- KGuardはNinlil public APIを利用します。
- Ninlil CoreはKGuard、Product V1、PoC1をimportしません。
- KGuardのschema、business rule、UI、SLO policyはKGuard側に置きます。
- KGuard固有enumをNinlil wire/coreへ追加しません。

## Legacy LinkOS Lab v1

現行`linkos/`は **Legacy LinkOS Lab v1** として凍結します。
...
- KGuard enumを追加しながら汎用coreへ育てること
...
| KGuard concept | Ninlil contract | Required evidence / rule |
| --- | --- | --- |
| 表示盤の絶対状態 | `DesiredStateCommand` | Pico/application反映後`APPLIED`。physical feedbackが追加された場合`VERIFIED` |
| 漏水/つまり等の発生・復旧transition | `EventFact` | Controller/KGuard journal commit後`DURABLY_RECORDED` |
...
「漏水なら表示を使用不可にする」はNinlilの責務ではありません。KGuard rule engineがEventFactを受け、別のDesiredStateCommandを生成します。

## Display flow

1. KGuardがlogical installation mappingを提供し、Ninlil Controller resolverがconcrete device + binding epochへ固定する。
2. Desired absolute state、generation、idempotency key、deadline、required evidenceを提出する。
3. Ninlilがadmission結果を返す。
4. `ADMITTED_READY/SCHEDULED`でもKGuard UIは「受付済み」であり「反映済み」と表示しない。
5. Endpoint Service AdapterがPicoへ適用する。
6. Picoの成功応答とpersistent result cache commit後だけ`APPLIED`。
7. KGuard UIはtarget別Outcomeを表示する。
...
KGuardはNinlil transaction Outcomeとは別に、`全完了 / 一部完了 / 未達 / 結果不明`のaggregate viewを作ります。一部失敗時はtarget別一覧を表示し、失敗targetだけを新しいrosterとtransactionで再実行します。交換・撤去済みtargetを元transactionへ暗黙追加しません。

## Leak Event flow

1. battery endpointがphysical transitionを検出する。
2. KGuard sensor adapterがstable event IDとpayloadを作る。
...
5. Controllerがevent identity/digest/payloadをKGuard journalへatomic commitする。
6. commit後だけ`DURABLY_RECORDED` Receiptを返す。
7. endpointはcustody release policyに従いspoolを解放する。

同じcontactがactiveのままwakeするたび新eventを作りません。KGuard schemaは`ACTIVE transition`、必要なperiodic reminder、`CLEAR transition`を区別します。

### Leak local admission failure

検知したこととEventFactをadmitできたことを分けます。Grant切れ、storage異常、spool full、profile不整合でlocal admissionできない場合、KGuard sensor adapterは次を行います。
...
6. spool解放のためのEventFact discardは、actor、理由、最終evidenceをKGuard auditへ保存した明示操作だけ許可する。

Storage自体が故障している場合に「検知を保存済み」と表示しません。Product profileはLED pattern、local fail-safe、retry間隔、operator escalation時間を具体値で定めます。

## KGuard end-to-end evidence view

Ninlil ReceiptとKGuard/cloud processingを1つの成功にまとめません。

| KGuard表示段階 | 根拠 |
| --- | --- |
| endpoint受付/保存 | endpoint local admission record |
| 現場PC保存 | Controller/KGuard journal commit後の`DURABLY_RECORDED` |
| KGuard rule処理済み | rule engineの独立operation record |
...
- reporting/analytics: KGuard側で必要な粒度を保存
...
7. evidence後にKGuard UIを「設置済み」へ変更
...
KGuard removal profileは作業期限、drain最大待機、assessment TTL、sleepy nodeのlast-seen許容age、未移行一覧、強制撤去承認者、撤去後reconcile/rollback期限を必須fieldにします。期限切れassessmentで物理撤去を許可しません。
...
- KGuardの現場PCで`ninlild`をSite Controllerとして動かします。
...
## KGuard policy, not Ninlil default

次はKGuard profileで決めます。
...
Operator-visible stateは[11-operator-model.md](11-operator-model.md)のstable codeへ写像し、KGuard profileで文言、owner、timeout、runbookを具体化します。特に`OP_RESULT_UNCERTAIN`、`OP_PARTIAL`、`OP_LOCAL_SAFETY`を単なる「失敗」にまとめません。

KGuard applicationがrequestごとにarbitrary priorityを上げることはできません。Site policyがservice identityへclass/quotaを割り当てます。
...
M1a完了後は、まずsoftware-only `TEST` laneでPC Controller + simulated/optional loopback EndpointとKGuard Display/Leak adapterのsingle-target縦切りを実施します。PC + USB Cell Agent + Display + Leak + SX1262のphysical `LAB` laneは、M3のESP-IDF/Cell Agent最小sliceとM5のLAB Tx Gate/radio subsetのgate完了後に開始します。どちらもproduction radioやfield SLOの合格を意味しません。
...
### Phase D: KGuard adapter

- Display/Leak schemaとServiceDescriptor
- Product V1 command/eventとのapplication-level adapter
...
- KGuard Coreを`rg`してNinlil portable Coreへのimportが0。
- DisplayはAPPLIED前に反映済み表示されない。
- Leak EventFactはKGuard journal commit前にDURABLY_RECORDEDにならない。
