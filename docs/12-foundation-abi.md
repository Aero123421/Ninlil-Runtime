...
- Retained attempt indexへ入るのは当該Runtimeがentropyから生成したApplication attempt IDとcancel-attempt IDだけです。Inbound messageがechoしたremote attempt IDは追加しません。各entryはparent transaction ID、Application/Cancel kind、attempt recordへbindingし、parentがnon-terminalの間とterminal transaction retention中は保持します。Exclusive retention endのcleanupはparent transaction/target/idempotency/evidence、全Application/cancel attempt recordとindex、transaction ID index、resource releaseを**1 FULL transaction**で削除します。Commit OK前はcollision、OK後だけ同じ128-bit candidateを将来再利用可能です。COMMIT_UNKNOWNは全groupのauthoritative resolutionまで再利用可否を推測せずnamespaceをfenceします。M1aのID uniqueness保証はactive + retained境界までで、永久tombstoneは保証しません。
...
Cached reverse Receipt/Disposition/CustodyAccepted/CancelResultはduplicate-safe at-least-once replyです。Durable cacheはsend state `PENDING`、`WAITING_RETRY`、`CLOSED_SENT_OR_UNKNOWN`、`CLOSED_DENIED`だけを持ちます。PENDING/WAITINGだけがautomatic send可能です。
...
- Exact duplicate inbound request/messageはclosed cached resultを再びPENDINGへするnew reply opportunityをFULL commitでき、application callback/result/custody mutationを繰り返しません。New permitを取得し、same logical reply bytes/bindingを送ります。
...
- Runtimeはnon-zero clock epochを持つtrusted local sampleからtoken expiryを作れる場合だけcallbackを呼びます。clock uncertain/port failureではtokenを発行せずretryable receiver-unavailable pathへ進みます。
...
- M1a delivery tokenは`context_id = delivery.transaction_id`、`generation = receiverでの同transaction callback invocation count`（1始まりのchecked `uint64_t`）、callback前clock sampleのepoch ID、`expires_at_ms = completion_expires_at_ms`です。tokenはそのRuntime内のtransaction/delivery/digest/expiryへbindingし、別Runtime/transactionへ移植できません。generation increment不能ならcallbackを呼ばず`COUNTER_EXHAUSTED`でfail closed/reconcileします。
- `delivery_view.delivery_count == token.generation`です。Reconcile viewの`prior_callback_invocations`はこのdeliveryで既に開始commitしたcallback回数のchecked uint64で、次のredelivery generationを作る前の値です。どちらもnarrowingしません。
- Runtimeはcallbackを呼ぶ**前**に上記tokenを割り当て、`delivery_started_at_ms`と`completion_expires_at_ms = checked(delivery_started_at_ms + application_completion_timeout_ms)`を計算し、`DELIVERY_STARTED`、`token_valid = 1`、transaction/delivery/digest binding、prior invocation countを1つの`FULL` transactionへcommitします。commit失敗または不明ならcallbackを呼びません。
- `CALLBACK_COMPLETE`では`out_sync_result`がvalid必須です。Runtimeがcallback return後にresult cache/Dispositionとtoken invalidationを同じ`FULL` transactionへcommitします。commit前にReceiptを発行しません。
...
- `CALLBACK_DEFER`ではresultを無視します。Applicationはcallback中にdelivery tokenの値をcopyし、`ninlil_delivery_complete()`成功またはcompletion timeoutまでだけ使用できます。token pointer自体を保持してはなりません。
- Deferred中、payload pointerは無効になります。Applicationは必要なdataを自分でcopyします。
- `CALLBACK_FATAL`、unknown callback action、`CALLBACK_COMPLETE`だがinvalid resultでは、callback前にcommit済みtokenを放置しません。Runtimeはtoken invalidation、active slot release、Delivery=`RECOVERY_REQUIRED`、effect certainty=`EFFECT_POSSIBLE`を1つのFULL transactionへcommitし、positive Receiptを生成しません。reasonはFATALで`APPLICATION_FAILED`、unknown/invalidで`CALLBACK_CONTRACT`、Runtime healthは`DEGRADED`です。Controller receiverでは`controller.before_callback_recovery_commit` / `controller.after_callback_recovery_commit`、Endpoint receiverでは`endpoint.before_callback_recovery_commit` / `endpoint.after_callback_recovery_commit`のexact pairを通ります。
...
- `on_reconcile`の`REDELIVER`は再callback、`KNOWN_RESULT`はresult commit、`RETRY_LATER`はdescriptorのfixed `retry_backoff_ms`だけを使うlocal待機、`OUTCOME_UNKNOWN`はremoteへunknown Dispositionを返します。`RETRY_LATER`では`out_known_result`全体をignoreするため、Application指定delayは存在しません。Runtimeはtrusted local sampleから`retry_not_before_ms = checked(now_ms + descriptor.retry_backoff_ms)`を作り、clock uncertain/epoch不一致/overflowではtimerを推測せずfail closedします。Public/remote absolute retry timeを生成しません。
- Unknown reconcile actionまたはinvalid `KNOWN_RESULT`は`RECOVERY_REQUIRED`を保持し、health `DEGRADED` / reason `CALLBACK_CONTRACT`です。known result、Disposition、Receiptをcommitせず、同じrecovery passで再callbackしません。
...
- completion expiryはcallback return時ではなく、callback前にcommitした`delivery_started_at_ms + application_completion_timeout_ms`です。callback実行時間もbounded windowへ含めます。
- callbackが`DEFER`を返した場合、新しいdurable writeへ依存せず、callback前にcommit済みの`DELIVERY_STARTED` tokenをactive `DEFERRED_WAIT` projectionとして保持します。
- callback中にprocessがcrashした場合もdurable `DELIVERY_STARTED`が残ります。recoveryは旧processでcopyされたtokenをcompletion可能とせず、apply contractに従い`on_reconcile`またはidempotent redispatchへ進めます。
- timeout到達時、Runtimeはtoken invalidationとactive slot解放をFULL commitし、deliveryをinternal `RECOVERY_REQUIRED`へ移します。
...
| 4b. context IDはknownだがactive tokenなし、retained consumed/expired tombstone、またはactive contextに対してgeneration/clock epoch/expires-at/bindingが不一致 | `NINLIL_E_INVALID_STATE` | authoritative recordを変更せず、実際のactive tokenが別にあればactiveのまま |
...
Complete成功/timeout成功でactive deferred slotを直ちに解放します。Invalid token recordはserviceの`required_dedup_window_ms`以上かつruntime `result_cache_retention_ms`以内のbounded result-cache retentionで保持します。保持中のlate/double completionは`NINLIL_E_INVALID_STATE`、retention後は`NINLIL_E_NOT_FOUND`です。Epoch changeまたはRuntime restartで旧tokenをactiveへ復元しません。
...
- EventFact admissionは`NINLIL_M1A_MAX_EVENT_RESUME_OPERATIONS`個のfixed resume operation/audit/result slotと、1個のdiscard operation/audit/result slotをevent spool bytes内にreserveします。Management ledger reservationはchecked `8 * 256 + 512 = 2560` logical bytesです。Portable `EVENT_SPOOL_BYTES` admission costはexactly `payload.length + NINLIL_M1A_EVENT_MANAGEMENT_RESERVATION_BYTES`で、attempt detail/retry summary/cumulative summaryのphysical storage overheadを含めません。reservation不能ならadmitしません。slotはevent terminal後もservice dedup retention終了まで保持し、silent eviction/reuseしません。
...
| `on_delivery` | callback前のDELIVERY_STARTED/token claim commit。DEFERでも同じ |
...
- Cleanupはrecordとそのdedup/evidence/resource reservationを1 FULL transactionで削除/releaseし、capacity blocked improvementがあれば同じcommitでepochを進めます。Partial cleanup、terminalより早いresult/evidence delete、untrusted-clock deleteは禁止です。
...
| `DEFERRED_TOKEN` | active delivery token 1件 / `max_deferred_tokens` | DELIVERY_STARTED/DEFERRED_WAIT active token | callback前token slot | delivery start前→complete/timeout/recovery invalidation commit |
...
| `on_delivery` dispatch | callback 1 + state transitions 2 | 1つ目はcallback前DELIVERY_STARTED、2つ目はCOMPLETE/FATAL/contract result。DEFERなら2つ目を返す。Callback return resultをvolatileに次stepへ持ち越さない |
...
3. Active tokenが1件以上あれば、`context_id` unsigned-byte lexicographic、次にgeneration昇順で全件を列挙し、token invalidation、active slot release、Delivery `RECOVERY_REQUIRED`を**1つのStorage transaction / 1回のFULL commit**へstagingします。`runtime.before_destroy_recovery_commit` / `runtime.after_destroy_recovery_commit`はgroup全体に各最大1回です。個別token commitやpartial group visibilityを許しません。Active token 0ならStorage write/hook 0です。
...
| `CALLBACK_FATAL`、unknown callback action、invalid COMPLETE resultのtoken invalidation + `RECOVERY_REQUIRED` commit | Receiver roleに応じた`controller.before_callback_recovery_commit` / `controller.after_callback_recovery_commit`または`endpoint.before_callback_recovery_commit` / `endpoint.after_callback_recovery_commit` | callback前commit failure、reconcile callback result（`*_reconcile_commit` pairを使用） |
