...
本章のsemantic stateをprivate Storage key/value、atomic witness、retention、capacity contributionへ写像する正本は[17. Foundation Domain Store v1](17-foundation-domain-store.md)です。本章のC structやstate表をそのままmemory dumpして保存しません。
...
- NINLIL_M1A_MAX_EVENT_RESUME_OPERATIONS (=8) 件のresume operation / audit / resultと1件のdiscard operation / audit / resultに対するlogical fixed capacity slot。EVENT_SPOOL_BYTESへchecked `8 * 256 + 512 = 2560` logical bytesをpre-reserveし、未使用slotのStorage recordはmaterializeしない
...
- 使用済みEvent resume / discard operationのaudit / request digest / result recordはterminal後のservice dedup retention終了までsilent eviction / reuseしません。各successful resume / discardは対応logical slotだけを`reserved`から`used`へ移し、そのoperation IDのledger recordを初めてmaterializeし、live total/high-waterを増やしません。Required Receipt / discard terminal commitはpayload `used`と未使用slot `reserved`を解放し、terminal retention中のportable `used`はexactly `successful_resume_operation_count * 256 + (discard_operation_committed ? 512 : 0)`、`reserved = 0`です。Terminal Eventへ新operationを受理するreusable slotにはしません。
...
Receipt issuerはapplication resultまたはingress evidenceをdurably commitしたlogical timeを`ninlil_time_sample_t evidence_time`として送ります。ControllerもReceipt自体をdurable ingress（ORDERED_INGRESS create）する時にlocal `ninlil_time_sample_t controller_ingress_time`を固定します。比較可能性は以下のexact fieldだけで判定します。
...
**Durable source（crash/restart後のM1A-TIME-009/010再現）**: Controller durable ingress time sampleの永続正本は17章ORDERED_INGRESS bodyの`controller_ingress_clock_epoch` / `controller_ingress_at_ms` / `controller_ingress_trust`だけです。SCHEDULER_OWNER logical time、CLOCK_BASELINE、EVIDENCE_CELL、TRANSACTION_STATE、witness、in-memory reducer inputだけの保持を代替にしてはなりません。VALID_RECEIPT reducer inputは実行中に同値を運びますが、未reduceのdurable copyはORDERED_INGRESS rowです。RECEIPT reductionは同じFULL transactionでTRANSACTION_STATEのdeadline verdictとEVIDENCE_CELL（issuer evidence_timeのみ）を固定してからORDERED_INGRESSをeraseします。EVIDENCE_CELLへcontroller sampleを保存しません。
...
| DEFERRED_WAIT | callbackがDEFERを返した**同一Runtime instanceのin-memory/public projection**。Domain Store physical `RESULT_CACHE.delivery_state`は**2 DELIVERY_STARTEDのまま**ACTIVE tokenを保持する（追加FULL writeなし。storage値3はV1 illegal） |
...
DesiredState Endpoint Deliveryはstateと直交するcancel tombstoneを持ちます。Tombstoneはmatching dedicated cancel attempt ID、original transaction / source / target / service / digest / generation binding、dispatch_fenced、cached CANCEL_RESULTを保持し、terminal result-cache retentionまで削除しません。Private Domain Storeでは`RESULT_CACHE.cancel_result_kind`が`FENCED_BEFORE_DISPATCH`（never-startedのみ）または`TOO_LATE_EFFECT_POSSIBLE`（1回でもDELIVERY_START/effect possibility後）をdelivery_stateと直交に保持します（17章）。

DELIVERY_STARTEDのdurable RESULT_CACHEは最低限、delivery tokenのcontext ID / generation / clock_epoch_id / expires_at_ms、delivery identity、delivery_started_at clock epoch / ms、completion_expires_at_ms、`token_state=ACTIVE`、およびcounter `N = delivery_count = callback_invocations = token_generation`を持ちます。M1aのcontext IDはtransaction IDと完全一致し、`N`/token generationはそのreceiverで同Deliveryに対して**DELIVERY_START FULL commitが発行した**1始まりchecked uint64です（actual `on_delivery`開始回数。REDELIVERでは増やさない）。tokenのclock_epoch_idはdelivery_started_atのepoch（both non-zero exact一致）、expires_at_msはcompletion_expires_at_msと一致かつ both non-zeroです。**`delivery_started_at_ms` は任意 u64（0 可）** — trusted clock `now_ms=0` は legal。timer retain 時も started_at の non-zero を要求しません（17章 timer 5-tuple）。tokenは発行Runtimeとtransaction / delivery / digest / expiryにbindingし、同じtokenを別Runtimeまたは別Deliveryへ再利用してはなりません。
...
| VALID_RECEIPT | transaction、target、issuer、self-contained service identity value、stage、content digest/generation、`ninlil_time_sample_t evidence_time`、Controller durable ingress time（durable正本はORDERED_INGRESS `controller_ingress_*`。EVIDENCE_CELL非保存）、exact evidence bytes |
...
- Retained indexへ入るのはlocal Runtimeが生成したApplication/cancel attempt IDだけで、remoteからechoされたIDは追加しません。Entryはparent transaction/kind/attempt recordへbindingし、parent nonterminal中とterminal transaction retention中は削除しません。Exclusive retention cleanupは17章CLEANUP_PLANでdetailをbounded batch化し、index phase中はnamespace attempt-reuse fenceによりnew allocation 0、FINALIZE authoritative commit後だけcandidate再利用可です。各batch COMMIT_UNKNOWNはplan generation/member truthを解決するまでblind retryせずfenceを維持します。
...
| ABSENT | schema/digest/authorization不正 | DISPOSITION_COMMITTED | corresponding Disposition。Private Domain StoreはAPPLICATION_FIRST originのRESULT_CACHE（`application_seen=1`、`application_attempt_count≥1`、`N=0`、token NONE、E_DISP、**cancel_result_kind=0 (F_NONE)**、completed non-zero）をFULL commit。pre-callback state5にF_FENCEDを載せない |
| ABSENT | durable inbox / delivery / result capacity不足 | DISPOSITION_COMMITTEDまたはno allocation | exact CAPACITY_EXHAUSTED / NO_EFFECT_PROVEN / RETRY_SAME_AFTER matrix。Disposition record自体も確保不能ならBearer-level no-allocation failure。確保できる場合のRESULT originは上と同じAPPLICATION_FIRST pre-callback terminal |
| INBOX_COMMITTED | EventFactはno deadline。Commandはtrusted current sampleがdeadline epochと一致かつnow < absolute deadline。token capacityあり、prior counter `N + 1`がchecked uint64で表現可能 | DELIVERY_STARTED | operation kind **9 phase 1**。`N` 3値を同一postへchecked +1、ACTIVE token、role `*_delivery_started_commit` の後に application callback |
...
| INBOX_COMMITTED | DELIVERY_STARTで`N+1`不能（**prior `N=MAX`かつretained tombstone必須**） | RECOVERY_REQUIRED | operation kind **9 phase 2**。callback 0、`N=MAX`+tombstone維持、E_REC `COUNTER_EXHAUSTED`、role `*_delivery_start_counter_exhausted_commit`（started hook 不使用）。`N=0`+NONE START不能は到達不能 |
...
| DELIVERY_STARTED | callbackがDEFERを返す | DEFERRED_WAIT（in-memory/public projectionのみ） | **追加FULL writeなし**。physical RESULT_CACHEは`delivery_state=2` + ACTIVE tokenのまま。budget上の未使用第2 state-transition reservationは既存規則どおり同一stepへ返す |
...
| DELIVERY_STARTED | CALLBACK_FATAL | RECOVERY_REQUIRED | kind11: `RECOVERY_REQUIRED_TOMBSTONE` + E_REC `APPLICATION_FAILED` + EFFECT_POSSIBLE、slot release、health DEGRADED、Receipt 0 |
| DELIVERY_STARTED | unknown callback action、またはCALLBACK_COMPLETE + invalid result | RECOVERY_REQUIRED | kind11: `RECOVERY_REQUIRED_TOMBSTONE` + E_REC `CALLBACK_CONTRACT` + EFFECT_POSSIBLE、slot release、health DEGRADED、Receipt 0 |
| DELIVERY_STARTED | effect後か不明、result未commit（authoritative 収束後） | RECOVERY_REQUIRED | `RECOVERY_REQUIRED_TOMBSTONE`（または ACTIVE 回収なら EXPIRED）+ E_REC **`OUTCOME_UNKNOWN`**、Receipt 0 |
| DEFERRED_WAIT（physicalはDELIVERY_STARTED + ACTIVE） | DELIVERY_COMPLETE_REQUEST、context ID = transaction ID、exact active generation=`N`、token clock epoch / expires-at = durable marker、発行Runtime / delivery / digest binding一致、trusted current sampleが同epochかつnow <= expires-at | RESULT_COMMITTEDまたはDISPOSITION_COMMITTED | kind **10 phase 1**: CONSUMED + E_POS/E_DISP、slot 解放 |
| DEFERRED_WAIT（physicalはDELIVERY_STARTED + ACTIVE） | DEFER_CONTEXT_TIMEOUT、同じtoken generation=`N` | RECOVERY_REQUIRED | kind **10 phase 2**: EXPIRED + E_REC `APPLICATION_COMPLETION_TIMEOUT`、slot 解放、Receipt 0 |
| token tombstone retention中（CONSUMED/EXPIRED/RECOVERY_REQUIRED_TOMBSTONE、known context） | DELIVERY_COMPLETE_REQUEST | current不変 | NINLIL_E_INVALID_STATE |
| token/RESULT record不存在（cleanup後を含む） | DELIVERY_COMPLETE_REQUEST | current不変 | NINLIL_E_NOT_FOUND |
| RECOVERY_REQUIRED | RECONCILE_RESULT=KNOWN_RESULT | RESULT_COMMITTEDまたはDISPOSITION_COMMITTED | known resultをcommit。`token_state=CONSUMED`、completed non-zero。reconcileはD_HELD（G/I保持、timer zero） |
| RECOVERY_REQUIRED | RECONCILE_RESULT=REDELIVER | INBOX_COMMITTED | **`N`を増やさない**。past token tombstoneとreconcile history（D_HELD）を保持し、application result/recovery partial tupleはE_ZEROへ戻す。次のDELIVERY_STARTだけが`N`をchecked +1し新ACTIVE tokenを作る |
| RECOVERY_REQUIRED | RECONCILE_RESULT=RETRY_LATER、trusted local clock、descriptor fixed retry_backoffをchecked addition可能 | RECONCILE_WAIT | `reconcile_retry_generation=checked(G+1)`（`I`維持）と`retry_not_before = local now + descriptor.retry_backoff_ms`をatomic commit。postは`G≥1,I≥0,G≤I+1`（**`G=I+1`非必須**）。out result全体をignore。G+1不能ならtimer 0・RECOVERY維持・`COUNTER_EXHAUSTED` |
| RECOVERY_REQUIRED | RECONCILE_RESULT=OUTCOME_UNKNOWN | DISPOSITION_COMMITTED | OUTCOME_UNKNOWN Disposition、positive Receiptなし。`token_state=CONSUMED`（retained tombstone一意形）、completed non-zero、reconcile D_HELD（`I≥G`） |
| RECOVERY_REQUIRED | unknown reconcile action、またはKNOWN_RESULT + invalid result | RECOVERY_REQUIRED | E_REC reason を **`CALLBACK_CONTRACT` へ置換**（tombstone identity/timers/`N`維持）、health prio3 成立、positive Disposition/Receipt 0、same pass 再callback 0。後続再dispatchは別claimで`I`+1（`I>G`可）。valid terminal reconcile で source clear |
| DELIVERY_STARTED（DEFERRED_WAIT projectionを含む）、RESULT_COMMITTED、DISPOSITION_COMMITTED、RECOVERY_REQUIRED、RECONCILE_WAIT | REMOTE_CANCEL_REQUEST | current + cancel tombstone | effectを取り消さずcached CANCEL_RESULT=TOO_LATE_EFFECT_POSSIBLE（`cancel_result_kind=F_LATE`）をFULL commit後にreverse send |
...
Private Domain Storeは先行cancel ownerをdirect lookupするため、上表の`ABSENT + cancel tombstone`をprivate `DELIVERY.creation_kind=CANCEL_FIRST`、`RESULT_CACHE.delivery_state=CANCEL_TOMBSTONE_ONLY`として物理保存します。これはsemantic/public Delivery presentではありません。Logical delivery keyはattempt IDを含まず、後着APPLICATIONはsource/transaction/target keyとservice/content/generation等のfull bindingがexact一致する場合だけ既存rootへattachし、Application ATTEMPT、inbox、callback、effectを追加しません。Binding不一致は既存rootを変更しないconflictです。
...
- Runtimeはcallbackを呼ぶ前にnon-zero epochのtrusted local clockをsampleし、同じepoch上で`completion_expires_at_ms = checked(delivery_started_at_ms + application_completion_timeout_ms)`を計算します。**`now_ms=0` / `delivery_started_at_ms=0` は legal**（timeout≥1 により expiry non-zero）。clock uncertain / port failure / **overflow**ではtokenを発行せずcallbackを呼びません。
- **DELIVERY_START kind9 phase1 だけ**がcounter `N` 3値を同一postへchecked +1（初回 post generation 1）。increment不能は kind9 phase2 で callback 0、tombstone維持、E_REC `COUNTER_EXHAUSTED`。REDELIVERは`N`を増やしません。
- callback前のFULL transactionは physical DELIVERY_STARTED、context=transaction ID、generation=post `N`、binding、completion clock、`token_state=ACTIVE`を含みます。
- 上記 success 後だけ on_delivery へ token を渡します。
- DEFER は追加 FULL write なし（state2+ACTIVE）。DEFERRED_WAIT は in-memory/public projection のみ。
- process restart / create recovery（kind21）および controlled destroy（kind19）は旧 ACTIVE を completion 可能に戻さず **EXPIRED + E_REC `OUTCOME_UNKNOWN` + RECOVERY_REQUIRED** へ収束します（timeout reason と混同しない）。
- clock epoch mismatch の ACTIVE 回収も kind21 で **EXPIRED + OUTCOME_UNKNOWN**；`CLOCK_UNCERTAIN` は RESULT に書かず CLOCK_FENCE health のみ。
- 同期 complete / delivery_complete 成功は kind10 phase1（CONSUMED+terminal）。timeout は kind10 phase2（EXPIRED + `APPLICATION_COMPLETION_TIMEOUT`）。
- delivery_complete と timeout が同時刻なら complete を先に処理します。raw COMMIT_UNKNOWN 中は RESULT を推測 mutation しません。
- active token slotを解放しても、result / Dispositionまたはreconcileが完了するまでdelivery reservationは解放しません。
- token tombstoneは12章のrequired_dedup_window_ms / result-cache retention contractでboundedに保持します。retention中のlate delivery_completeはNINLIL_E_INVALID_STATE、retention/cleanup後のrecord不在はNINLIL_E_NOT_FOUNDです。
- expired/consumed/recovery tombstone recordはACTIVE completionを受理せず、application result読取りでACTIVEへ戻しません。
...
Deliveryはdurable `reconcile_retry_generation`（`G`）と`reconcile_invocation_count`（`I`）を持ちます（17章RESULT_CACHE）。

- same-record 共通不変: recovery履歴ありでは **`G≥1`、`I≥0`、`G≤I+1`**（`I>G`合法）。**`G=I+1`は必須ではない**。
- 最初にRECOVERY_REQUIREDへ入るcommitで`G=1,I=0`（D_OPEN）。独立episode再入では`G=checked(G+1)`、`I`維持（postは`G≤I+1`；過去crashがあれば`I≥G`もあり得る）。G/Iを0へ戻しません。
- `I`は**actual `on_reconcile` entry回数**です。各entryの直前FULL claimが`I=checked(I+1)`し、`G`は維持します。同Gのcrash後再実行も**別claim**として`I`を再度+1します。claim済み/未claimをG/I算術だけで区別せず、RESULT_CACHE same-record factorは **D_OPEN**（timer zero）に統合します（詳細は17章；actual claimはD3）。
- 同じlogical claimのCOMMIT_UNKNOWN retryだけ、同じpost-`I`をkind 11 identityへ再利用します。authoritative claim commit後の再callbackはnew post-`I`です。
- `RETRY_LATER` result commitだけが`G=checked(G+1)`とnot-beforeをatomic保存し（`I`維持、D_WAIT）、due後はtimer clearでD_OPENに戻ります。
- `G`または`I`の次increment不能、またはnot-before checked導出不能ではcallback/timer successを作らず`COUNTER_EXHAUSTED` markerをcommitしてRECOVERY_REQUIREDに留めます。
- KNOWN_RESULT、OUTCOME_UNKNOWN、REDELIVER成功はreconcile workを終了し、active timerだけzeroにした**D_HELD**（成功action claimありのため`I≥G`）として`G`/`I`を保持します（再初期化しない）。

| Action | Guard / durable transition | Result |
| --- | --- | --- |
| KNOWN_RESULT | out_known_result valid。result cache / DispositionをFULL commit。token CONSUMED、completed non-zero、D_HELD | positiveならReceipt、negativeならDisposition |
| REDELIVER | apply contractがIDEMPOTENTまたはapplication dedup対応。**`N`（delivery_count/callback_invocations/token_generation）は増やさない**。INBOX_COMMITTEDへ戻しpast token tombstoneとD_HELDを保持、result tupleはE_ZERO | 次のDELIVERY_STARTだけが`N`を+1して新ACTIVE tokenを作りon_deliveryを実行 |
| RETRY_LATER | trusted local clockからdescriptor fixed `retry_backoff_ms`だけでinternal not-beforeをchecked導出可能。`G=checked(G+1)`、`I`維持 | RECONCILE_WAIT。out result全体をignore |
| OUTCOME_UNKNOWN | effect可能性とunknown DispositionをFULL commit。token CONSUMED、D_HELD | positive Receiptなし |

- REDELIVERで同じtransaction / event IDとpayload digestを維持します。`N`のincrementはDELIVERY_STARTだけです。次STARTで`N+1`がchecked不能ならcallback 0のままRECOVERY_REQUIRED + `COUNTER_EXHAUSTED`に留め、KNOWN_RESULTまたはOUTCOME_UNKNOWNによるreconcileを必要とします。
...
- on_reconcile中にcrashした場合、RECOVERY_REQUIREDまたはRECONCILE_WAITのdurable stateから再実行します。再entry前のclaim FULLが`I`を+1します。
- KNOWN_RESULTのcommit acknowledgement不明ではReceiptを発行せず、storage recovery後にresult cacheを読み直します。
- unknown reconcile actionまたはinvalid KNOWN_RESULTは RECOVERY 維持のまま E_REC reason を **`CALLBACK_CONTRACT` へ置換**（tombstone/`N`維持）、health prio3 を成立させ、positive Disposition/Receipt 0、same pass 再callback 0。valid terminal reconcile で source clear。
...
Admission時に、raw detail `L`件とは別に固定長evidence summary recordを1件確保します。Public EVIDENCE accountingはtargetごとにsummary 1 + raw `L`の`L + 1` slotsで、admission commit直後はsummary used=1、raw reserved=`L`です。Domain Storeのphysical wireは17章`EVIDENCE_CELL`（subtype `0x32`）であり、SUMMARY + raw `L`を最大encoded sizeのfixed-size cell（TX body exact 734 / DELIVERY body exact 798、cell lifetime中value length不変）としてadmission時にmaterializeします。Summary cellが最低限持つdurable fieldは次です。

- highest_receipt_stage
- latest_evidence_stage（public値はmonotonic highestと常に一致）
- latest issuer identity
- latest materialのdurable ingress sequence
- latest materialのissuer `evidence_time`（clock epoch / now / trustのvalue copy）。**EVIDENCE_CELLへ保存するtime tripleはissuer `evidence_time`だけ**です
- latest bounded evidence dataとそのdigest
- late_evidence_present（SUMMARY `late_material`、`late_evidence_count > 0`とexact一致）
...
Controller durable ingress timeは、実行中のVALID_RECEIPT reducer inputと、17章ORDERED_INGRESS durable copy bodyの`controller_ingress_*`（local copy metadata、Bearer `message_semantic_digest`非包含）が保持します。同じFULL reductionでdeadline verdictをTRANSACTION_STATEへ固定し、EVIDENCE_CELLをissuer evidence_time側だけ更新してからORDERED_INGRESSをeraseします。**summary / raw EVIDENCE_CELLのdurable fieldにはcontroller sampleを保存しません**（issuer `evidence_time` tripleのみ）。latest fieldsの並び替え鍵はController durable ingress **sequence**（EVIDENCE_CELLが保持）であり、Controller ingress time sample自体ではありません。

Receipt分類:

- exact duplicate: transaction、target、issuer、service identity、stage、content/generation、derived evidence digest、exact evidence bytes、issuer `evidence_time`のepoch / now / trustが全て一致。**Controller durable ingress timeはduplicate tupleに含めません**（元仕様どおり）。
...
New materialをrawへ追加すると1 slotだけreserved→usedへ移し、raw `L`件後はsummary updateだけでslot数を変えません。Terminal commitはunused raw reservationを解放せず、summary/raw usedとともにtransaction evidence retention cleanupまで保持します。Summary + raw L cellは12章 / 17章どおり最大encoded sizeのfixed-size EVIDENCE_CELL（ownerがTRANSACTIONならbody exact 734、DELIVERYならexact 798）としてadmission時にphysical materializeし、cell lifetime中value lengthを変えず、replacement用journal headroomもpreallocateします。したがってunrelated workで後からStorageがfullでも、admitted transactionのlate materialは残るraw cellまたは既存summary replacementへ記録できます。Storage I/O/corruption/commit unknownは別のfail-closed pathです。SummaryはTRANSACTIONへ暗黙内包せず、EVIDENCEの`+1` unitとして12章limitへ含めます。
...
| on_delivery dispatch | callback 1 + state transitions 2 | callback前DELIVERY_STARTEDとreturn後COMPLETE/FATAL/contract result。DEFERは追加durable writeなしで2つ目のstate-transition reservationを返し、volatile resultを次stepへ持ち越さない |
...
- Per-delivery durable `N = delivery_count = callback_invocations = token_generation`はchecked uint64で、narrowingしません。増加はDELIVERY_START FULL commitだけです。`prior_callback_invocations` public viewは当該Deliveryで既に開始commitしたcallback回数（=`N`）を写します。`application_callback_invocations`と`reconcile_invocations`は別のuint64 metricsでactual function entry直前に各1増やします。per-delivery durable `reconcile_invocation_count`はactual on_reconcile entry回数で、同Gのcrash再実行でもclaimごとにchecked +1し`I>G`を許します。Per-step `callbacks_invoked`は両callbackを合算するbounded ABI counterで、durable countやmetricsの代用ではありません。
...
- DEFER return後に追加FULL write / token commitを要求しません。physical DELIVERY_STARTED + ACTIVE recordがrecoveryの正本です。
...
- physical DELIVERY_STARTED + ACTIVEだけが残るEndpointは、callback中crash、DEFER projection中crash、sync result commit前crashを区別できないため、**EXPIRED + E_REC `OUTCOME_UNKNOWN` + RECOVERY_REQUIRED**へcommitし、on_reconcileへ進みます（`APPLICATION_COMPLETION_TIMEOUT` に倒さない）。
...
- M1A-END-008: callbackがDEFERを返した後、追加FULL writeなし。physical RESULT_CACHEはDELIVERY_STARTED + ACTIVEのまま、DEFERRED_WAITはin-memory/public projectionのみ。
...
- M1A-END-011: token timeoutはkind10 phase2で EXPIRED + E_REC `APPLICATION_COMPLETION_TIMEOUT` + RECOVERY_REQUIRED を atomic commitし active slot 解放。
- M1A-END-012: timeout後retention中のlate delivery_completeはNINLIL_E_INVALID_STATE、retention後はNINLIL_E_NOT_FOUND。
- M1A-END-013: timeoutと同時のstorage failure / COMMIT_UNKNOWNではtokenをsuccess完了と推測せず、authoritative all-or-none解決後だけ該当pathへ。
- M1A-END-014: on_reconcile KNOWN_RESULTはresult commit後だけReceipt。
- M1A-END-015: on_reconcile REDELIVERは`N`を増やさずINBOXへ戻し past tombstone 保持。次のDELIVERY_START kind9 phase1だけが`N`を+1して新ACTIVEでcallback。
...
- M1A-END-017: on_reconcile OUTCOME_UNKNOWN actionは positive Receiptなし（terminal E_DISP OUTCOME_UNKNOWN）。E_REC 集合にも `OUTCOME_UNKNOWN` を含み destroy/restart/clock ACTIVE 回収に使う。
- M1A-END-018: callback前 token commit の crash で application callback invocation 0。
- M1A-END-019: DEFER中process crash / restart は旧 ACTIVE を EXPIRED + E_REC `OUTCOME_UNKNOWN` + RECOVERY_REQUIRED へ（timeout reason と混同しない）。
...
- M1A-END-026: reconcile REDELIVERではcontext_idとtransactionを維持し、`N`を変えず旧token tombstoneを保持してINBOXへ戻す。次DELIVERY_START kind9 phase1のpost `N=prior+1`だけが新ACTIVE generationを発行する。
...
- M1A-END-028: prior `N=UINT64_MAX`ではkind9 phase2で new token/callback 0、tombstone維持、RECOVERY_REQUIRED + E_REC `COUNTER_EXHAUSTED`（`*_delivery_start_counter_exhausted_commit`）。
- M1A-END-029: sync resultがtoken expiryと同時刻ならkind10 phase1を先にcommit。expiry超過はkind10 phase2 timeout。clock epoch mismatch の ACTIVE 回収は kind21 で EXPIRED+`OUTCOME_UNKNOWN`（RESULT に CLOCK_UNCERTAIN を書かない）。
...
- M1A-END-031: CALLBACK_FATALは `RECOVERY_REQUIRED_TOMBSTONE` + E_REC `APPLICATION_FAILED` + EFFECT_POSSIBLE + slot release FULL commit、health DEGRADED、Receipt 0。
- M1A-END-032: unknown callback actionまたはinvalid COMPLETE resultは `RECOVERY_REQUIRED_TOMBSTONE` + E_REC `CALLBACK_CONTRACT` で recovery commit、same Runtime instanceの再callback 0。
- M1A-END-033: fatal/contract recovery commit failureまたはCOMMIT_UNKNOWNはReceipt / redelivery 0、health DEGRADED、authoritative all-or-none 解決まで RESULT を推測 mutation しない。
- M1A-END-034: unknown reconcile actionまたはinvalid KNOWN_RESULTは RECOVERY 維持、E_REC を `CALLBACK_CONTRACT` へ置換、tombstone/`N`維持、health prio3、positive Disposition/Receipt 0、same pass 再callback 0。
- M1A-END-035: delivery_count / token generation / prior_callback_invocationsはUINT32_MAXを超えてもuint64で一致し、narrowing 0。Prior=UINT64_MAX-1はgeneration UINT64_MAXを1回発行可能、prior=UINT64_MAXの次incrementだけfail closed（kind9 phase2）。
