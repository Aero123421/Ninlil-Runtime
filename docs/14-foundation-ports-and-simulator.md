...
| ESP-IDF storage/clock/entropy/execution port | M3 | 本章のportable conformance subsetとpower-cut HILが必須。M3-prepは[18章](18-m3-prep-esp-idf-component.md)、basic adaptersは[20章](20-m3-basic-esp-idf-platform-adapters.md)、storage candidateは[21章](21-m3-esp-idf-durable-storage.md)。storageはhost conformance済みだがESP FULL unproven / HIL未実行のためrow全体は未達 |
...
| reference application adapter | M6 | generic fixtureを置換してもCore/portへapplication-specific ruleを追加しない |
...
- caller-owned adapter state を shutdown / zero 化 / 再利用する前に、全 adapter call を quiesce し、その ops を借用する Runtime の `ninlil_runtime_destroy()` を完了します。shutdown と同時の Runtime call は契約外です。
...
Open mapperはstatus/handle shapeをstatus mappingより優先します。Storageは`OK + handle`だけ継続、`OK + NULL`、non-OK + handle、unknownを`STORAGE_CORRUPT`へ正規化し、unexpected handleをexactly once closeします。Bearerは`OK + handle`だけ継続、`OK + NULL`、non-OK + handle、unknownを`DEGRADED`へ正規化し、unexpected handleをexactly once closeします。Known non-OK + NULLはCR5/CR7のmappingを使います。
...
#### M1a Core planとprovider staging minimum

M1a Coreの全READ_WRITE operation builderはbegin snapshotからcanonical final viewを先に確定し、差分を次のnormal formで発行します。

1. final viewに存在し、begin viewから新規またはcomplete encoded valueが変わる各keyを、final valueで最大1回`put`する。
2. begin viewに存在し、final viewに存在しない各keyを最大1回`erase`する。
3. transient value、同一keyの反復replace、put後erase、final viewにない一時keyを発行しない。

従って各call後のmaterialized viewは`begin keys ∪ final keys`の部分集合であり、各valueも対応するbegin/final valueのどちらかです。Capacity内のbegin/final viewに対するclosed boundは次です。

```text
begin_entries  <= max_entries
final_entries  <= max_entries
begin_bytes    <= max_bytes
final_bytes    <= max_bytes
staging_entries = begin_entries + final_entries   # checked add; overflow = CORRUPT
staging_bytes   = begin_bytes   + final_bytes     # checked add; overflow = CORRUPT
intermediate_* <= staging_*
```

17章の全named operation catalog、最大256-memberのcleanup batch、最大160-memberのdestroy recovery groupを監査対象とする。Member countはStorage live entry数ではなくCREATE/REPLACE/ERASE witness countであり、上記normal formとunion boundはmember ceilingに依存せず全planへ適用する。Coreはplan開始前にbegin/final countとlogical bytesをchecked計算し、各viewが`max_*`を超えるcall列を生成しない。超えるsemantic workはlegalなatomicity boundaryで分割できる場合だけ分割し、分割不能ならStorageへ部分発行せずCore contract failureとする。staging合計のchecked加算overflowはmutation 0の`CORRUPT`でfail closedする。`2 * max_*`とのu64比較は不要である。

M1a providerのminimum staging capabilityは、begin viewとfinal viewのunionに相当するentry/byteを保持できることです（例: committed 32件からdistinct 32件を先にputした64件view）。別のpublic capability field/queryはなく、FULL durabilityと同じplatform preconditionです。capacity maximaに`UINT64_MAX / 2`上限は課しません。Providerはbound内の中間viewをstaging ceilingだけを理由に拒否せず、final-net viewがcapacity内ならput-before-erase順もcommitできます。各viewが`max_*`を1でも超えるdirect Storage callはM1a Core precondition外で、providerはmutation 0の`NO_SPACE`を返せます。
...
| `SH6_STAGING_BOUNDARY` | capacity 32 entries / 69,632 bytes。満杯begin viewからdistinctな満杯final viewをput-before-eraseで作る | 64 entriesと139,264 logical bytesちょうどまで全put OK、old erase後final-net commit OK。Core plan oracleはexact boundをacceptし、65 entries / 139,265 bytesをrejectしてover-bound callを0回にする |
...
Capacity maximaは有限かつnon-zeroで、`UINT64_MAX`を拒否します。Entry accountingとstaging合計の加減算はcheckedです。Fixture作成時のinvalid maximaはfixture作成失敗であり、Storage Port callとしてstatusへ射影しません。
...
- `max_entries`/`max_bytes`をunknown、0、`UINT64_MAX`にしてM1aをadmitしてはなりません。物理freeが取得不能なことは許可しますが、configured logical maximaは有限でなければなりません。staging合計はchecked加算で合成し、overflowはCORRUPTでfail closedします。
...
| Vector | Send-gate sequence | Required result |
| --- | --- | --- |
...
- hook nameは12章§17のNamed fault hook registry 134件を正確に使用します。任意文字列をsilent ignoreしません。Registry追加/削除時は下記mirrorとの差分をCI failureにします。
...
次の134行は12章§17と順序・文字列ともexact一致するmachine-compared mirrorです。Comment、alias、prefix wildcardはregistry要素ではありません。
...
CIは両章のcode blockからcomment/blankを除いた配列を抽出し、count=134、unique=134、ordered set equalityを検査します。文字列集合だけでなく順序も差分にします。
...
134 hookすべてについて「hook直前crash」と、after系では「hook直後crash」をsystematicに実行します。Pairのないput/send/callback/reopen/scan hookもmirror各行を独立scenarioとして到達させ、到達不能/余剰hookをrelease gate failureにします。
...
Foundation fixtureのlimitは12章`NINLIL-FOUNDATION-SMALL-1`から次のchecked derivationで固定します。Accepted configごとに`N = max_nonterminal_transactions + max_retained_terminal_transactions`を求め、kind 1..11を表のformulaどおり導出します。Result cacheの加算、target/evidenceの全加算・乗算（`max_evidence_per_target + 1`を`uint64_t`へpromoteしてから演算）をcheckedとし、1件でもoverflowしたらprofile upper判定より先に`NINLIL_E_INVALID_ARGUMENT`、partial limit publish 0です。
...
Vector `S1_RECEIPT_BEFORE_MANAGEMENT`はt=2,000に次のordinal順を明示します。
...
| `NIN-FND-STO-003` key/prefix/capacity | binary keys、stable order、MB1〜MB8 mutable buffer/iterator pair atomicity、SH1〜SH6 handle/capacity/staging exact boundary |
...
| `NIN-FND-FLT-001` named hook registry | 12章§17との134-name ordered mirror一致、全hook crash、specific transition pair matrix |
...
- 134-name ordered hook mirror一致、HC1〜HC15と全named crash boundary
...
- M6: reference applicationのDisplay/Leak testはgeneric conformanceに追加して実行し、置換しない。
