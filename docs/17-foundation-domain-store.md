...
**D1-B3k境界**: B3kはEVENT_SPOOL (`0x50`) pure body encode/decodeと本same-record閉包だけの **implementation complete** です（vector format `ninlil-domain-store-v1-d1b3k`）。D2-S3 scan到達は B3l–B3o implementation pending の間 block（§8.6.1）。Live TRANSACTION_ANCHOR PVD、grant re-verify、availability resume path、STATE/RETRY/MANAGEMENT cardinality、payload BLOB、retention/cleanupはD3以降です。
...
**Decision identifier: D1-B3k..B3o。** 本節は5 remaining Domain Store bodiesのsame-record Normative contract freezeを固定する。**D1-B3k implementation complete**（pure body + same-record + vector `d1b3k`）。**D1-B3l..B3o は docs-only Normative freeze / implementation pending**。**D1 complete / D2-S3 complete / Stage 5 complete をclaimしない**。public C ABI / export symbol / public status を追加しない。

| Slice | Subtype | Body same-record（本freeze） | Status | Vector format | Implementation / D2-S3 |
| --- | ---: | --- | --- | --- | --- |
| **D1-B3k** | `0x50` EVENT_SPOOL | exact 300、ID128=tx、revision=spool_revision、state×cause、resume 0..8、discard iff DISCARDED、reservation KEY_DIGEST | **implementation complete** | `ninlil-domain-store-v1-d1b3k` | implemented; D2-S3 still blocked by B3l–B3o |
...
| **D2-S3** | 全current domain structural / same-record validation（envelope、4096、subtype body local、duplicate/order接続）。**step 5 same-record/local: witness header+chunk framing/matrix のscan到達**（D1 pure witness codecに依存。member old/new・partial group・successor chainはD3）。**D1-B3k (`0x50`) implemented; D1-B3l..B3o（`0x51`/`0x52`/`0x61`/`0x63`）は§8.6 docs-only Normative freeze済みだが implementation pendingの間、S3 completionはblock**（§8.6.1） | 否 |
...
| D2-S3 | current domain structural/same-recordをscan pathから到達させるvectors。**witness header+chunk same-record framing/matrixのscan到達**（D1 witness pure codec依存。member old/new・chainはD3）依存D1 body hexは当該subtype D1 deliverableが正本。format `ninlil-domain-store-v1-d1b3k` is current authority; future `d1b3l`..`d1b3o`は§8.6.1で予約（artifact未存在） | **D1-B3l..B3o docs-frozen / implementation pendingの間 S3 completion block**（§8.6.1; B3k implemented） |
