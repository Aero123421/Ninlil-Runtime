...
4. `completion_expires_at_ms` **non-zero**
5. `token_expires_at_ms` **non-zero** かつ **`completion_expires_at_ms` と exact 一致**

`completion_expires_at_ms = checked(delivery_started_at_ms + application_completion_timeout_ms)` かつ descriptor 上 `application_completion_timeout_ms ≥ 1` のため、`delivery_started_at_ms=0` でも expiry は non-zero。checked addition overflow / clock uncertain / non-zero epoch 不可では **token 発行前に fail**（callback 0、ACTIVE shape を作らない）。
...
| unknown reconcile action / invalid KNOWN_RESULT | tombstone identity/timers/`N` **維持** | **CALLBACK_CONTRACT へ置換**（health prio3 成立） | kind **11 phase 2/3**。valid terminal reconcile で source clear |
...
| 6 RECOVERY_REQUIRED | A_APP | (a) DELIVERY_START `N+1`不能: **`N=MAX` + retained tombstone** + E_REC(`COUNTER_EXHAUSTED`)、(b) その他: `N≥1` + tombstone∈{EXPIRED,RECOVERY_REQUIRED_TOMBSTONE} + E_REC reason ∈ {APPLICATION_FAILED, CALLBACK_CONTRACT, APPLICATION_COMPLETION_TIMEOUT, **OUTCOME_UNKNOWN**, COUNTER_EXHAUSTED}（上表割当） | **D_OPEN** | E_REC | F_NONEまたはF_LATE | (0,0) |
