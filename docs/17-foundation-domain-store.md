...
`ended_clock_epoch / ended_at_ms`はcycleをcloseしたlogical inputの時刻です。特に8th accepted-send Receipt timeoutでは、processing sample `W`ではなくchecked timeout deadline `V = send_observed_at_ms + attempt_receipt_timeout_ms` exactを保存します。checked加算不能はwriter/management reducerでfail closedし、wrapした時刻を保存しません。
...
| `audit_clock_epoch` / `audit_committed_at_ms` | new canonicalはepoch **non-zero**（time **0可**; public resume resultには非露出）。既存V1互換はlegacy `zero/zero`のdecodeとledger-first replay/conflictに限り受理し、generic/new encodeは拒否する。`zero/non-zero` mixedは禁止 | epoch **non-zero**（`audit_committed_at_ms`は **0可**; 12章presenceはepoch） |
...
MANAGEMENT_LEDGERはEvent admission時に物理slotを作りません。成功したdistinct resume/discardのFULL mutationだけが`transaction_id || operation_id` composite keyのledgerを1件createし、Event spool/count、reservation vector、state/resultを同じwitness groupで更新します。新規writerは両kindともaccepted management sampleのnon-zero epochと同epoch time（0可）を既存audit tupleへpersistし、retention中は再起動後もowner chronology baselineとします。Public resume resultはこのinternal audit tupleを露出しません。Upgrade前に作成済みのkind 15 legacy `zero/zero`はreplay/conflict互換のためdecodeとledger-first replay/conflictに限り受理し、generic/new encoderは拒否します。chronologyを再構成したとはみなさず、ledger-miss managementは12章どおりfail closedします。Event spoolはresume最大8、discard最大1をguardし、RESERVATIONは未使用分をlogical `reserved`、作成済みledger分をlogical `used`としてexact 256/512 bytesずつ表します。Ledger hitはcanonical request digestとoperation kindを先に比較し、一致ならpersist済みreplay fieldをcurrent Event state/revisionを再評価せず返します。不一致はpublic conflict resultです。live counter/state/spool整合は **D3** です。
...
- `BEARER_STATE`: `availability_epoch:u64 + available:u32 + observation_clock_epoch[16] + observed_at_ms:u64`。Absent before first observation、以後non-zero strictly increasing epochだけoperation kind 20 witnessでcreate/replaceします。Same/old epochはwrite 0、same epoch/different availableはcontract failureです。Automatic Event availability consumeは、consumeするpersist済みepochに対応するこのobservation tupleを12章のowner chronology baseline `A`として使い、cross-epochまたは`T < A`でEvent側のkind 14 mutationを行いません。
...
| `vector_count` | **94** |
| Full JSON sha256 | `f6c4e86a02a33093438e5828811b8f27dba30182b302e5e567990dbba8cac975` |
| Canonical `content_sha256` | `6e4c2cd29323c4ab53fe2d6452150478e50b48bea8f31a8a45bd8ed06f5980a8`（artifact embedded pin） |
...
| S3 | `spec/vectors/domain-scan-structural-v1.json` | `ninlil-domain-scan-structural-v1-d2s3` | 89 | `af8133f1b01c87b79b24a80479aa5d0f8480d124ead2cdb3c57277fc41d8d754` |
| S4 | `spec/vectors/domain-scan-exact-get-v1.json` | `ninlil-domain-scan-exact-get-v1-d2s4` | 30 | `c907142751b0920d0ef1ba889b1efb33098ac866780e69367e7af1f43b52642b` |
| S5 | `spec/vectors/domain-scan-composition-v1.json` | `ninlil-domain-scan-composition-v1-d2s5` | 22 | `b0b6c6c3cab5843cad12030a431ed1de8d6ccf4230e92774f6b0b8b3d15d5be4` |
| D1 | `spec/vectors/domain-store-v1.json` | `ninlil-domain-store-v1-d1b3o` | 1549 | `e7d2a7229b26fccd6f7cf821c779077fa449a03551d8ab7cda88998b6abe9502` |
...
| Crossrow authority | format `ninlil-domain-scan-crossrow-v1-d3s1` / `vector_count` **94** / full JSON sha256 `f6c4e86a02a33093438e5828811b8f27dba30182b302e5e567990dbba8cac975` / canonical `content_sha256` `6e4c2cd29323c4ab53fe2d6452150478e50b48bea8f31a8a45bd8ed06f5980a8` / independent generator + production bridge / exact 94 kinds / mutation 0 / frozen D1+S1–S5 pins |
...
| Crossrow authority | format `ninlil-domain-scan-crossrow-v1-d3s2` / `vector_count` **144** = prefix **94** + suffix **50** / full JSON sha256 `0429bc8174210fbb824940d0cfcfadd8ad3e8f1f74dee2b5ec507e9ea113c4d2` / canonical `content_sha256` `8958aff6c31dcd09f13eae2b6df44b5ae3f4e7bb25a879f369c7b7824de30bf4` / independent generator + production bridge / mutation **0** |
...
**Status:** **Accepted（2026-07-21; Sol xhigh r3 P0=0/P1=0）**（deterministic generator + candidate JSON self-check済み; production implementation + two-lane bridge 139 green追従済み。D3 complete / Stage 5 は別条で未claim）。  
...
**Withdrawn pre-acceptance draft:** any uncommitted / non-Accepted authority JSON (including historical full SHA `0658cbe092313e19dbb4498dcd5a786517651cb984ddfc7f97f2a47b372cf36f` and related 228/84 drafts) that required **BIND GET=0** while claiming full reverse-owner / untyped-orphan L1 for arbitrary rows is **non-constructible** under fixed **754** / no full-ID set and is **withdrawn as a pre-acceptance draft**. Do **not** treat that SHA as Normative or Accepted. The generator candidate now regenerates the **same authority path** `spec/vectors/domain-scan-crossrow-v1.json` under this WG text; its current **R28 Proposed** candidate is **283 = frozen D3-S2 prefix 144 (exact invariant) + D3-S3 suffix 139**, where suffix **139 = `rep1_l2` production 138 + `formal_precheck` 1**, content SHA `9d33b213d3b0cf24bc993717d11c88d5ff7485f11c59d50fed845ebb6c888e22`, full-file raw SHA `c8cecf86d430deeed3885e9da8d0b4b79d525f2d1183092d56f99314d47076b6` (**vector body changed vs R27-base**: +3 Mode27 EF state-dependent reason closure CORRUPT — R28 is oracle/spec gate repair: EventFact state-dependent reason closure in the oracle classifier + production Mode27 adoption guard (EF AWAITING={NONE(0)} — no deadline/cancel, so 68/83/86/128/129 rejected; CAPACITY_EXHAUSTED(11) parks not WAITING; DISPATCHING consumes ≥1 attempt); the D1 constructibility gate keeps EF rows constructible so the adoption guard is regression-testable; Mode27 EventFact lifecycle permanent 4×4 spool enumeration). Historical R25 SHAs `bb56954f…` / `997f64e5…`, R24 SHAs `12d1729e…` / `1b210e29…`, and R23 SHAs `bdaed45d…` / `3aeb1b02…` remain **superseded** as incomplete authority. Formal precheck ID `D3S3_M27_DUPLICATE_DIGEST_MATCH_CORRUPT` / `precheck_error=DUPLICATE_COMPLETE_KEY` is validator-only（no production API; no synthesized runtime status/session/checkpoint/result; zero Port because bridge does not call production）. Exact call/Port grammar is **`rep1_l2` only**. Independent generator-time D1-legality on every suffix row is **no production C as oracle** and **no D1 deferral**. **Honest chronology:** R21 claimed complete independent Normative D1 and was **Sol high NO-GO**. R22 repaired several R21 holes with typed differential pos74/neg60 and was **Sol high NO-GO**. **R23** repaired R22 residuals and was **Sol high NO-GO** again — **R23 did not complete D1** (TRANSACTION_STATE enum-only false-green; CELL/CANCEL/DELIVERY empty-body `struct.error`; CLI `check []` AttributeError). **R24** partially repaired R23 and was **Sol high NO-GO P1=2 P2=1**. **R25** repaired R24 residuals and was **Sol high NO-GO**. **R26** repaired R25 residuals but Root QA found residual STATE matrix NO-GO holes (FAILED+22 false-red; EXPIRED/UNKNOWN+20 false-red; WAITING accepting terminal-only reasons; SATISFIED+65 wrong outcome; CANCEL_AFTER_EFFECT(83) terminal; AWAITING missing 128/129; Mode27 spool cross-pair incomplete enumeration). **R27** closes those holes without weakening R16–R26 by re-extracting the full family×state×outcome×reason×deadline×evidence×dependent product from docs/12+docs/13（SATISFIED={64}; EXPIRED={65,66,20}; UNKNOWN={69,20} (129 AWAITING-only); CANCELLED={82}; FAILED={22,70,80,128}; AWAITING={0,68,83,86,128,129}; WAITING={11,85,128,130,132}; STATE legal positives **28**; Mode27 lifecycle 4×4=16 permanent cells）. COMMIT_UNKNOWN five fence paths + boundary **4380** + secondary differential typed 74/60, body_decode 306/0/306, body_roundtrip 132 retained. Production typed diagnostic remains cross-check only. **R27-Sol:** Mode27 D3-S3 exact cross-row (family/avd/pvd/rev/state×park); generate full-document adopt (suffix rows={} / vector_count=true → exit2); check closed nested known-integer schema (faults_expected_used bool/u32 overflow → exit2). **R28:** EventFact state-dependent reason closure (EF AWAITING={NONE(0)} — 68/83/86/128/129 rejected; EF WAITING rejects CAPACITY_EXHAUSTED(11); EF DISPATCHING attempt_in_cycle≥1) in the oracle classifier (o1a_mode27_cross_row_ok) + production Mode27 adoption guard; the D1 constructibility gate keeps EF rows constructible; generate fail-closed on suffix row={} / missing value_hex / checkpoint closed-schema bool/float/unknown (previous_key_length / last_carrier_key_len / event_start etc., exit2). **Production / bridge follow this oracle gate (two-lane bridge 139 vectors green: production 138 + formal_precheck 1).** Sol re-review remains pending; **Accepted / GO forbidden**. No historical giant JSON duplicate is required in-tree.
...
- claim D3-S3を超える範囲の完成（production bridge 139 green と R28 Accepted authority 自体は 2026-07-21 に成立済み）

##### 18.14.19.14 Follow-on work（non-normative）

| Work item | Owner |
| --- | --- |
| Generator regenerates candidate authority with BIND-WG Port events + control-tuple checkpoints + R19–R28 D1-legality gate（same path; R28 content `0155b692…` / raw `8b17304f…`; suffix139=`rep1_l2`138+`formal_precheck`1; boundary **4380**; STATE positives **28**; Mode27 lifecycle 4×4） | O1b — candidate emitted/self-check済み; acceptance pending |
...
| Sol independent re-review of R27 candidate (R21–R26 were Sol high / Root residual NO-GO); production/bridge evidence; ADR-0015 Accepted only after GO | governance（**satisfied 2026-07-21**: Sol xhigh r3 P0=0/P1=0 + bridge 139 green → ADR-0015 **Accepted**） |
