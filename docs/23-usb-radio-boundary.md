...
| Slice | 内容 | 依存 | 完了の意味にしないこと |
| --- | --- | --- | --- |
| **R1** | `ninlil_radio_hal` 契約 + host spy（**transmit-with-permit 唯一入口**）。**現状: host implementation candidate**（CTest `radio_hal_r1` + `radio_hal_r1_gate`）。seal 非渡し + working-only authority callbacks + consume last + ctx clock only。structural production-source TX token scan（**R9 call-graph/link sole-edge gate は未**）。**R1 complete / production radio / NIN-CMP-003 / R9 完成ではない** | §9、docs/05 | real SX1262; R2 authority+time; R9 call-graph; wire; Japan; legal; RF/HIL |
...
| R1–R10 | 各実装 PR | §10.2。R1: `radio_hal_r1` + `radio_hal_r1_gate`（host only）。R10 なしに production radio と言わない |
