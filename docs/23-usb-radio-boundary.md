...
状態: **Normative for U0 / physical radio boundary freeze** + **U1 implementation candidate / host tests** + **U2 A2 ESP CDC implementation candidate / host pure tests + target compile/link** + **U3 C3 control-session + C4 pump implementation candidate / host fake-stream tests** + **U3↔U4 production-private boundary candidate**（logical epoch claim / tracked TX raw-outstanding≤1 / claim 中 RX-only cold）+ **U4 NCL1 pure codec candidate / host vectors** + **U4 logical session host candidate**（`src/transport/logical_session.*`; actual U3+NCG1+NCL1 composition; §8.9 engine IDs 38 Required host gate）+ **R1 ninlil_radio_hal host candidate**（sole transmit-with-permit + spy; production default-deny）+ **R2 Physical Compliance Permit authority host candidate**（[24章](24-r2-physical-compliance-permit-authority.md) / [ADR-0004](adr/0004-r2-durable-permit-authority.md); private durable authority; **legal/RF/HIL未**）+ **R3 LoRa airtime host candidate**（[27章](27-r3-airtime-calculator.md) / [ADR-0007](adr/0007-r3-airtime-calculator.md); closed SX1262 LoRa ToA）+ **R4 SX1262 control-plane host candidate**（[28章](28-r4-sx1262-control-plane-backend.md) / [ADR-0008](adr/0008-r4-sx1262-control-plane-backend.md); reset/init/SPI + explicit TX deny; **RF TX/HIL/legal未**）+ **R5 LAB_ONLY profile loader host candidate**（[29章](29-r5-lab-only-profile-loader.md) / [ADR-0009](adr/0009-r5-lab-only-profile-loader.md); full permit bind; **FIELD/Japan/HIL未**）<br>
（**U1 complete ではない** — **Required HIL Linux+macOS pending**; **U2 complete ではない** — **Required HIL ESP flash + host CDC roundtrip pending**; **U3 は host framing/session candidate のみ**; **U4 は host logical session candidate であり USB series complete / HIL ではない**; **R1 は host candidate のみ**; **R2 は private host candidate — legal / production profile / RF / HIL 未**; **R3 は host candidate のみ — Japan production 数値 / R3 complete / HIL ではない**; **R4/R5 は host candidate のみ — R4 complete / R5 complete / RF / legal / HIL / production radio ではない**; **USB series 未完成**; **SX1262 physical RF TX path 未実装**; M3 incomplete / M5 incomplete）<br>
注: byte-pin済みのdocs/25–26に残る進捗文言はU5/U6 freeze成立時点のsnapshotであり、本章とdocs/READMEの現行R2状態を上書きしない。freeze本文の更新には新version・Accepted ADR・独立再レビューが必要である。<br>
...
| private session object と payload ownership 経路（U3/U4 直結） | する（U3: NCG1 session+pump 実装 candidate; U3↔U4: logical epoch / tracked TX / RX-only cold 境界 candidate; U4: NCL1 pure codec + logical session host candidate） | Normative private + **U3 implementation candidate** + **U3↔U4 boundary candidate** + **U4 pure codec + logical session host candidate**（series complete / HIL ではない） |
| 完全な assignment / Transport Custody | **U0 ではしない**（**U5/U6 は別章で Normative freeze**: [25章](25-u5-cell-operating-assignment.md) / [26章](26-u6-transport-custody.md)） | U5/U6 docs freeze; 実装は後続 |
| security session protocol | **しない** | 後続 freeze |
| Network Attachment / Join、relay、multi-parent topology | **しない** | 後続 freeze（§14） |
| secure compact radio wire の production bytes / version | **U0 時点ではしない**（当時 unallocated） | **R6 docs freeze で初割当**（[30章](30-r6-secure-radio-wire.md) / [ADR-0010](adr/0010-r6-secure-radio-wire.md); **R7 実装ではない**） |
...
- assignment lease 適用・custody spool・Application Receipt 完了（U5/U6 **docs freeze ≠ 実装完了**; [25章](25-u5-cell-operating-assignment.md)/[26章](26-u6-transport-custody.md)）
- control channel の AEAD / session authenticity 完了
- SX1262 HAL・MAC・LBT・airtime ledger 実装済み
- secure radio wire version の **U0 による** 割当（**R6** で `wire_profile_id=0x11` docs draft 割当; 本章 U0 完了 claim には含めない）
- **compile success = HIL PASS** / compile success must not equal HIL の否定
- application-specific schema / policy が portable Core に含まれること
...
| I1 | Reference application integration |

### 2.2 Compile / source dependency（include/link のみ）

```text
I1 --> C0 (public API only)

C0 --> C1 abstract only
C0 -X-> A1, A2, D1, I1 schema, termios, TinyUSB, SX1262

C3 --> C2
C3 -X-> A1, A2, I1

C4 --> C1, C2, C3, A1|A2, L1
C4 -X-> I1 as Core types; -X-> D1 for USB series

A1 --> C1 + POSIX termios/poll
A2 --> C1 + esp_tinyusb
A1/A2 -X-> C0 reducers, P1 impl, D1, I1
...
4. I1 は integration のみ。portable Core / private control 必須語彙にしない。
...
3. host 実装は `ports/posix/`（または同等 host port）に置き、application-specificなhost専用ディレクトリへ埋め込まない。
...
したがって U3/U4 は **明示的な private session object + payload ownership 経路**（§4.2–4.3）を実装必須とする。これは portable Core 公開 API でも application-specific語彙でもない。

### 4.2 所有境界

| 資源 | Owner | 非 Owner |
| --- | --- | --- |
| USB device / CDC interface / fd | A2 task（Cell）または A1 thread（Controller） | C0、application、I1 |
...
2. application-specific message enum / policy ID を C3 必須フィールドにしない。
...
**実装候補（host fake-stream 検証）:** `src/transport/control_session.*` が U4 logical session より前に必要な **production-private** 境界を提供する。**NCL1 pure codec** と **U4 logical session host**（`src/transport/logical_session.*`; sole-owner composition）は別途実装候補。本節の U3 境界 green は **U4 complete / USB series complete / HIL** を意味しない。
...
2. assignment epoch / controller_term の **logical messages** は **[25章 U5](25-u5-cell-operating-assignment.md)** が正本。U1–U4 は HELLO/liveness のみ（assignment type を v1 catalog へ追加しない）。
...
| **Negotiated control protocol version** | HELLO `min/max/selected_control_version`（u16） | U4 は **1 のみ**（semantic catalog: HELLO/PING/PONG/RESET） | **v2** は assignment/custody catalog（[25章](25-u5-cell-operating-assignment.md) / [26章](26-u6-transport-custody.md)）。**NCL1 logical_version とは別 domain**（[06章](06-versioning-and-compatibility.md)） |
| Secure radio wire | **R6** `wire_profile_id=0x11`（**no major/minor domain**） | **docs freeze Accepted**（[30章](30-r6-secure-radio-wire.md) / [ADR-0010](adr/0010-r6-secure-radio-wire.md) **Accepted**; one-way contexts; re-GO 2026-07-19 P0=P1=P2=0; R7 full AEAD / HIL 未完） | R7 |
...
| **controller_term**（U5; [25章](25-u5-cell-operating-assignment.md)） | active Controller writer term。発行・consume で bind。**assignment replacement 時の stale permit fence に必須** |
| **assignment_digest**（U5; [25章](25-u5-cell-operating-assignment.md)） | CellOperatingAssignment canonical digest（32B）。同一 identity/revision/epoch でも digest/term 変異時は **新 permit のみ** |
| **permit_bind_generation**（U5 ARW; [25章 §8.5](25-u5-cell-operating-assignment.md)） | apply ごとの単調 generation。fence 後は **旧 generation の outstanding permit を consume 不可** |
| physical transmitter identity | どの physical radio path / 送信機か（Foundation bearer ではない; [24章](24-r2-physical-compliance-permit-authority.md) §1.5） |
| channel | 周波数 channel 識別（live radio settings / Live bind set L） |
| PHY parameters | bandwidth / SF / CR / 等 profile 範囲内（L に含む） |
| frame digest | 送信 bytes の digest（**R6:** algorithm id `1` = SHA-256(exact outer frame); [30章](30-r6-secure-radio-wire.md)） |
| frame byte length | exact |
| conservatively calculated maximum airtime | ledger 予約と一致 |
| not-before / expiry | R2 authority clock の `{epoch_id, now_ms}` に束縛した時間窓。R6 `0x11` は active profile の `profile_clock_epoch_id` と Permit 発行時 `clock_epoch_id` の一致を必須とし、host wall clock / Unix time を使わない（[30章 §15.3.1.1](30-r6-secure-radio-wire.md)） |
| permit sequence | 単調・再利用禁止 |

**発行後変異:**

- SiteAssignment identity/revision/epoch、**controller_term**、**assignment_digest**、Hardware/Regulatory profile revision、live radio settings、または frame digest/length/PHY が **発行時スナップショットと異なる** → **consume 拒否**（TX しない）。新 plan + 新 permit が必要。
- CellOperatingAssignment の apply/replacement（同一 identity/revision/epoch でも term/digest 等の mutation を含む）では、[25章 §4.1 / §10](25-u5-cell-operating-assignment.md) どおり **outstanding permit を atomic fence** してから新 active を公開する。fence 省略で旧 permit が consume 可能になる gap は **禁止**。
- active TX reservation がある profile/assignment の in-place 変更禁止（[05章](05-security-and-compliance.md) Profile変更）。
- R2 Normative 正本 `docs/24` / ADR-0004 と private runtime authority host candidate は統合済み。本表の term/digest/`permit_bind_generation` bind と U5 fence は **docs 整合済み**。R2 **production profile / HIL / legal / RF complete は未**。
...
| 項目 | 状態 |
| --- | --- |
| secure compact radio wire identity | **R6 docs freeze Accepted** `wire_profile_id=0x11` only（**no major/minor**; NRW1/AES128-GCM; one-way contexts; E2E security id≠Attachment; [30章](30-r6-secure-radio-wire.md) / [ADR-0010](adr/0010-r6-secure-radio-wire.md) **Accepted**; re-GO 2026-07-19 P0=P1=P2=0）。**R7 full AEAD 実装・RF·USB 実機 HIL ではない** |
| frame byte layout / AEAD coverage | **R6 Normative draft**（outer AAD 19B + E2E AAD 14B + dual 16B tags; profile `0x11`; DATA/ACK lanes; durable lifecycle; CELL_64_V1; [30章](30-r6-secure-radio-wire.md)）。実装は R7 |
...
| **U4** | NCL1 + HELLO/PING/PONG/RESET、session_generation/cookie fencing、§8.9 vectors **Required gate**。**現状: logical session host candidate**（`src/transport/logical_session.*` sole-owner of U3 epoch/token/pump/take_rx/tracked TX; init/bind/step/snapshot/unbind; engine behavior IDs **38** + pure-codec **8** = required **46**; sibling `logical-session-u4-v1.json` + independent generator + host CTest/gates）。**U4 complete / USB series complete / Required HIL ではない** | U3 + §4.3.1 boundary | assignment、security、public ABI、HIL、series 完成 |
| **U5** | CellOperatingAssignment / control protocol v2 assignment catalog。**正本: [25章](25-u5-cell-operating-assignment.md)** + [ADR-0005](adr/0005-u5-cell-operating-assignment-control-v2.md)（**docs freeze; 実装未**） | U4、docs/22 cell skeleton | full topology、Site Membership/Attachment 完了、U5 implementation complete |
| **U6** | Transport Custody（OFFER/ACCEPT/REJECT/BUSY、dual FULL）。**正本: [26章](26-u6-transport-custody.md)** + [ADR-0006](adr/0006-u6-transport-custody.md)（**docs freeze; 実装未; ESP success は HIL 後**） | U4/U5（control_version v2）、storage port | Application Receipt、U6 implementation complete、ESP FULL without HIL |
...
| U4 | §8.9 vectors + state machine table **Required**（pure codec bridge + **logical session engine bridge** 38 IDs / 10 scenario families; TIME_REGRESSED/DEADLINE_OVERFLOW exact zero-mutation; cookie CSPRNG≠jitter） | session loss/reorder | NCL1 fuzz smoke | optional; **現状 host candidate — series complete ではない** |
| U5–U6 | [25章](25-u5-cell-operating-assignment.md) §13–14 / [26章](26-u6-transport-custody.md) §14–15 | host pure + crash inject（U6） | mutation | optional USB HIL; ESP custody success は 21章 HIL 後のみ |
...
| Slice | 内容 | 依存 | 完了の意味にしないこと |
| --- | --- | --- | --- |
| **R1** | `ninlil_radio_hal` 契約 + host spy（**transmit-with-permit 唯一入口**）。**現状: host implementation candidate**（CTest `radio_hal_r1` + `radio_hal_r1_gate`）。seal 非渡し + working-only authority callbacks + consume last + ctx clock only。structural production-source TX token scan（**R9 call-graph/link sole-edge gate は未**）。**R1 complete / production radio / NIN-CMP-003 / R9 完成ではない** | §9、docs/05 | real SX1262; R2 authority implementation; R9 call-graph; wire; Japan; legal; RF/HIL |
| **R2** | Physical Compliance Permit **authority**（FIFO / advance_expired_heads / ceiling·per-permit airtime / fresh-epoch CLOCK_FENCE / ops→user / publish exact）。**現状: private host candidate**（[24章](24-r2-physical-compliance-permit-authority.md) **§14**、[ADR-0004](adr/0004-r2-durable-permit-authority.md)、`pcp_r2_docs_gate` semantic、CTest `pcp_r2_authority`）。**legal / production profile / RF / HIL completeではない** | R1、§9、docs/05、docs/24 | legal; R3/R5 production bind; RF/HIL; release re-review |
| **R3** | Airtime calculator（closed SX1262 LoRa ToA; integer ceil-us）。**現状: host candidate**（[27章](27-r3-airtime-calculator.md)、[ADR-0007](adr/0007-r3-airtime-calculator.md)、`airtime_r3_bridge` / `airtime_r3_gate` / private `airtime_calculator.c`）。profile 値は R5 | R2（受渡し契約） | Japan production 数値; duty/LBT/legal; R3 complete; RF/HIL |
| **R4** | SX1262 backend **reset/init/SPI** only、TX path は explicit deny（SetTx 構造的不在）。**現状: host control-plane candidate**（[28章](28-r4-sx1262-control-plane-backend.md) / [ADR-0008](adr/0008-r4-sx1262-control-plane-backend.md); CTest `sx1262_r4` + `sx1262_r4_gate`）。**R4 complete / RF TX / HIL / legal ではない** | R1、drivers layout | RF TX; R9; production radio |
| **R5** | LAB_ONLY profile loader + permit bind **全項目** mismatch test（SiteAssignment 含む）。**現状: host candidate**（[29章](29-r5-lab-only-profile-loader.md) / [ADR-0009](adr/0009-r5-lab-only-profile-loader.md); `profile_loader.{h,c}` + `profile_r5` / `profile_r5_gate`）。**R5 complete / FIELD / Japan / HIL ではない** | R2–R3 | FIELD/PRODUCTION |
| **R6** | Secure radio wire **Normative freeze Accepted**（`wire_profile_id=0x11` 初割当; no minor）。**現状: docs freeze Accepted**（[30章](30-r6-secure-radio-wire.md) / [ADR-0010](adr/0010-r6-secure-radio-wire.md); one-way contexts / DATA·ACK lanes / E2E security id / CELL_64_V1; `radio_wire_r6_docs_gate`; independent re-GO 2026-07-19 P0=P1=P2=0）。**R7 full AEAD 実装 / M4·M5 handshake 実装 / ESP N6 capacity / RF·USB 実機 HIL / legal / production ではない** | M5 security prep（handshake 実装は **未完了** — R6 docs freeze は post-attachment input contract のみ） | R7 実装・HIL |
...
4. application-specific語彙を portable Core へ入れない。
...
| R1–R10 | 各実装 PR | §10.2。R1: `radio_hal_r1` + `radio_hal_r1_gate`（host only）。R2: [24章 §14](24-r2-physical-compliance-permit-authority.md) + `pcp_r2_docs_gate` + host authority CTest。R3: [27章](27-r3-airtime-calculator.md) + `airtime_r3_*`。R5: [29章](29-r5-lab-only-profile-loader.md) + `profile_r5` / `profile_r5_gate`（LAB_ONLY host candidate; FIELD/Japan/HIL 非主張）。R10 なしに production radio と言わない |
...
4. 完全 assignment / topology epoch / controller_term logical messages → **U5 で freeze**（[25章](25-u5-cell-operating-assignment.md); 実装は後続）
5. Transport Custody spool と USB の正確な handoff state machine → **U6 で freeze**（[26章](26-u6-transport-custody.md); 実装は後続）
6. control channel AEAD / identity bind
7. secure radio wire **R7 実装**（R6 docs freeze 済み: [30章](30-r6-secure-radio-wire.md)）
...
- [x] physical TX 順序と Permit bindings（SiteAssignment 含む）; logical permit 非認可; wire version **R6 docs freeze で割当**（実装は R7; U0 時点は unallocated だった）
...
- [x] 後続 freeze（Attachment/Membership、relay、multi-parent 等）を未確定と明示。assignment/custody の control catalog は U5/U6 別章へ委譲
...
- [ ] SX1262 physical RF TX path（対象外・**未実装**）
- [ ] Site Membership / Attachment 本線 / relay / multi-parent（対象外・未確定）
- [x] U4 host vector/engine bridge（pure codec + logical session 38 engine IDs; host CTest/gates）（**U4 complete / U-series 完成 HIL は未実施; 完成主張しない**）
- [ ] U-series 完成 Required HIL 実測（**未実施**; 完成主張しない）
