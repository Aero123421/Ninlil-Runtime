...
状態: **Normative for U0 / physical radio boundary freeze only**（**USB production code 未実装 / SX1262 production code 未実装 / M3 incomplete / M5 incomplete**）<br>
...
| private session object と payload ownership 経路（U3/U4 直結） | する（設計） | Normative private |
...
| USB / SX1262 production 実装 | **しない**（本 slice は docs only） | 未実装 |

次を **主張しない**（forbidden claims）:

- M3 complete / M4 complete / M5 complete / V1 / field-ready / production radio
- USB CDC driver・CDC task・POSIX serial adapter 実装済み
...
| 実装 path | **`esp_tinyusb`**（ESP-IDF 公式 managed component **path 選定のみ**）を U-series が使う。**U0 が pin するのは ESP-IDF `ports/esp-idf/ESP_IDF_VERSION` = exact **v5.5.3** のみ**。`esp_tinyusb` managed component の **exact version 番号 / idf_component.yml lock / 依存 pin は U0 では固定しない** — **U2 実装 PR で exact version と lock を固定**（U0 docs は path 選定と「version/lock は U2」を述べるだけ。ADR の “pinned esp_tinyusb path” も **path 選定**の意味であり exact version pin ではない） |
...
| 項目 | 規範 |
| --- | --- |
| open flags | `O_RDWR | O_NOCTTY | O_NONBLOCK`（または同等）。controlling tty にしない |
| exclusive open | Linux は可能なら `flock` または `TIOCEXCL` 相当で **同一 path の二重 open を拒否**。失敗は structured `BUSY`。macOS も非共有 open を目指す |
| termios raw | 入力/出力ともに raw: `ICANON`/`ECHO`/`ISIG` off、software flow 既定 off、8N1。固定 baud は CDC-ACM では host 側の論理値（実 CDC は USB bulk; 互換のため 115200 8N1 raw を default profile とする） |
...
| fd lifecycle | open → configure termios → poll loop → **close は owner thread のみ**。fork 後の fd 共有禁止。close 後の use-after-close は adapter が generation で fence |
| unplug errno | `EIO` / `ENODEV` / `ENXIO` / 0-length read after POLLHUP を **link down** に写像。session を INVALID（§5） |
| reconnect | 同一 path の再 open は新規 link。**自動 HELLO 再交渉は session 層**（U4）。adapter は link up イベントのみ |
...
| **U1** | POSIX explicit-path termios/poll、§3.2 UX、bounded rings、WOULD_BLOCK、default 容量の測定フック | U0 | NCG1、HELLO、**series 完成** |
| **U2** | ESP32-S3 `esp_tinyusb` CDC-ACM adapter、ISR→raw ring、task drain、compile smoke | U0、ESP-IDF pin | device HIL なしの完成主張 |
| **U3** | NCG1 を C3 session + C4 pump 経由で U1/U2 に接続、resync stats、payload ownership | U1+U2 または fake stream、docs/19 | HELLO、custody |
| **U4** | NCL1 + HELLO/PING/PONG/RESET、session_generation/cookie fencing、§8.9 vectors **Required gate** | U3 | assignment、security、public ABI |
...
| U2 | packaging + esp32s3 **compile/link** + **`esp_tinyusb` managed component exact version/lock を PR で固定** | n/a | n/a | **Required HIL（U2 完了主張時）:** ESP32-S3 実機 flash + host からの **CDC I/O 往復**。**compile ≠ HIL**。ESP-IDF は U0 pin の **v5.5.3** |
| U3 | framing golden + stream chunk inject + ownership handoff tests | byte fault inject | NCG1 fuzz 継続 | optional device framing; series 完成には U7 と合わせて評価 |
...
- [x] USB series Required HIL: U1 実 USB **Linux および macOS**; U7 smoke **Linux および macOS**; soak ≥30 min continuous 宣言; ESP-IDF **v5.5.3** pin / `esp_tinyusb` は **path 選定のみ**・exact version/lock は **U2**
