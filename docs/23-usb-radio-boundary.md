...
| R1–R10 | 各実装 PR | §10.2。R1: `radio_hal_r1` + `radio_hal_r1_gate`（host only）。R2: [24章 §14](24-r2-physical-compliance-permit-authority.md) + `pcp_r2_docs_gate`（semantic docs）; host pure 実装 PR。R3: [27章](27-r3-airtime-calculator.md) + `airtime_r3_*`（host candidate; Japan/HIL 非主張）。R10 なしに production radio と言わない |
