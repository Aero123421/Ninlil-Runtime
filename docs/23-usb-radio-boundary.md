...
- R2 Normative 正本は `docs/24` / ADR-0004（main 統合済み）。U5 は origin/main へ rebase 済みで、本表の term/digest/`permit_bind_generation` bind と U5 fence は **docs 整合済み**。R2 **runtime authority implementation / HIL / legal / RF complete は未**（docs 契約の一致のみ）。
...
| **R2** | Physical Compliance Permit **authority**（FIFO / advance_expired_heads / ceiling·per-permit airtime / fresh-epoch CLOCK_FENCE / ops→user / publish exact）。**現状: Normative freeze**（[24章](24-r2-physical-compliance-permit-authority.md) **§14**、[ADR-0004](adr/0004-r2-durable-permit-authority.md)、`pcp_r2_docs_gate` semantic）。**implementation pending — R2 complete / re-review GO ではない** | R1、§9、docs/05、docs/24 | R2 code; legal; R3/R5; RF/HIL; re-review PASS |
