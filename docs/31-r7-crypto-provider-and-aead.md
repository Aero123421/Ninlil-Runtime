...
状態: **Proposed / R7 T0A specification candidate**  
正本候補 ADR: [ADR-0011](adr/0011-r7-crypto-provider-boundary.md)（Proposed）  
前提: [30章](30-r6-secure-radio-wire.md) の R6 wire freeze は不変

本章は R7/W1 が NRW1 の AES-128-GCM、HKDF-SHA-256、SHA-256 を利用するための
**production-private 境界**を定義する。public ABI ではない。T0A は仕様候補であり、
...
## 12. T0A completion boundary

T0A は本仕様、Proposed ADR、ADR index の整合まで。production code、test、CI をまだ受入れ
ない。T0 implementation candidate は本章 §§1–11 を全て満たし、独立再監査後にのみ
ADR-0011 を Accepted へ変更できる。ADR-0011 は実装・全 gate・独立監査完了まで
**Proposed** のまま。
