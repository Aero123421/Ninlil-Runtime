...
**D3-S0 + D3-S1a + D3-S2a + D3-S3a + D3-S4a docs freeze complete / D3-S1 implementation complete / D3-S2/S3/S4 implementation + D3-S5..S12 pending**（Stage 5 / public Runtime still pending）。**durable storage PR #80 merged（HIL pending）**。**U0 boundary docs freeze complete / U1 host candidate / U2 ESP CDC candidate / U3 host framing-session candidate / U4 logical session host candidate / R1 radio_hal host candidate / R2 authority host candidate / U5·U6 docs freeze（実装 pending） / R3 airtime host candidate / R4 SX1262 control-plane host candidate / R5 LAB_ONLY profile host candidate / R6 NRW1 secure radio wire docs freeze Accepted（Stage 9; independent re-GO 2026-07-19 P0=P1=P2=0; R7 full AEAD / M4·M5 / ESP N6 capacity / RF·USB 実機 HIL / legal / production 未完） / R7 T0 crypto + T1 SINGLE codec candidates Accepted / T1b docs-only Proposed（R7 full / T1b implementation / T2+ pending） / U7・R8–R10 pending**。対象release: Foundation / pre-alpha
...
**R7 T1b context binding / verified HKDF scheduleはdocs-only Proposed**（[33章](33-r7-t1b-context-binding-hkdf.md) / [ADR-0013](adr/0013-r7-t1b-context-binding-hkdf.md) Proposed）。実装・vector・CI・独立受入は未実施であり、T1bまたはR7完成を主張しない。
...
| 33 | [R7 T1b context binding and HKDF](33-r7-t1b-context-binding-hkdf.md) | byte-exact binding + verified typed key schedule（[ADR-0013](adr/0013-r7-t1b-context-binding-hkdf.md) Proposed; **docs-only、実装/受入/R7 full未完**） |
