...
4. Application-specific message / policy / domain vocabularyをportable Coreへ持ち込む。
...
| I1 | **Reference application integration** | `integrations/reference_app/` | reference vertical のみ |
...
C0 (Portable Core)
  ...
  -X-> A1, A2, D1, I1 schema, termios, TinyUSB, SX1262, RadioLib, SQLite
...
C3 (NCL1 codec/session)  [portable private]
  --> C2
  -X-> A1, A2, C0 public headers as required surface, I1
...
C4 (composition/pump)    [private glue]
  ...
  -X-> I1 vocabulary as required Core types
  -X-> P1/W1/H1/D1 as USB series dependency

A1 (POSIX adapter)
  --> C1 (implements contract)
  --> POSIX termios/poll
  -X-> C0 business reducers, P1, W1, H1, D1, I1

A2 (ESP USB CDC adapter)
  --> C1 (implements contract)
  --> pinned ESP-IDF / esp_tinyusb
  -X-> C0 business reducers, P1 as USB path, W1, H1, D1, I1
...
P1 (Physical Compliance Gate)
  ...
  -X-> A1, A2, I1, C0 application payload interpretation
...
| --- | --- | --- |
| C0 Portable Core | C11、自身の private model、port **vtable 抽象**（C1 の abstract ops を含む） | A1/A2、termios、TinyUSB、ESP-IDF、D1、RadioLib、I1 schema、USB path 文字列、P1 具象 |
| C1 byte-stream contract | C11 のみ（関数ポインタ表） | 具象 adapter、driver |
| C2 NCG1 codec | C11 / portable only | platform USB/radio |
| C3 NCL1 codec/session | C2 | A1/A2、C0 public 必須化、I1、radio |
| C4 composition/pump | C1–C3、A1 または A2、L1 private | I1 必須語彙、public `include/ninlil` への型露出、D1 |
| A1 POSIX adapter | C1、POSIX | C0 reducer 改変、P1 実装、SX1262、I1 |
| A2 ESP CDC adapter | C1、ESP-IDF / esp_tinyusb | C0 への USB 型露出、I1、physical RF TX |
| L1 Cell local composition layer | Core port 抽象、C4 出力、P1 | A1/A2 内部 ring 直操作 |
| P1 Compliance Gate | HardwareProfile、RegulatoryProfile、SiteAssignment、live settings、ledger | application 業務解釈、Foundation virtual permit 流用、USB adapter、I1 |
| W1 secure wire/MAC | security session（後続） | portable Core raw wire struct 露出、logical `memcpy` |
| H1 radio HAL | P1 permit、D1 | C0、C3、USB、I1 |
| D1 SX1262 | radio HAL 契約、SPI/GPIO | C0、control protocol、I1 |
| I1 Reference application integration | C0 **public** API、integration-local adapters | portable Core / C3 必須語彙への逆流 |
...
1. **MUST**: portable Coreはdirectory名に関係なく、application-specific schema/policy、ESP-IDF、TinyUSB、termios、SX1262、RadioLib、SQLiteを直接参照しない（[01章](../01-architecture.md) Dependency rule の具体化）。
...
9. **MUST**: Reference application固有のvocabularyは`integrations/reference_app/`に閉じ、portable Coreとprivate control protocolの必須語彙にしない。
...
- Reference application vertical（M6）はpublic Ninlil APIとexperimental adapterのみを使い、Core依存を逆流させない。
