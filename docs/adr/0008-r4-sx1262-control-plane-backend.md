...
7. **Primary sources:** DS.SX1261-2.W.APP **Rev 2.2 Dec 2024** + sx126x_driver **v2.3.2** + SWSD003 **v2.3.0** を docs/28 に pin。NRESET は R4 で **exact `reset_pulse_us=1000`（SWSD003 1ms）** — DS「typically 100µs」は **保証 minimum としない**。BUSY TSW 600ns→post guard、TCXO delay steps×15.625µs ≠ BUSY ms、GetErrors before Clear、最終 **STBY_RC only**、control cmd_status **`{0,1,2}`**（実チップcmd=1を受理）、SPI で SX1261/2 SKU 識別禁止（STATUS_INVALID）。HIL/実測未は非主張。
