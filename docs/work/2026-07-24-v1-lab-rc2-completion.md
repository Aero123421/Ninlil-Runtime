...
- provider restart test を失効ポインタのアドレス比較から、provider 状態再生成の意味的検証へ変更した。
- loopback bearer の nonblocking partial read 状態を保持し、header/body の途中で EAGAIN が発生しても frame 境界を維持するよう修正した。
- Linux の peer 切断競合を SIGPIPE によるプロセス終了ではなく、通常の fail-closed transport error として扱うよう修正した。
- 2-process LAB E2E に scheduler yield と receipt 送信完了条件を追加し、macOS 高負荷時の競合を解消した。
- C4 LAB USB の caller-owned object alignment を実装型と一致させ、Clang の stack layout に依存する初期化失敗を解消した。
- C5 radio / C4-C5 E2E test fixture の storage・clock・entropy を明示解放し、LeakSanitizer gate を通過可能にした。
...
| SQLite3-disabled portable build | PASS — 243 / 243 |
| Linux GCC 13 strict example/direct-1hop compile | PASS |
| Linux Clang / ASan C4-C5 regression | PASS — alignment・fixture leak |
| macOS failure-path regression | PASS — Display / provider / direct 1-hop 各50回 |
| fragmented loopback receive regression | PASS — 100回 |
| `git diff --check` | PASS |
