...
| OR-07 | MFDT/R7 ESP DRAM gateが実mapを検査しない | IN_PROGRESS | official launcherの実mapへgate接続済み。fresh amd64 feature-ON CI待ち |
| OR-08 | Node.jsが未宣言・未固定・SBOM不在 | CLOSED | Node >=18、CI 22.18.0 pin、inventory/notice、SPDX `APPLICATION` + `BUILD_TOOL_OF` |
| OR-09 | copyright holder/SPDX/DCO経路が不在 | EXTERNAL | review基準799本＋現行追加13本のfirst-party C/H 812/812 SPDX、実集合gate、source-license inventory、DCO 1.1 gate/workflowはlocal CLOSED。正式holder/yearとNOTICE、remote DCO required化だけはowner/legal/admin証拠待ち。Apache LICENSE本文は変更しない |
...
| OR-21 | MFDT/R7/RRMPのprocess-global mutable state | CLOSED | RRMP durable scratchとlegacy bind pointer、MFDT production全体、R7 issue coordinator/checked-issue stateをcaller ownerへ移行。MFDT normal/sanitizer各31/31とoversize slot/fini各2/2、R7各17/17、RRMP all-profile/sanitizer/ESP evidenceはPASS。mutable-static/source/archive負例を含む独立review GO |
| OR-22 | `ninlil_runtime_step`の`in_step`を7箇所で手動clear | CLOSED | 単一epilogueへ集約。error後の即時step回復を既存delivery testで確認、focused 3/3 PASS |
| OR-23 | top-level CMakeの83%がtest登録 | CLOSED | tests-only 6,483行を専用authorityへ機械分離。分離前後のdefault 442/all-private 526既存rosterはsemantic exact。現行統合snapshotはdefault 448/all-private 535（disabled 21）、tests-OFF 0。分離後の意図的増分はlayering 2、fuzzer tests-OFF負例1、R7 owner 1、RRMP serial-owner 1、MFDT owner-state 2件。追加disabled 5件はDomain public Runtime非ready時のComposition 3件とMFDT Runtime-owner 2件をcompile/link-onlyへ是正 |
...
| OR-26 | C/C++ CodeQLがない | IN_PROGRESS | official CodeQL v4 commit pin、tests-OFF installable Host＋6-feature all-private Host production build、scope退行negativeを追加。ESP固有sourceは非claim、remote初回結果待ち |
...
| OR-32 | 47 normative docsのsource/translation statusが未記載 | CLOSED | review SHAのdocs直下47件、現行49件をsource language付き中央台帳で完全列挙。未登録source追加・不正language・番号付き索引漏れをgateで拒否 |
...
| OR-35 | positive security findings（nonce/replay/AEAD/TLS/file checks等） | NO_ACTION | 現行安全境界を回帰gateで維持する。別の弱化差分を入れない |
