...
static ninlil_rt_service_slot_t *find_desired_receiver(
    ...
    const ninlil_service_identity_t *identity)
{
    ...
    for (index = 0u; index < runtime->service_capacity; ++index) {
        ...
        if (slot->descriptor.family != NINLIL_FAMILY_DESIRED_STATE) {
            continue;
        }
    ...
    ninlil_rt_v1_step_delivery_result_t *out_result)
{
    ...
    receiver = find_desired_receiver(runtime, &message->service);
    if (receiver == NULL) {
        return NINLIL_OK;
    }
    ...
    ninlil_rt_v1_step_delivery_result_t *out_result)
{
    ...
    for (index = 0u; index < runtime->service_capacity; ++index) {
        ninlil_rt_service_slot_t *slot = &runtime->services[index];
        if (slot->in_use != 0u && slot->attached != 0u
            && slot->descriptor.family == NINLIL_FAMILY_DESIRED_STATE
