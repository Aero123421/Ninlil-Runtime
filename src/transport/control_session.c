...
/*
 * U3 C3 session + C4 pump implementation.
 ...
 * session->last_error / stats / queues / residual / state.
 */
    ...
    ninlil_ctrl_session_error_t *out_error)
{
    ...
    /* Post-I/O ticket before accepting any TX progress. */
    ticket = validate_link_generation_ticket(
        session, NINLIL_CTRL_SESSION_STAGE_TX, out_error);
    if (ticket != NINLIL_CTRL_SESSION_OK) {
        return ticket;
    }

    if (st == NINLIL_BYTE_STREAM_WOULD_BLOCK) {
        /* Normative: accepted must be 0; residual (full frame) retained. */
        if (accepted != 0u) {
    ...
    /* C1 all-or-none: OK requires accepted == remain. Partial OK = violation. */
    if (accepted != remain) {
        session->stats.tx_partial_ok_fences =
            sat_add_u64(session->stats.tx_partial_ok_fences, 1u);
        fence_session(
            ...
            NINLIL_CTRL_SESSION_STAGE_TX,
            st,
            "C1 partial OK protocol violation",
            out_error);
        return NINLIL_CTRL_SESSION_IO_ERROR;
    }
    ...
    ninlil_ctrl_session_error_t *out_error)
{
     ...
     * Fail-closed continuity fence (ordered stream premise broken for us).
     */
    if (offset < length) {
        fence_session(
            session,
    ...
    ninlil_ctrl_session_error_t *out_error)
{
    ...
    for (spins = 0u; spins < 32u; ++spins) {
        ...
        ticket = validate_link_generation_ticket(
            session, NINLIL_CTRL_SESSION_STAGE_RX, out_error);
        if (ticket != NINLIL_CTRL_SESSION_OK) {
            return ticket;
        }
    ...
    ninlil_ctrl_session_error_t *out_error)
{
    ...
    ticket = validate_link_generation_ticket(
        session, NINLIL_CTRL_SESSION_STAGE_PUMP, out_error);
    if (ticket != NINLIL_CTRL_SESSION_OK) {
        return ticket;
    }
    ...
    ninlil_ctrl_session_error_t *out_error)
{
    ...
    if (!intent_try_enqueue(session, fields)) {
        session->stats.tx_would_block =
            sat_add_u64(session->stats.tx_would_block, 1u);
        set_error_sticky(
            session,
