    ...
    uint32_t *out_len)
{
    ...
    if (family == 6u && subtype == 0x33u) {
        return ninlil_model_domain_encode_body_cancel_state(
            &any->cancel_state, out, capacity, out_len);
    }
    if (family == 6u && subtype == 0x32u) {
    ...
    if (family == 6u && subtype == 0x42u) {
        return ninlil_model_domain_encode_body_reverse_reply(
            &any->reverse_reply, out, capacity, out_len);
    }
    if (family == 6u && subtype == 0x50u) {
    ...
    if (family == 6u && subtype == 0x52u) {
        return ninlil_model_domain_encode_body_management_ledger(
            &any->management_ledger, out, capacity, out_len);
    }
    if (family == 6u && subtype == 0x61u) {
        return ninlil_model_domain_encode_body_retention_basis(
            &any->retention_basis, out, capacity, out_len);
    }
    if (family == 6u && subtype == 0x63u) {
        return ninlil_model_domain_encode_body_cleanup_plan(
            &any->cleanup_plan, out, capacity, out_len);
    }
    return NINLIL_E_INVALID_ARGUMENT;
}
...
#define QCHECK(cond)                                                           \
    ...
    if (strcmp(v->op, "body_decode") == 0
        ...
        /* MANAGEMENT_LEDGER positives: kind matrix + replay_rev = expected+1. */
        if (v->family == 6u && v->subtype == 0x52u) {
            const ninlil_model_domain_body_management_ledger_t *ml =
                &any.management_ledger;
            QCHECK(ml->operation_kind
                    == NINLIL_MODEL_DOMAIN_MANAGEMENT_KIND_EVENT_RESUME
                || ml->operation_kind
                    == NINLIL_MODEL_DOMAIN_MANAGEMENT_KIND_EVENT_DISCARD);
            QCHECK(ml->reserved0 == 0u && ml->reserved1 == 0u
                && ml->reserved2 == 0u && ml->reserved3 == 0u);
            QCHECK(ml->ordered_sequence >= 1u);
            QCHECK(ml->expected_spool_revision >= 1u
                && ml->expected_spool_revision < UINT64_MAX);
            QCHECK(ml->replay_spool_revision
                == ml->expected_spool_revision + 1u);
            QCHECK(ml->audit_length >= 1u
                && ml->audit_length
                    <= NINLIL_MODEL_DOMAIN_MANAGEMENT_AUDIT_BYTES);
            QCHECK(!zeros(ml->operation_id, 16u));
            QCHECK(!zeros(ml->transaction_id, 16u));
            QCHECK(!zeros(ml->event_id, 16u));
            QCHECK(!zeros(ml->actor_id, 16u));
            if (ml->operation_kind
                == NINLIL_MODEL_DOMAIN_MANAGEMENT_KIND_EVENT_RESUME) {
                QCHECK(ml->expected_content_digest_algorithm == 0u);
                ...
                QCHECK(ml->replay_result_reason == NINLIL_REASON_NONE);
                QCHECK(ml->replay_retry_cycle_id >= 1u);
            } else {
                QCHECK(ml->expected_content_digest_algorithm
                    == NINLIL_MODEL_DOMAIN_CONTENT_DIGEST_SHA256);
                QCHECK(!zeros(ml->expected_event_id, 16u));
                QCHECK(!zeros(ml->expected_content_digest, 32u));
                QCHECK(memcmp(ml->event_id, ml->expected_event_id, 16u)
                    == 0);
                QCHECK(ml->acknowledge_flag == 1u);
                QCHECK(!zeros(ml->audit_clock_epoch, 16u));
                QCHECK(ml->replay_spool_released == 1u);
                QCHECK(ml->request_reason >= 1u && ml->request_reason <= 4u);
                QCHECK(ml->replay_result_kind
                    == NINLIL_EVENT_DISCARD_ALREADY_DISCARDED);
                QCHECK(ml->replay_result_reason
                    == NINLIL_REASON_OPERATOR_DISCARDED_WITHOUT_REQUIRED_RECEIPT);
                QCHECK(ml->replay_retry_cycle_id == 0u);
            }
        }
        /* RETENTION_BASIS positives: kind 2/3, length 106/170, matrix. */
        if (v->family == 6u && v->subtype == 0x61u) {
            const ninlil_model_domain_body_retention_basis_t *rb =
                &any.retention_basis;
            QCHECK(rb->subject_kind
                    == NINLIL_MODEL_DOMAIN_RETENTION_SUBJECT_TRANSACTION
                || rb->subject_kind
                    == NINLIL_MODEL_DOMAIN_RETENTION_SUBJECT_DELIVERY);
            QCHECK(rb->reserved == 0u);
            QCHECK(rb->basis_pending <= 1u && rb->retention_overflow <= 1u);
            QCHECK(rb->required_window_ms > 0u);
            QCHECK(rb->subject_key_raw != NULL);
            if (rb->subject_kind
                == NINLIL_MODEL_DOMAIN_RETENTION_SUBJECT_TRANSACTION) {
                QCHECK(rb->subject_key_raw_length == 16u);
                QCHECK((uint32_t)n == 106u);
            } else {
                QCHECK(rb->subject_key_raw_length == 80u);
                QCHECK((uint32_t)n == 170u);
            }
            QCHECK(rb->retention_state
        ...
        /* CLEANUP_PLAN positives: kind 2/3, length 142/206, phase matrix. */
        if (v->family == 6u && v->subtype == 0x63u) {
            const ninlil_model_domain_body_cleanup_plan_t *cp =
                &any.cleanup_plan;
            QCHECK(cp->subject_kind
                    == NINLIL_MODEL_DOMAIN_CLEANUP_SUBJECT_TRANSACTION
                || cp->subject_kind
                    == NINLIL_MODEL_DOMAIN_CLEANUP_SUBJECT_DELIVERY);
            QCHECK(cp->reserved == 0u);
            QCHECK(cp->attempt_reuse_fenced <= 1u);
            QCHECK(cp->cleanup_generation >= 1u);
            QCHECK(cp->batch_generation >= 1u);
            QCHECK(cp->subject_key_raw != NULL);
            QCHECK(!zeros(cp->subject_primary_value_digest, 32u));
            if (cp->subject_kind
                == NINLIL_MODEL_DOMAIN_CLEANUP_SUBJECT_TRANSACTION) {
                QCHECK(cp->subject_key_raw_length == 16u);
                QCHECK((uint32_t)n == 142u);
            } else {
                QCHECK(cp->subject_key_raw_length == 80u);
                QCHECK((uint32_t)n == 206u);
            }
            QCHECK(cp->cleanup_phase
    ...
    if (strcmp(v->op, "typed_record") == 0) {
        ...
        if (expect != NINLIL_OK) {
            ...
            if (v->subtype == 0x01u) {
            ...
            } else if (v->subtype == 0x33u) {
                /* CANCEL_STATE: rev>=1/flags0, composite key, RAW16 borrow. */
                QCHECK(rec.envelope.header.record_revision >= 1u);
                QCHECK(rec.envelope.header.flags == 0u);
                QCHECK(rec.cancel_state.owner_key_raw != NULL);
                QCHECK(rec.cancel_state.owner_key_raw_length > 0u);
                /* owner_key_raw contents start after kind+res+len = 6. */
                QCHECK(rec.cancel_state.owner_key_raw == &body_start[6]);
                QCHECK(!zeros(rec.cancel_state.transaction_id, 16u));
                QCHECK(!zeros(rec.cancel_state.primary_key_digest, 32u));
                QCHECK(!zeros(rec.envelope.header.primary_id, 16u));
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
                QCHECK(rec.key.identity_length == 32u);
            } else if (v->subtype == 0x32u) {
                /* EVIDENCE_CELL: rev>=1/flags0, composite key, RAW16 borrow. */
                QCHECK(rec.envelope.header.record_revision >= 1u);
                QCHECK(rec.envelope.header.flags == 0u);
                QCHECK(rec.evidence_cell.owner_key_raw != NULL);
                QCHECK(rec.evidence_cell.owner_key_raw_length > 0u);
                /* owner_key_raw contents start after kind+cell+len = 6. */
                QCHECK(rec.evidence_cell.owner_key_raw == &body_start[6]);
                QCHECK(!zeros(rec.evidence_cell.primary_key_digest, 32u));
                QCHECK(!zeros(rec.evidence_cell.target_digest, 32u));
                QCHECK(!zeros(rec.envelope.header.primary_id, 16u));
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
                QCHECK(rec.key.identity_length == 32u);
                ...
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
                QCHECK(rec.key.identity_length == 32u);
            } else if (v->subtype == 0x42u) {
                /* REVERSE_REPLY: rev>=1/flags0/PVD NZ, exact 330, raw86+raw80. */
                QCHECK(rec.envelope.header.record_revision >= 1u);
                QCHECK(rec.envelope.header.flags == 0u);
                QCHECK(!zeros(rec.envelope.header.primary_value_digest, 32u));
                QCHECK(!zeros(rec.envelope.header.head_witness_digest, 32u));
                QCHECK(rec.reverse_reply.reply_key_raw != NULL);
                QCHECK(rec.reverse_reply.reply_key_raw_length == 86u);
                QCHECK(rec.reverse_reply.reply_key_raw == &body_start[2]);
                QCHECK(rec.reverse_reply.delivery_key_raw != NULL);
                QCHECK(rec.reverse_reply.delivery_key_raw_length == 80u);
                QCHECK(rec.reverse_reply.delivery_key_raw == &body_start[90]);
                QCHECK(!zeros(rec.reverse_reply.transaction_id, 16u));
                QCHECK(!zeros(rec.reverse_reply.attempt_id, 16u));
                QCHECK(!zeros(rec.reverse_reply.semantic_digest, 32u));
                QCHECK(!zeros(rec.reverse_reply.body_blob_key_digest, 32u));
                QCHECK(rec.reverse_reply.reserved == 0u);
                QCHECK(rec.reverse_reply.send_invocation_count
                    <= rec.reverse_reply.send_operation_generation);
                QCHECK(rec.reverse_reply.reply_kind >= 1u
                    && rec.reverse_reply.reply_kind <= 4u);
                QCHECK(!zeros(rec.envelope.header.primary_id, 16u));
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
                QCHECK(rec.key.identity_length == 32u);
            } else if (v->subtype == 0x50u) {
                /* EVENT_SPOOL: rev==spool_rev>=1/flags0/PVD NZ, exact 300. */
                QCHECK(rec.envelope.header.record_revision >= 1u);
                QCHECK(rec.envelope.header.record_revision
                    == rec.event_spool.spool_revision);
                QCHECK(rec.envelope.header.flags == 0u);
                QCHECK(!zeros(rec.envelope.header.primary_value_digest, 32u));
                QCHECK(!zeros(rec.envelope.header.head_witness_digest, 32u));
                QCHECK(!zeros(rec.event_spool.transaction_id, 16u));
                QCHECK(!zeros(rec.event_spool.event_id, 16u));
                QCHECK(!zeros(rec.event_spool.payload_blob_key_digest, 32u));
                QCHECK(!zeros(rec.event_spool.reservation_key_digest, 32u));
                QCHECK(rec.event_spool.retry_cycle_id >= 1u);
                QCHECK(rec.event_spool.successful_resume_count <= 8u);
                QCHECK(rec.event_spool.discard_committed <= 1u);
                QCHECK((rec.event_spool.discard_committed == 1u)
                    == (rec.event_spool.spool_state
                        == NINLIL_MODEL_DOMAIN_SPOOL_STATE_DISCARDED));
                QCHECK(memcmp(
                           rec.envelope.header.primary_id,
                           rec.event_spool.transaction_id, 16u)
                    == 0);
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_ID128);
                QCHECK(rec.key.identity_length == 16u);
            ...
            } else if (v->subtype == 0x52u) {
                /* MANAGEMENT_LEDGER: rev exact1/flags0/PVD NZ, composite key. */
                QCHECK(rec.envelope.header.record_revision == 1u);
                QCHECK(rec.envelope.header.flags == 0u);
                QCHECK(!zeros(rec.envelope.header.primary_value_digest, 32u));
                QCHECK(!zeros(rec.envelope.header.head_witness_digest, 32u));
                QCHECK(rec.envelope.body.length == 364u);
                QCHECK(!zeros(rec.management_ledger.transaction_id, 16u));
                QCHECK(!zeros(rec.management_ledger.operation_id, 16u));
                QCHECK(rec.management_ledger.operation_kind == 15u
                    || rec.management_ledger.operation_kind == 16u);
                QCHECK(memcmp(
                           rec.envelope.header.primary_id,
                           rec.management_ledger.transaction_id, 16u)
                    == 0);
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
                QCHECK(rec.key.identity_length == 32u);
                QCHECK(rec.management_ledger.replay_spool_revision
                    == rec.management_ledger.expected_spool_revision + 1u);
            } else if (v->subtype == 0x61u) {
                /* RETENTION_BASIS: flags0/rev>=1/PVD NZ, composite key. */
                QCHECK(rec.envelope.header.record_revision >= 1u);
                QCHECK(rec.envelope.header.flags == 0u);
                QCHECK(!zeros(rec.envelope.header.primary_value_digest, 32u));
                QCHECK(!zeros(rec.envelope.header.head_witness_digest, 32u));
                QCHECK(rec.envelope.body.length == 106u
                    || rec.envelope.body.length == 170u);
                QCHECK(rec.retention_basis.subject_key_raw != NULL);
                QCHECK(rec.retention_basis.subject_kind == 2u
                    || rec.retention_basis.subject_kind == 3u);
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
                QCHECK(rec.key.identity_length == 32u);
                QCHECK(rec.retention_basis.required_window_ms > 0u);
            } else if (v->subtype == 0x63u) {
                /* CLEANUP_PLAN: flags0/rev==batch_gen/PVD bijection. */
                QCHECK(rec.envelope.header.record_revision >= 1u);
                QCHECK(rec.envelope.header.flags == 0u);
                QCHECK(rec.envelope.header.record_revision
                    == rec.cleanup_plan.batch_generation);
                QCHECK(!zeros(rec.envelope.header.primary_value_digest, 32u));
                QCHECK(!zeros(rec.envelope.header.head_witness_digest, 32u));
                QCHECK(memcmp(
                           rec.cleanup_plan.subject_primary_value_digest,
                           rec.envelope.header.primary_value_digest, 32u)
                    == 0);
                QCHECK(rec.envelope.body.length == 142u
                    || rec.envelope.body.length == 206u);
                QCHECK(rec.cleanup_plan.subject_key_raw != NULL);
                QCHECK(rec.cleanup_plan.subject_kind == 2u
                    || rec.cleanup_plan.subject_kind == 3u);
                QCHECK(rec.cleanup_plan.cleanup_phase >= 1u
                    && rec.cleanup_plan.cleanup_phase <= 3u);
                QCHECK(rec.key.identity_kind
                    == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
                QCHECK(rec.key.identity_length == 32u);
            }
...
static int test_catalog_and_replay(const char *path)
{
    ...
    REQUIRE(cov52 == file.catalog.dsb3_subtype_52_positive);
    REQUIRE(cov61 == file.catalog.dsb3_subtype_61_positive);
    REQUIRE(cov63 == file.catalog.dsb3_subtype_63_positive);
    /* D1-B1 + D1-B2 + D1-B3a..o subtype coverage. */
    if (cov01 == 0u || cov60 == 0u || cov62 == 0u || cov64 == 0u
        ...
        || cov51 == 0u || cov52 == 0u || cov61 == 0u || cov63 == 0u) {
        unimplemented = 1u;
    }
    ...
    (void)fprintf(stdout,
        ...
        "cov32=%u cov40=%u cov41=%u cov42=%u cov50=%u cov51=%u cov52=%u "
        "cov61=%u cov63=%u sizeof(ninlil_dv_vector_t)=%zu\n",
        ...
        cov33, cov32, cov40, cov41, cov42, cov50, cov51, cov52, cov61, cov63,
...
{
    /*
     * Fixed-body alias storage/output must fit the largest exact fixed D1
     * bodies exercised here: MANAGEMENT_LEDGER (364) and REVERSE_REPLY (330).
     * A 256-byte member is insufficient for those EncodedBytes views.
     */
    uint8_t out[512];
    uint8_t out_before[512];
    uint8_t dig[32];
    uint8_t marker[16];
    uint32_t len = 77u;
    uint32_t len_before;
...
#define CHECK_FIXED_BODY_ALIAS(Type, Encode, Decode, EncodedBytes)             \
    ...
    CHECK_FIXED_BODY_ALIAS(
        ...
        ninlil_model_domain_encode_body_management_ledger,
        ninlil_model_domain_decode_body_management_ledger,
...
#define CHECK_VAR_ENCODE_BTS_AFTER_DECODE(Type, Decode, Encode, HexLit)        \
    ...
    /* --- D1-B3f CANCEL_STATE (RAW16 owner_key_raw) --- */
    CHECK_VAR_ENCODE_BODY_OUT_ALIAS(
        ninlil_model_domain_body_cancel_state_t,
        ninlil_model_domain_encode_body_cancel_state);
    CHECK_VAR_ENCODE_BODY_LEN_ALIAS(
        ninlil_model_domain_body_cancel_state_t,
        ninlil_model_domain_encode_body_cancel_state);
    CHECK_VAR_ENCODE_RAW_ALIASES(
    ...
    CHECK_VAR_DECODE_ALIAS_AND_OVERFLOW(
        ninlil_model_domain_body_cancel_state_t,
        ninlil_model_domain_decode_body_cancel_state);

    /* --- D1-B3n RETENTION_BASIS (RAW16 subject_key_raw) --- */
    CHECK_VAR_ENCODE_BODY_OUT_ALIAS(
        ninlil_model_domain_body_retention_basis_t,
        ninlil_model_domain_encode_body_retention_basis);
    CHECK_VAR_ENCODE_BODY_LEN_ALIAS(
        ninlil_model_domain_body_retention_basis_t,
        ninlil_model_domain_encode_body_retention_basis);
    CHECK_VAR_ENCODE_RAW_ALIASES(
    ...
    CHECK_VAR_DECODE_ALIAS_AND_OVERFLOW(
        ninlil_model_domain_body_retention_basis_t,
        ninlil_model_domain_decode_body_retention_basis);

    /* --- D1-B3o CLEANUP_PLAN (RAW16 subject_key_raw) --- */
    CHECK_VAR_ENCODE_BODY_OUT_ALIAS(
        ninlil_model_domain_body_cleanup_plan_t,
        ninlil_model_domain_encode_body_cleanup_plan);
    CHECK_VAR_ENCODE_BODY_LEN_ALIAS(
        ninlil_model_domain_body_cleanup_plan_t,
        ninlil_model_domain_encode_body_cleanup_plan);
    CHECK_VAR_ENCODE_RAW_ALIASES(
    ...
    CHECK_VAR_DECODE_ALIAS_AND_OVERFLOW(
        ninlil_model_domain_body_cleanup_plan_t,
        ninlil_model_domain_decode_body_cleanup_plan);

    /* --- D1-B3g EVIDENCE_CELL (RAW16 owner_key_raw) --- */
    CHECK_VAR_ENCODE_BODY_OUT_ALIAS(
        ninlil_model_domain_body_evidence_cell_t,
        ninlil_model_domain_encode_body_evidence_cell);
    CHECK_VAR_ENCODE_BODY_LEN_ALIAS(
        ninlil_model_domain_body_evidence_cell_t,
        ninlil_model_domain_encode_body_evidence_cell);
    CHECK_VAR_ENCODE_RAW_ALIASES(
    ...
    /* --- D1-B3j REVERSE_REPLY (dual RAW16: reply_key_raw + delivery_key_raw) --- */
    CHECK_VAR_ENCODE_BODY_OUT_ALIAS(
        ninlil_model_domain_body_reverse_reply_t,
        ninlil_model_domain_encode_body_reverse_reply);
    CHECK_VAR_ENCODE_BODY_LEN_ALIAS(
        ninlil_model_domain_body_reverse_reply_t,
        ninlil_model_domain_encode_body_reverse_reply);
    /* Invalid-shape macros remain as supplementary coverage. */
    CHECK_VAR_ENCODE_RAW_ALIASES(
        ninlil_model_domain_body_reverse_reply_t,
        ninlil_model_domain_encode_body_reverse_reply,
        reply_key_raw, reply_key_raw_length);
    CHECK_VAR_ENCODE_RAW_ALIASES(
        ninlil_model_domain_body_reverse_reply_t,
        ninlil_model_domain_encode_body_reverse_reply,
        delivery_key_raw, delivery_key_raw_length);
    ...
    CHECK_VAR_DECODE_ALIAS_AND_OVERFLOW(
        ninlil_model_domain_body_reverse_reply_t,
        ninlil_model_domain_decode_body_reverse_reply);
    /*
     * Causal alias tests: golden-legal REVERSE_REPLY body, valid fields.
     * reply_key_raw contents = delivery_key_raw:RAW16 || reply_kind, so
     * delivery_key_raw = reply_key_raw+2 keeps bijection/matrix legal and only
     * makes the two nested ranges overlap in real memory. capacity >= 330 so
     * if multi_ranges_ok were removed, encode would succeed and these asserts
     * would fail (out_length/out buffer would change).
     */
    {
        ninlil_dv_file_t file;
        char err[256];
        ...
        uint8_t enc[512];
        size_t hn = 0u;
        ninlil_model_domain_body_reverse_reply_t body;
        ninlil_model_domain_body_reverse_reply_t object;
        ninlil_model_domain_body_reverse_reply_t object_before;
        uint8_t reply_own[86];
        uint8_t reply_before[86];
        uint8_t delivery_own[80];
        uint8_t delivery_before[80];
        uint8_t local_out[512];
        uint8_t local_before[512];
        uint32_t local_len;
        uint32_t local_len_before;
        uint32_t ok_len = 0u;
        ...
        REQUIRE(ninlil_model_domain_decode_body_reverse_reply(
                    (ninlil_bytes_view_t){enc, (uint32_t)hn}, &body)
            == NINLIL_OK);
        REQUIRE(body.reply_key_raw_length == 86u);
        REQUIRE(body.delivery_key_raw_length == 80u);
        REQUIRE(body.reply_key_raw != NULL);
        REQUIRE(body.delivery_key_raw != NULL);
        /* Nested delivery contents live at reply_key_raw+2 (after RAW16 u16). */
        REQUIRE(memcmp(body.reply_key_raw + 2, body.delivery_key_raw, 80u)
            == 0);

        (void)memcpy(reply_own, body.reply_key_raw, 86u);
        (void)memcpy(delivery_own, body.delivery_key_raw, 80u);

        /* Baseline: disjoint owned RAWs → encode must succeed (fields legal). */
        object = body;
        object.reply_key_raw = reply_own;
        object.delivery_key_raw = delivery_own;
        (void)memset(local_out, 0x5A, sizeof(local_out));
        ok_len = 0u;
        REQUIRE(ninlil_model_domain_encode_body_reverse_reply(
                    &object, local_out, (uint32_t)sizeof(local_out), &ok_len)
            == NINLIL_OK);
        REQUIRE(ok_len == 330u);
        REQUIRE(memcmp(local_out, enc, 330u) == 0);

        /*
         * Dual RAW overlap: delivery_key_raw → reply_key_raw+2.
         * Content/length/txn/kind/state remain legal; only ranges overlap.
         */
        object = body;
        object.reply_key_raw = reply_own;
        object.reply_key_raw_length = 86u;
        object.delivery_key_raw = reply_own + 2;
        object.delivery_key_raw_length = 80u;
        object_before = object;
        (void)memcpy(reply_before, reply_own, 86u);
        (void)memset(local_out, 0x5A, sizeof(local_out));
        (void)memcpy(local_before, local_out, sizeof(local_out));
        local_len = 99u;
        local_len_before = local_len;
        REQUIRE(ninlil_model_domain_encode_body_reverse_reply(
                    &object, local_out, (uint32_t)sizeof(local_out),
                    &local_len)
            == NINLIL_E_INVALID_ARGUMENT);
        REQUIRE(local_len == local_len_before);
        REQUIRE(memcmp(local_out, local_before, sizeof(local_out)) == 0);
        REQUIRE(memcmp(&object, &object_before, sizeof(object)) == 0);
        REQUIRE(memcmp(reply_own, reply_before, 86u) == 0);

        /*
         * reply_key_raw ↔ out: place legal reply bytes in out so fields_ok
         * would pass without multi_ranges; capacity still >= 330.
         */
        object = body;
        (void)memset(local_out, 0x5A, sizeof(local_out));
        (void)memcpy(local_out, reply_own, 86u);
        object.reply_key_raw = local_out;
        object.reply_key_raw_length = 86u;
        object.delivery_key_raw = delivery_own;
        object.delivery_key_raw_length = 80u;
        object_before = object;
        (void)memcpy(delivery_before, delivery_own, 80u);
        (void)memcpy(local_before, local_out, sizeof(local_out));
        local_len = 88u;
        local_len_before = local_len;
        REQUIRE(ninlil_model_domain_encode_body_reverse_reply(
        ...
        REQUIRE(memcmp(local_out, local_before, sizeof(local_out)) == 0);
        REQUIRE(memcmp(&object, &object_before, sizeof(object)) == 0);
        REQUIRE(memcmp(delivery_own, delivery_before, 80u) == 0);

        /*
         * delivery_key_raw ↔ out: same causality for the second RAW.
         */
        object = body;
        (void)memset(local_out, 0x5A, sizeof(local_out));
        (void)memcpy(local_out, delivery_own, 80u);
        object.reply_key_raw = reply_own;
        object.reply_key_raw_length = 86u;
        object.delivery_key_raw = local_out;
        object.delivery_key_raw_length = 80u;
        object_before = object;
        (void)memcpy(reply_before, reply_own, 86u);
        (void)memcpy(local_before, local_out, sizeof(local_out));
    ...
    /*
     * NULL nested RAW with nonzero length: range gate admits (NULL not in
     * multi_ranges), then required==0 → INVALID_ARGUMENT with *out_length=0
     * and out buffer / body object untouched (same as other body codecs).
     */
    {
        ninlil_model_domain_body_reverse_reply_t object;
        ninlil_model_domain_body_reverse_reply_t object_before;
        ...
        (void)memset(&object, 0, sizeof(object));
        (void)memset(raw_own, 0x41, sizeof(raw_own));
        object.reply_key_raw = NULL;
        object.reply_key_raw_length = 8u;
        object.delivery_key_raw = raw_own;
        object.delivery_key_raw_length = 8u;
        object_before = object;
        (void)memset(local_out, 0x5A, sizeof(local_out));
        (void)memcpy(local_before, local_out, sizeof(local_out));
        local_len = 41u;
        REQUIRE(ninlil_model_domain_encode_body_reverse_reply(
                    &object, local_out, sizeof(local_out), &local_len)
            == NINLIL_E_INVALID_ARGUMENT);
        REQUIRE(local_len == 0u);
        REQUIRE(memcmp(local_out, local_before, sizeof(local_out)) == 0);
        REQUIRE(memcmp(&object, &object_before, sizeof(object)) == 0);

        object.reply_key_raw = raw_own;
        object.reply_key_raw_length = 8u;
        object.delivery_key_raw = NULL;
        object.delivery_key_raw_length = 8u;
        object_before = object;
        local_len = 42u;
        REQUIRE(ninlil_model_domain_encode_body_reverse_reply(
...
static int test_catalog_format_mutations(const char *path)
{
    ...
    {
        char *p = strstr(mut, "\"format\": \"ninlil-domain-store-v1-d1b3o\"");
        REQUIRE(p != NULL);
        /* overwrite to wrong format of same length */
        (void)memcpy(p,
            "\"format\": \"ninlil-domain-store-v1-d1b3X\"",
            strlen("\"format\": \"ninlil-domain-store-v1-d1b3X\""));
...
/*
 * D1-B3f CANCEL_STATE: independent oracle for primary_key_digest,
 * COMPOSITE key identity, primary_id, closed matrix shape (attempt/digest
 * pair + bijection + gate/timeout), and typed-path output-zero on failure.
 * First-809 exact append-only preservation is the durable Python fingerprint
 * gate (PRE_B3F_VECTORS_FINGERPRINT).
 */
static int test_cancel_state_contracts(const char *path)
{
    ninlil_dv_file_t file;
    char err[256];
    size_t i;
    ...
    REQUIRE(path != NULL);
    REQUIRE(ninlil_dv_load_file(path, &file, err, sizeof(err)) == 0);
    /* Non-CS id-prefix guard; exact first-809 bytes gated by Python FP. */
    REQUIRE(file.vector_count > 809u);
    for (i = 0u; i < 809u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_CS_", 8) != 0);
    }
    ...
    for (i = 0u; i < file.vector_count; ++i) {
        ...
        REQUIRE(hex_to(ninlil_dv_str(v->body_hex), enc, sizeof(enc), &bn)
            == 0);
        REQUIRE(
            bn == NINLIL_MODEL_DOMAIN_BODY_CANCEL_STATE_TX_BYTES
            || bn == NINLIL_MODEL_DOMAIN_BODY_CANCEL_STATE_DELIVERY_BYTES);
        REQUIRE(ninlil_model_domain_decode_body_cancel_state(
                (ninlil_bytes_view_t){enc, (uint32_t)bn}, &body)
            == NINLIL_OK);
        REQUIRE(!zeros(body.transaction_id, 16u));
        REQUIRE(!zeros(body.primary_key_digest, 32u));
        REQUIRE(body.reserved == 0u);
        REQUIRE(body.owner_key_raw != NULL);
        REQUIRE(body.owner_key_raw_length > 0u);

        /* Independent KEY_DIGEST(complete primary) — do not discard. */
        if (body.cancel_owner_kind
            == NINLIL_MODEL_DOMAIN_CANCEL_OWNER_TRANSACTION) {
            seen_tx = 1;
            REQUIRE(body.owner_key_raw_length
                == NINLIL_MODEL_DOMAIN_CANCEL_OWNER_KEY_TX_BYTES);
            REQUIRE(memcmp(body.owner_key_raw, body.transaction_id, 16u)
                == 0);
            {
                ninlil_bytes_view_t idv;
                idv.data = body.transaction_id;
                ...
                REQUIRE(ninlil_model_domain_build_key(
                    ...
                    == NINLIL_OK);
                REQUIRE(ninlil_model_domain_key_digest(
                        (ninlil_bytes_view_t){built.bytes, built.length},
                        &dig)
                    == NINLIL_OK);
                REQUIRE(memcmp(body.primary_key_digest, dig.bytes, 32u)
                    == 0);
            }
        } else if (
            body.cancel_owner_kind
            == NINLIL_MODEL_DOMAIN_CANCEL_OWNER_DELIVERY) {
            seen_dlv = 1;
            REQUIRE(body.owner_key_raw_length
                == NINLIL_MODEL_DOMAIN_CANCEL_OWNER_KEY_DELIVERY_BYTES);
            REQUIRE(memcmp(body.owner_key_raw + 32, body.transaction_id, 16u)
                == 0);
            ninlil_model_domain_encode_u16_be(
                components, body.owner_key_raw_length);
            o = 2u;
            (void)memcpy(&components[o], body.owner_key_raw,
                body.owner_key_raw_length);
            o += body.owner_key_raw_length;
            cv.data = components;
            cv.length = o;
            REQUIRE(ninlil_model_domain_composite_digest(
                    NINLIL_MODEL_DOMAIN_SUBTYPE_DELIVERY, cv, &dig)
        ...
        /* Matrix / bijection shape checks — independent of production helper. */
        {
            int attempt_zero = zeros(body.cancel_attempt_id, 16u);
            int digest_zero = zeros(body.message_semantic_digest, 32u);
            REQUIRE(attempt_zero == digest_zero);
            if (body.cancel_state
                == NINLIL_MODEL_DOMAIN_CANCEL_STATE_NONE) {
                seen_none = 1;
                REQUIRE(attempt_zero);
                REQUIRE(body.cancel_kind == 0u && body.reason == 0u
                    && body.effect_certainty == 0u);
                REQUIRE(body.cancel_send_gate_state
                ...
                seen_pend = 1;
                REQUIRE(!attempt_zero);
                REQUIRE(body.cancel_kind == 2u && body.reason == 86u
                    && body.effect_certainty == 0u);
                REQUIRE(body.cancel_owner_kind
                    == NINLIL_MODEL_DOMAIN_CANCEL_OWNER_TRANSACTION);
            } else if (
                body.cancel_state
                == NINLIL_MODEL_DOMAIN_CANCEL_STATE_FENCED_BEFORE_DISPATCH) {
                seen_fenced = 1;
                REQUIRE(body.cancel_kind == 1u && body.reason == 82u
                ...
                seen_too_late = 1;
                REQUIRE(!attempt_zero);
                REQUIRE(body.cancel_kind == 3u && body.reason == 83u
    ...
    REQUIRE(seen_none != 0);
    REQUIRE(seen_pend != 0);
    REQUIRE(seen_fenced != 0);
    REQUIRE(seen_too_late != 0);

    for (i = 0u; i < file.vector_count; ++i) {
        const ninlil_dv_vector_t *v = &file.vectors[i];
        if (v->subtype != 0x33u || strcmp(v->op, "typed_record") != 0
            || strcmp(v->expected_status, "OK") != 0) {
            continue;
        }
        seen_typed = 1;
        ...
        REQUIRE(ninlil_model_domain_parse_key(
            ...
            == NINLIL_OK);
        REQUIRE(kv.family == NINLIL_MODEL_DOMAIN_FAMILY_DOMAIN);
        REQUIRE(kv.subtype == NINLIL_MODEL_DOMAIN_SUBTYPE_CANCEL_STATE);
        REQUIRE(kv.identity_kind
            == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
        REQUIRE(kv.identity_length == 32u);
        REQUIRE(kv.identity != NULL);
        ...
        REQUIRE(ninlil_model_domain_validate_typed_record(
                ...
                (ninlil_bytes_view_t){val_buf, (uint32_t)vn}, &rec)
            == NINLIL_OK);
        REQUIRE(rec.subtype == 0x33u);
        REQUIRE(rec.envelope.header.record_revision >= 1u);
        REQUIRE(rec.envelope.header.flags == 0u);
        REQUIRE(!zeros(rec.envelope.header.head_witness_digest, 32u));
        REQUIRE(!zeros(rec.envelope.header.primary_value_digest, 32u));
        REQUIRE(!zeros(rec.cancel_state.primary_key_digest, 32u));
        REQUIRE(!zeros(rec.cancel_state.transaction_id, 16u));

        /* Independent COMPOSITE key recompute — must match key identity. */
        ninlil_model_domain_encode_u16_be(
            components, rec.cancel_state.cancel_owner_kind);
        o = 2u;
        ninlil_model_domain_encode_u16_be(
            &components[o], rec.cancel_state.owner_key_raw_length);
        o += 2u;
        (void)memcpy(
            &components[o], rec.cancel_state.owner_key_raw,
            rec.cancel_state.owner_key_raw_length);
        o += rec.cancel_state.owner_key_raw_length;
        cv.data = components;
        cv.length = o;
        REQUIRE(ninlil_model_domain_composite_digest(
                NINLIL_MODEL_DOMAIN_SUBTYPE_CANCEL_STATE, cv, &dig)
            == NINLIL_OK);
        REQUIRE(memcmp(kv.identity, dig.bytes, 32u) == 0);
        REQUIRE(memcmp(rec.key.identity, dig.bytes, 32u) == 0);

        /* Independent primary_id binding. */
        if (rec.cancel_state.cancel_owner_kind
            == NINLIL_MODEL_DOMAIN_CANCEL_OWNER_TRANSACTION) {
            (void)memcpy(expect_pid, rec.cancel_state.transaction_id, 16u);
        } else {
            ninlil_model_domain_encode_u16_be(
                components, rec.cancel_state.owner_key_raw_length);
            o = 2u;
            (void)memcpy(
                &components[o], rec.cancel_state.owner_key_raw,
                rec.cancel_state.owner_key_raw_length);
            ...
            REQUIRE(ninlil_model_domain_composite_digest(
                    NINLIL_MODEL_DOMAIN_SUBTYPE_DELIVERY, cv, &dig)
                == NINLIL_OK);
            (void)memcpy(expect_pid, dig.bytes, 16u);
        }
        REQUIRE(memcmp(rec.envelope.header.primary_id, expect_pid, 16u)
            == 0);
    }
    REQUIRE(seen_typed != 0);
    REQUIRE(typed_ok >= 4u);
    ...
    vn = 0u;
    for (i = 0u; i < file.vector_count; ++i) {
        if (strcmp(file.vectors[i].id, "DSB3_CS_TX_NONE_TYPED") == 0) {
            REQUIRE(hex_to(
                    ninlil_dv_str(file.vectors[i].key_hex), key_buf,
                    sizeof(key_buf), &kn)
                == 0);
    ...
    ninlil_dv_free(&file);
    (void)fprintf(stdout,
        "cancel_state contracts ok cs_vectors=%u typed_ok=%u\n", cs_count,
        typed_ok);
    return 0;
}

/*
 * D1-B3g EVIDENCE_CELL: fixed 734/798 lengths, service_slot pad, digest
 * recompute, COMPOSITE key / primary_id, old-898 exact prefix guard, and
 * typed-path output-zero on failure.
 */
static int test_evidence_cell_contracts(const char *path)
{
    ninlil_dv_file_t file;
    char err[256];
    size_t i;
    int seen_tx = 0;
    int seen_dlv = 0;
    int seen_sum_empty = 0;
    int seen_sum_mat = 0;
    int seen_raw_unused = 0;
    int seen_raw_mat = 0;
    int seen_typed = 0;
    int seen_len0_digest = 0;
    int seen_svc_min = 0;
    int seen_svc_max = 0;
    ...
    REQUIRE(path != NULL);
    REQUIRE(ninlil_dv_load_file(path, &file, err, sizeof(err)) == 0);
    /* First 898 vectors must remain pre-B3g (no DSB3_EV_ prefix). */
    REQUIRE(file.vector_count > 898u);
    for (i = 0u; i < 898u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_EV_", 8) != 0);
    }
    REQUIRE(strcmp(file.vectors[0].id, "DSK1_SHA256_EMPTY") == 0);
    REQUIRE(strcmp(file.vectors[897].id, "DSB3_CS_REV0") == 0);

    /* domain_sha256: length 0 requires data == NULL. */
    REQUIRE(ninlil_model_domain_sha256(NULL, 0u, &dig) == NINLIL_OK);
    (void)memcpy(empty_sha, dig.bytes, 32u);
    REQUIRE(!zeros(empty_sha, 32u));
    ...
    for (i = 0u; i < file.vector_count; ++i) {
        ...
        REQUIRE(hex_to(ninlil_dv_str(v->body_hex), enc, sizeof(enc), &bn)
            == 0);
        REQUIRE(
            bn == NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_CELL_TX_BYTES
            || bn == NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_CELL_DELIVERY_BYTES);
        REQUIRE(ninlil_model_domain_decode_body_evidence_cell(
                (ninlil_bytes_view_t){enc, (uint32_t)bn}, &body)
            == NINLIL_OK);
        REQUIRE(body.owner_key_raw != NULL);
        REQUIRE(body.owner_key_raw_length > 0u);
        REQUIRE(!zeros(body.primary_key_digest, 32u));
        REQUIRE(!zeros(body.target_digest, 32u));
        REQUIRE(body.reserved0 == 0u && body.reserved1 == 0u);
        REQUIRE(body.disposition == 0u && body.effect_certainty == 0u);

        if (body.evidence_owner_kind
            == NINLIL_MODEL_DOMAIN_EVIDENCE_OWNER_TRANSACTION) {
            seen_tx = 1;
            REQUIRE(body.owner_key_raw_length
                == NINLIL_MODEL_DOMAIN_EVIDENCE_OWNER_KEY_TX_BYTES);
            REQUIRE(bn == NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_CELL_TX_BYTES);
            {
                ninlil_bytes_view_t idv;
                idv.data = body.owner_key_raw;
                idv.length = 16u;
                REQUIRE(ninlil_model_domain_build_key(
                        NINLIL_MODEL_DOMAIN_FAMILY_DOMAIN,
                        NINLIL_MODEL_DOMAIN_SUBTYPE_TRANSACTION_ANCHOR,
            ...
            seen_dlv = 1;
            REQUIRE(body.owner_key_raw_length
                == NINLIL_MODEL_DOMAIN_EVIDENCE_OWNER_KEY_DELIVERY_BYTES);
            REQUIRE(
                bn == NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_CELL_DELIVERY_BYTES);
            ninlil_model_domain_encode_u16_be(
                components, body.owner_key_raw_length);
            o = 2u;
            (void)memcpy(
                &components[o], body.owner_key_raw,
                body.owner_key_raw_length);
            o += body.owner_key_raw_length;
            cv.data = components;
            cv.length = o;
        ...
        if (body.cell_kind == NINLIL_MODEL_DOMAIN_EVIDENCE_CELL_KIND_SUMMARY
            ...
            REQUIRE(body.evidence_length == 0u);
            REQUIRE(zeros(body.evidence_digest, 32u));
        } else if (
            ...
            REQUIRE(
                ...
                && body.highest_receipt_stage
                    == body.material_receipt_stage);
            {
                const uint8_t *edata =
                    body.evidence_length == 0u ? NULL : body.evidence_bytes;
                REQUIRE(ninlil_model_domain_sha256(
                        edata, body.evidence_length, &dig)
                    == NINLIL_OK);
                REQUIRE(memcmp(body.evidence_digest, dig.bytes, 32u) == 0);
            }
            if (body.evidence_length == 0u) {
                seen_len0_digest = 1;
                REQUIRE(memcmp(body.evidence_digest, empty_sha, 32u) == 0);
            }
            {
                uint32_t svc_len =
                    ninlil_model_domain_service_identity_encoded_length(
                        &body.service);
                REQUIRE(svc_len >= 54u && svc_len <= 240u);
                if (svc_len == 54u) {
                    seen_svc_min = 1;
                }
                if (svc_len == 240u) {
            ...
            seen_raw_unused = 1;
            REQUIRE(body.slot_index >= 1u && body.slot_index <= 8u);
        } else if (
            body.cell_kind == NINLIL_MODEL_DOMAIN_EVIDENCE_CELL_KIND_RAW
            && body.cell_state
                == NINLIL_MODEL_DOMAIN_EVIDENCE_CELL_STATE_MATERIALIZED) {
            seen_raw_mat = 1;
            REQUIRE(body.highest_receipt_stage == 0u);
            REQUIRE(body.latest_evidence_stage == 0u);
            REQUIRE(body.valid_material_count == 0u);
            {
                const uint8_t *edata =
                    body.evidence_length == 0u ? NULL : body.evidence_bytes;
                REQUIRE(ninlil_model_domain_sha256(
    ...
    REQUIRE(seen_tx != 0);
    REQUIRE(seen_dlv != 0);
    REQUIRE(seen_sum_empty != 0);
    REQUIRE(seen_sum_mat != 0);
    REQUIRE(seen_raw_unused != 0);
    REQUIRE(seen_raw_mat != 0);
    REQUIRE(seen_len0_digest != 0);
    REQUIRE(seen_svc_min != 0);
    REQUIRE(seen_svc_max != 0);

    for (i = 0u; i < file.vector_count; ++i) {
        const ninlil_dv_vector_t *v = &file.vectors[i];
        if (v->subtype != 0x32u || strcmp(v->op, "typed_record") != 0
            || strcmp(v->expected_status, "OK") != 0) {
            continue;
        }
        seen_typed = 1;
        ...
        REQUIRE(ninlil_model_domain_parse_key(
                (ninlil_bytes_view_t){key_buf, (uint32_t)kn}, &kv)
            == NINLIL_OK);
        REQUIRE(kv.subtype == NINLIL_MODEL_DOMAIN_SUBTYPE_EVIDENCE_CELL);
        REQUIRE(kv.identity_kind
            == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
        ...
        ninlil_model_domain_encode_u16_be(
            components, rec.evidence_cell.evidence_owner_kind);
        o = 2u;
        ninlil_model_domain_encode_u16_be(
            &components[o], rec.evidence_cell.owner_key_raw_length);
        o += 2u;
        (void)memcpy(
            &components[o], rec.evidence_cell.owner_key_raw,
            rec.evidence_cell.owner_key_raw_length);
        o += rec.evidence_cell.owner_key_raw_length;
        ninlil_model_domain_encode_u32_be(
            &components[o], rec.evidence_cell.slot_index);
        o += 4u;
        cv.data = components;
        cv.length = o;
        REQUIRE(ninlil_model_domain_composite_digest(
                NINLIL_MODEL_DOMAIN_SUBTYPE_EVIDENCE_CELL, cv, &dig)
            == NINLIL_OK);
        REQUIRE(memcmp(kv.identity, dig.bytes, 32u) == 0);

        if (rec.evidence_cell.evidence_owner_kind
            == NINLIL_MODEL_DOMAIN_EVIDENCE_OWNER_TRANSACTION) {
            (void)memcpy(
                expect_pid, rec.evidence_cell.owner_key_raw, 16u);
        } else {
            ninlil_model_domain_encode_u16_be(
                components, rec.evidence_cell.owner_key_raw_length);
            o = 2u;
            (void)memcpy(
                &components[o], rec.evidence_cell.owner_key_raw,
                rec.evidence_cell.owner_key_raw_length);
    ...
    vn = 0u;
    for (i = 0u; i < file.vector_count; ++i) {
        if (strcmp(file.vectors[i].id, "DSB3_EV_TX_SUM_EMPTY_TYPED") == 0) {
            REQUIRE(hex_to(
                    ninlil_dv_str(file.vectors[i].key_hex), key_buf,
                    sizeof(key_buf), &kn)
                == 0);
    ...
    /*
     * Padding-poison regression: inactive SUMMARY with semantic issuer/
     * service fields all zero but object padding left non-zero (0xA5) must
     * still encode OK and produce canonical zero PARTY + service_slot wire.
     * Object representation must not be required all-zero (portable C).
     */
    {
        ninlil_model_domain_body_evidence_cell_t empty_body;
        uint8_t gold[1024];
        uint8_t out[1024];
        size_t gn = 0u;
        uint32_t out_len = 0u;
        ...
        for (i = 0u; i < file.vector_count; ++i) {
            if (strcmp(file.vectors[i].id, "DSB3_EV_TX_SUM_EMPTY") == 0) {
                REQUIRE(hex_to(
                        ninlil_dv_str(file.vectors[i].body_hex), gold,
                        sizeof(gold), &gn)
                    == 0);
                break;
            }
        }
        REQUIRE(gn == NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_CELL_TX_BYTES);
        REQUIRE(ninlil_model_domain_decode_body_evidence_cell(
                (ninlil_bytes_view_t){gold, (uint32_t)gn}, &empty_body)
            == NINLIL_OK);

        /* Poison entire objects, then zero only semantic wire fields. */
        (void)memset(&empty_body.issuer, 0xA5, sizeof(empty_body.issuer));
        (void)memset(empty_body.issuer.runtime_id, 0, 16u);
        (void)memset(empty_body.issuer.application_instance_id, 0, 16u);
        empty_body.issuer.local_identity.flags = 0u;
        (void)memset(empty_body.issuer.local_identity.device, 0, 16u);
        (void)memset(empty_body.issuer.local_identity.installation, 0, 16u);
        (void)memset(empty_body.issuer.local_identity.site, 0, 16u);
        empty_body.issuer.local_identity.binding_epoch = 0u;
        empty_body.issuer.local_identity.membership_epoch = 0u;
        /*
         * On LP64, local_identity has padding before u64 epochs so object
         * size exceeds wire PARTY 100 and retains 0xA5. Field-wise zero
         * must still accept this; whole-object memcmp would incorrectly fail.
         */
        if (sizeof(empty_body.issuer) > NINLIL_MODEL_DOMAIN_PARTY_BYTES) {
            REQUIRE(!zeros(&empty_body.issuer, sizeof(empty_body.issuer)));
        }

        (void)memset(&empty_body.service, 0xA5, sizeof(empty_body.service));
        empty_body.service.namespace_id.length = 0u;
        (void)memset(empty_body.service.namespace_id.bytes, 0,
            sizeof(empty_body.service.namespace_id.bytes));
        empty_body.service.service_id.length = 0u;
        (void)memset(empty_body.service.service_id.bytes, 0,
            sizeof(empty_body.service.service_id.bytes));
        empty_body.service.schema_id.length = 0u;
        (void)memset(empty_body.service.schema_id.bytes, 0,
            sizeof(empty_body.service.schema_id.bytes));
        empty_body.service.descriptor_revision = 0u;
        (void)memset(empty_body.service.descriptor_digest, 0, 32u);
        empty_body.service.schema_major = 0u;
        empty_body.service.schema_minor = 0u;
        empty_body.service.family = 0u;
        /* Same portable poison for any trailing/alignment padding in service. */
        if (sizeof(empty_body.service)
            > NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_SERVICE_SLOT_BYTES) {
            REQUIRE(!zeros(&empty_body.service, sizeof(empty_body.service)));
        }

        (void)memset(out, 0x5A, sizeof(out));
        out_len = 0u;
        REQUIRE(ninlil_model_domain_encode_body_evidence_cell(
                &empty_body, out, (uint32_t)sizeof(out), &out_len)
            == NINLIL_OK);
        REQUIRE(out_len
            == NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_CELL_TX_BYTES);
        /* Identity prefix matches golden empty; remaining empty material wire
         * after identity is also zero. Full body equals golden empty. */
        REQUIRE(memcmp(out, gold, out_len) == 0);

        /* Explicit PARTY 100 + service_slot 240 zero on wire. */
        party_off = 4u + 2u + 16u + 32u + 32u + 4u + 2u + 2u + 24u;
        slot_off = party_off + NINLIL_MODEL_DOMAIN_PARTY_BYTES;
        REQUIRE(party_off + NINLIL_MODEL_DOMAIN_PARTY_BYTES
                + NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_SERVICE_SLOT_BYTES
            <= out_len);
        for (pi = 0u; pi < NINLIL_MODEL_DOMAIN_PARTY_BYTES; ++pi) {
            REQUIRE(out[party_off + pi] == 0u);
        }
        for (pi = 0u;
            pi < NINLIL_MODEL_DOMAIN_BODY_EVIDENCE_SERVICE_SLOT_BYTES;
            ++pi) {
            REQUIRE(out[slot_off + pi] == 0u);
        }
    }

    ninlil_dv_free(&file);
    (void)fprintf(stdout,
        "evidence_cell contracts ok ev_vectors=%u typed_ok=%u "
        "padding_poison_ok\n",
        ev_count, typed_ok);
    return 0;
}
...
/*
 * D1-B3j REVERSE_REPLY: exact 330, raw86/raw80 borrow, closed state matrix,
 * pre-B3j prefix guard (no DSB3_RR_ before RC end).
 */
static int test_reverse_reply_contracts(const char *path)
{
    ninlil_dv_file_t file;
    char err[256];
    size_t i;
    uint32_t rr_count = 0u;
    int seen_kind4 = 0;
    int seen_reopen_i0 = 0;
    int seen_reopen_i1 = 0;
    int seen_g2i1 = 0;
    int seen_state2 = 0;
    int seen_state3 = 0;
    int seen_state5 = 0;
    int seen_state3_i0 = 0;
    int seen_len = 0;
    int seen_len_pair = 0;
    int seen_typed = 0;
    int seen_key = 0;
    ...
    REQUIRE(path != NULL);
    REQUIRE(ninlil_dv_load_file(path, &file, err, sizeof(err)) == 0);
    REQUIRE(file.vector_count > 1118u);
    REQUIRE(strcmp(file.vectors[0].id, "DSK1_SHA256_EMPTY") == 0);
    REQUIRE(strcmp(file.vectors[1117].id, "DSB3_RC_COMPLETED_AT_ZERO") == 0);
    for (i = 0u; i < 1118u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_RR_", 8) != 0);
    }

    for (i = 0u; i < file.vector_count; ++i) {
        const ninlil_dv_vector_t *v = &file.vectors[i];
        if (strncmp(v->id, "DSB3_RR_", 8) != 0) {
            continue;
        }
        rr_count++;
        if (v->subtype == 0x42u && strcmp(v->expected_status, "OK") == 0
            && strcmp(v->op, "body_roundtrip") == 0) {
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf, sizeof(body_buf),
                        &bn)
                == 0);
            ...
            REQUIRE(ninlil_model_domain_decode_body_reverse_reply(
                ...
                == NINLIL_OK);
            REQUIRE(body.reply_key_raw_length == 86u);
            REQUIRE(body.reply_key_raw == &body_buf[2]);
            REQUIRE(body.delivery_key_raw_length == 80u);
            REQUIRE(body.delivery_key_raw == &body_buf[90]);
            REQUIRE(body.reserved == 0u);
            REQUIRE(body.send_invocation_count
                <= body.send_operation_generation);
            REQUIRE(body.reply_kind >= 1u && body.reply_kind <= 4u);
            olen = 0u;
            REQUIRE(ninlil_model_domain_encode_body_reverse_reply(
                    &body, out, (uint32_t)sizeof(out), &olen)
                == NINLIL_OK);
            REQUIRE(olen == 330u);
            REQUIRE(memcmp(out, body_buf, bn) == 0);
            if (body.reply_kind == NINLIL_MODEL_DOMAIN_REPLY_KIND_CANCEL_RESULT) {
                seen_kind4 = 1;
            }
            if (strcmp(v->id, "DSB3_RR_PENDING_REOPEN_I0") == 0) {
                REQUIRE(body.send_state
                    == NINLIL_MODEL_DOMAIN_REPLY_SEND_PENDING);
                REQUIRE(body.send_operation_generation >= 1u);
                REQUIRE(body.send_invocation_count == 0u);
                REQUIRE(body.availability_epoch == 0u);
                seen_reopen_i0 = 1;
            }
            if (strcmp(v->id, "DSB3_RR_PENDING_REOPEN_I1") == 0) {
                REQUIRE(body.send_invocation_count == 1u);
                REQUIRE(body.availability_epoch != 0u);
                seen_reopen_i1 = 1;
            }
            if (strcmp(v->id, "DSB3_RR_MIXED_G2_I1") == 0) {
                REQUIRE(body.send_operation_generation == 2u);
                REQUIRE(body.send_invocation_count == 1u);
                seen_g2i1 = 1;
            }
            if (body.send_state
                == NINLIL_MODEL_DOMAIN_REPLY_SEND_WAITING_RETRY) {
                seen_state2 = 1;
            }
            if (body.send_state
            ...
            if (body.send_state
                ...
                REQUIRE(body.send_counter_exhausted == 1u);
                REQUIRE(
                    body.send_operation_generation == UINT64_MAX
                    || body.send_invocation_count == UINT64_MAX);
                seen_state5 = 1;
            }
        }
        if (v->subtype == 0x42u && strcmp(v->op, "typed_record") == 0
            && strcmp(v->expected_status, "OK") == 0) {
            uint8_t kbuf[128];
            uint8_t vbuf[4096];
            size_t kn = 0u;
            ...
            REQUIRE(ninlil_model_domain_validate_typed_record(
                    ...
                    &rec)
                == NINLIL_OK);
            REQUIRE(rec.subtype == 0x42u);
            REQUIRE(rec.envelope.header.record_revision >= 1u);
            REQUIRE(rec.envelope.header.flags == 0u);
            REQUIRE(!zeros(rec.envelope.header.primary_value_digest, 32u));
            REQUIRE(!zeros(rec.envelope.header.head_witness_digest, 32u));
            REQUIRE(rec.reverse_reply.reply_key_raw_length == 86u);
            REQUIRE(rec.reverse_reply.delivery_key_raw_length == 80u);
            seen_typed = 1;
        }
        if (strcmp(v->id, "DSB3_RR_STATE3_I0") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_state3_i0 = 1;
        }
        if (strcmp(v->id, "DSB3_RR_LEN329") == 0
            || strcmp(v->id, "DSB3_RR_LEN331") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf, sizeof(body_buf),
                        &bn)
                == 0);
        ...
        if (strcmp(v->id, "DSB3_RR_LEN_PAIR_87_79") == 0
            || strcmp(v->id, "DSB3_RR_LEN_PAIR_85_81") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf, sizeof(body_buf),
                        &bn)
                == 0);
        ...
        if (strcmp(v->id, "DSB3_RR_KEY_COMPOSITE_MISMATCH") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_key = 1;
        }
        if (strcmp(v->id, "DSB3_RR_I0_AVAIL_NZ") == 0
            || strcmp(v->id, "DSB3_RR_I_POS_AVAIL0") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_i_avail = 1;
        }
    }
    REQUIRE(rr_count >= 50u);
    REQUIRE(seen_kind4 == 1);
    REQUIRE(seen_reopen_i0 == 1);
    REQUIRE(seen_reopen_i1 == 1);
    REQUIRE(seen_g2i1 == 1);
    REQUIRE(seen_state2 == 1);
    REQUIRE(seen_state3 == 1);
    REQUIRE(seen_state5 == 1);
    REQUIRE(seen_state3_i0 == 1);
    REQUIRE(seen_len == 1);
    REQUIRE(seen_len_pair == 1);
    REQUIRE(seen_typed == 1);
    REQUIRE(seen_key == 1);
    REQUIRE(seen_i_avail == 1);
    ninlil_dv_free(&file);
    (void)fprintf(stdout, "reverse_reply contracts ok rr_vectors=%u\n",
        rr_count);
    return 0;
}

/*
 * D1-B3k EVENT_SPOOL: exact 300, state×cause matrix, resume/discard,
 * reservation KEY_DIGEST, revision relation, pre-B3k prefix guard.
 */
static int test_event_spool_contracts(const char *path)
{
    ninlil_dv_file_t file;
    char err[256];
    size_t i;
    uint32_t es_count = 0u;
    int seen_active = 0;
    int seen_parked = 0;
    int seen_parked_causes = 0;
    int seen_released = 0;
    int seen_discarded = 0;
    int seen_resume0 = 0;
    int seen_resume8 = 0;
    int seen_len = 0;
    int seen_typed = 0;
    ...
    REQUIRE(path != NULL);
    REQUIRE(ninlil_dv_load_file(path, &file, err, sizeof(err)) == 0);
    REQUIRE(file.vector_count > 1179u);
    REQUIRE(strcmp(file.vectors[0].id, "DSK1_SHA256_EMPTY") == 0);
    REQUIRE(strcmp(file.vectors[1178].id, "DSB3_RR_DLV_LEN79") == 0);
    for (i = 0u; i < 1179u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_ES_", 8) != 0);
    }

    for (i = 0u; i < file.vector_count; ++i) {
        const ninlil_dv_vector_t *v = &file.vectors[i];
        if (strncmp(v->id, "DSB3_ES_", 8) != 0) {
            continue;
        }
        es_count++;
        if (v->subtype == 0x50u && strcmp(v->expected_status, "OK") == 0
            && strcmp(v->op, "body_roundtrip") == 0) {
            ...
            REQUIRE((uint32_t)bn == v->body_length);
            (void)memset(&body, 0, sizeof(body));
            REQUIRE(ninlil_model_domain_decode_body_event_spool(
                    (ninlil_bytes_view_t){body_buf, (uint32_t)bn}, &body)
                == NINLIL_OK);
            REQUIRE(body.spool_revision >= 1u);
            REQUIRE(body.retry_cycle_id >= 1u);
            REQUIRE(body.successful_resume_count <= 8u);
            REQUIRE(!zeros(body.transaction_id, 16u));
            REQUIRE(!zeros(body.event_id, 16u));
            REQUIRE(!zeros(body.payload_blob_key_digest, 32u));
            REQUIRE(!zeros(body.reservation_key_digest, 32u));
            olen = 0u;
            REQUIRE(ninlil_model_domain_encode_body_event_spool(
                    &body, out, (uint32_t)sizeof(out), &olen)
                == NINLIL_OK);
            REQUIRE(olen == 300u);
            REQUIRE(memcmp(out, body_buf, bn) == 0);
            if (body.spool_state
                ...
                REQUIRE(body.park_cause == NINLIL_EVENT_PARK_CAUSE_NONE);
                REQUIRE(body.discard_committed == 0u);
                seen_active = 1;
                if (body.successful_resume_count == 0u) {
                    seen_resume0 = 1;
                }
                if (body.successful_resume_count == 8u) {
                    seen_resume8 = 1;
                }
            }
            if (body.spool_state
                == NINLIL_MODEL_DOMAIN_SPOOL_STATE_PARKED_RETRY) {
                REQUIRE(body.park_cause >= 1u && body.park_cause <= 5u);
                REQUIRE(body.discard_committed == 0u);
                seen_parked = 1;
                parked_cause_bits |= (1 << (int)body.park_cause);
            }
            if (body.spool_state
                == NINLIL_MODEL_DOMAIN_SPOOL_STATE_RELEASED) {
                REQUIRE(body.park_cause == NINLIL_EVENT_PARK_CAUSE_NONE);
                REQUIRE(body.discard_committed == 0u);
                seen_released = 1;
            }
            if (body.spool_state
                == NINLIL_MODEL_DOMAIN_SPOOL_STATE_DISCARDED) {
                REQUIRE(body.park_cause == NINLIL_EVENT_PARK_CAUSE_NONE);
                REQUIRE(body.discard_committed == 1u);
                seen_discarded = 1;
            }
        }
        if (v->subtype == 0x50u && strcmp(v->op, "typed_record") == 0
            && strcmp(v->expected_status, "OK") == 0) {
            uint8_t kbuf[64];
            uint8_t vbuf[4096];
            size_t kn = 0u;
            ...
            REQUIRE(ninlil_model_domain_validate_typed_record(
                    ...
                    &rec)
                == NINLIL_OK);
            REQUIRE(rec.subtype == 0x50u);
            REQUIRE(rec.envelope.header.record_revision
                == rec.event_spool.spool_revision);
            REQUIRE(rec.envelope.header.record_revision >= 1u);
            REQUIRE(rec.envelope.header.flags == 0u);
            REQUIRE(!zeros(rec.envelope.header.primary_value_digest, 32u));
        ...
        if (strcmp(v->id, "DSB3_ES_LEN299") == 0
            || strcmp(v->id, "DSB3_ES_LEN301") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
        ...
        if (strcmp(v->id, "DSB3_ES_KEY_TX_MISMATCH") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_key = 1;
        }
        if (strcmp(v->id, "DSB3_ES_REV_MISMATCH") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_rev = 1;
        }
        if (strcmp(v->id, "DSB3_ES_RES_DIGEST_WRONG") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
        ...
        if (strcmp(v->id, "DSB3_ES_RES_DIGEST_BARE_COMPOSITE") == 0) {
            /* bare COMPOSITE(RESERVATION, owner_kind=2||RAW16(tx)) rejected */
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
        ...
        if (strcmp(v->id, "DSB3_ES_MUT_STATE") == 0) {
            /* production replay of generator mutation at spool_state @40 */
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 300u);
            REQUIRE(body_buf[40] == 0u && body_buf[41] == 0u
                && body_buf[42] == 0u && body_buf[43] == 99u);
            (void)memset(&body, 0xA5, sizeof(body));
            REQUIRE(ninlil_model_domain_decode_body_event_spool(
                    (ninlil_bytes_view_t){body_buf, (uint32_t)bn}, &body)
                == NINLIL_E_STORAGE_CORRUPT);
        ...
        if (strcmp(v->id, "DSB3_ES_ACTIVE_CAUSE1") == 0
            || strcmp(v->id, "DSB3_ES_PARKED_CAUSE0") == 0
            || strcmp(v->id, "DSB3_ES_DISCARD1_ACTIVE") == 0
            || strcmp(v->id, "DSB3_ES_DISCARD0_DISCARDED") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_matrix = 1;
        }
    }
    /* each park cause 1..5 appears at least once among positives */
    if ((parked_cause_bits & 0x3e) == 0x3e) {
        seen_parked_causes = 1;
    }
    REQUIRE(es_count >= 35u);
    REQUIRE(seen_active == 1);
    REQUIRE(seen_parked == 1);
    REQUIRE(seen_parked_causes == 1);
    REQUIRE(seen_released == 1);
    REQUIRE(seen_discarded == 1);
    REQUIRE(seen_resume0 == 1);
    REQUIRE(seen_resume8 == 1);
    REQUIRE(seen_len == 1);
    REQUIRE(seen_typed == 1);
    REQUIRE(seen_key == 1);
    REQUIRE(seen_rev == 1);
    REQUIRE(seen_res == 1);
    REQUIRE(seen_res_bare == 1);
    REQUIRE(seen_mut == 1);
    REQUIRE(seen_matrix == 1);
    ninlil_dv_free(&file);
    (void)fprintf(stdout, "event_spool contracts ok es_vectors=%u\n", es_count);
    return 0;
}
...
/*
 * D1-B3m MANAGEMENT_LEDGER: exact 364, kind15/16 matrix, canonical digest,
 * composite key tx||op, every negative production-replayed, pre-B3m 1287 prefix.
 */
static int test_management_ledger_contracts(const char *path)
{
    ninlil_dv_file_t file;
    char err[256];
    size_t i;
    uint32_t ml_count = 0u;
    uint32_t ml_neg_count = 0u;
    uint32_t ml_neg_replayed = 0u;
    int seen_resume = 0;
    int seen_discard = 0;
    int seen_r_rsn5 = 0;
    int seen_d_rsn4 = 0;
    ...
    int seen_len = 0;
    int seen_key = 0;
    int seen_digest_wrong = 0;
    int seen_preimage_mut = 0;
    int seen_r_mut_exp_rev = 0;
    int seen_r_mut_aud_len = 0;
    int seen_d_mut_exp_rev = 0;
    int seen_d_mut_aud_len = 0;
    int seen_excl_seq = 0;
    int seen_excl_r_event = 0;
    ...
    REQUIRE(path != NULL);
    REQUIRE(ninlil_dv_load_file(path, &file, err, sizeof(err)) == 0);
    REQUIRE(file.vector_count > 1287u);
    REQUIRE(strcmp(file.vectors[0].id, "DSK1_SHA256_EMPTY") == 0);
    REQUIRE(strcmp(file.vectors[1286].id, "DSB3_RS_KEY_ID128_KIND") == 0);
    for (i = 0u; i < 1287u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_ML_", 8) != 0);
    }

    for (i = 0u; i < file.vector_count; ++i) {
        const ninlil_dv_vector_t *v = &file.vectors[i];
        if (strncmp(v->id, "DSB3_ML_", 8) != 0) {
            continue;
        }
        ml_count++;
        ...
        if (strcmp(v->expected_status, "CORRUPT") == 0
            ...
            if (strcmp(v->op, "body_decode") == 0
                ...
                REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                    ...
                    == 0);
                (void)memset(&body, 0xA5, sizeof(body));
                REQUIRE(ninlil_model_domain_decode_body_management_ledger(
                        (ninlil_bytes_view_t){body_buf, (uint32_t)bn},
                        &body)
                    == NINLIL_E_STORAGE_CORRUPT);
                REQUIRE(zeros(&body, sizeof(body)));
        ...
        if (v->subtype == 0x52u && strcmp(v->expected_status, "OK") == 0
            && strcmp(v->op, "body_roundtrip") == 0) {
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            ...
            REQUIRE(ninlil_model_domain_decode_body_management_ledger(
                ...
                == NINLIL_OK);
            REQUIRE(!zeros(body.transaction_id, 16u));
            REQUIRE(!zeros(body.operation_id, 16u));
            REQUIRE(body.replay_spool_revision
                == body.expected_spool_revision + 1u);
            olen = 0u;
            REQUIRE(ninlil_model_domain_encode_body_management_ledger(
                    &body, out, (uint32_t)sizeof(out), &olen)
                == NINLIL_OK);
            REQUIRE(olen == 364u);
            REQUIRE(memcmp(out, body_buf, bn) == 0);
            if (body.operation_kind
                == NINLIL_MODEL_DOMAIN_MANAGEMENT_KIND_EVENT_RESUME) {
                seen_resume = 1;
                REQUIRE(body.expected_content_digest_algorithm == 0u);
                REQUIRE(body.acknowledge_flag == 0u);
                REQUIRE(body.replay_spool_released == 0u);
                REQUIRE(body.replay_retry_cycle_id >= 1u);
            ...
            if (body.operation_kind
                == NINLIL_MODEL_DOMAIN_MANAGEMENT_KIND_EVENT_DISCARD) {
                seen_discard = 1;
                REQUIRE(body.expected_content_digest_algorithm
                    == NINLIL_MODEL_DOMAIN_CONTENT_DIGEST_SHA256);
                REQUIRE(body.acknowledge_flag == 1u);
                REQUIRE(body.replay_spool_released == 1u);
                REQUIRE(body.replay_retry_cycle_id == 0u);
                REQUIRE(!zeros(body.audit_clock_epoch, 16u));
                REQUIRE(body.replay_result_kind
                    == NINLIL_EVENT_DISCARD_ALREADY_DISCARDED);
                REQUIRE(body.replay_result_reason
                    == NINLIL_REASON_OPERATOR_DISCARDED_WITHOUT_REQUIRED_RECEIPT);
                if (body.request_reason == 4u) {
                    seen_d_rsn4 = 1;
                }
                if (body.audit_committed_at_ms == 0u) {
                    seen_time0 = 1;
                }
            }
            if (body.audit_length == 128u) {
                seen_aud128 = 1;
            }
            if (body.expected_spool_revision == UINT64_MAX - 1u
                && body.replay_spool_revision == UINT64_MAX) {
                seen_rev_max = 1;
            }
        }
        if (v->subtype == 0x52u && strcmp(v->op, "typed_record") == 0
            && strcmp(v->expected_status, "OK") == 0) {
            uint8_t kbuf[64];
            uint8_t vbuf[4096];
            size_t kn = 0u;
            ...
            REQUIRE(rec.key.identity_kind
                == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
            REQUIRE(rec.envelope.body.length == 364u);
            seen_typed = 1;
        }
        if (strcmp(v->id, "DSB3_ML_LEN363") == 0
            || strcmp(v->id, "DSB3_ML_LEN365") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_len = 1;
        }
        if (strcmp(v->id, "DSB3_ML_KEY_TX_MISMATCH") == 0
            || strcmp(v->id, "DSB3_ML_KEY_OP_MISMATCH") == 0
            || strcmp(v->id, "DSB3_ML_KEY_ID128_KIND") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_key = 1;
        }
        if (strcmp(v->id, "DSB3_ML_DIGEST_WRONG") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_digest_wrong = 1;
        }
        if (strncmp(v->id, "DSB3_ML_R_MUT_", 14) == 0
            || strncmp(v->id, "DSB3_ML_D_MUT_", 14) == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_preimage_mut = 1;
        }
        /* Named pure stale-digest isolations (matrix remains valid). */
        if (strcmp(v->id, "DSB3_ML_R_MUT_EXP_REV") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 364u);
            /* expected=4 @108, replay=5 @348 */
            REQUIRE(body_buf[108] == 0u && body_buf[109] == 0u
                && body_buf[110] == 0u && body_buf[111] == 0u
                && body_buf[112] == 0u && body_buf[113] == 0u
                && body_buf[114] == 0u && body_buf[115] == 4u);
            REQUIRE(body_buf[348] == 0u && body_buf[349] == 0u
                && body_buf[350] == 0u && body_buf[351] == 0u
                && body_buf[352] == 0u && body_buf[353] == 0u
                && body_buf[354] == 0u && body_buf[355] == 5u);
            seen_r_mut_exp_rev = 1;
        }
        if (strcmp(v->id, "DSB3_ML_R_MUT_AUD_LEN") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 364u);
            REQUIRE(body_buf[176] == 0u && body_buf[177] == 2u);
            seen_r_mut_aud_len = 1;
        }
        if (strcmp(v->id, "DSB3_ML_D_MUT_EXP_REV") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            ...
            REQUIRE(body_buf[108] == 0u && body_buf[109] == 0u
                && body_buf[110] == 0u && body_buf[111] == 0u
                && body_buf[112] == 0u && body_buf[113] == 0u
                && body_buf[114] == 0u && body_buf[115] == 9u);
            REQUIRE(body_buf[348] == 0u && body_buf[349] == 0u
                && body_buf[350] == 0u && body_buf[351] == 0u
                && body_buf[352] == 0u && body_buf[353] == 0u
                && body_buf[354] == 0u && body_buf[355] == 10u);
            seen_d_mut_exp_rev = 1;
        }
        if (strcmp(v->id, "DSB3_ML_D_MUT_AUD_LEN") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 364u);
            REQUIRE(body_buf[176] == 0u && body_buf[177] == 2u);
            seen_d_mut_aud_len = 1;
        }
        /* Named excluded-field OK positives (digest equal, production RT). */
        if (strcmp(v->id, "DSB3_ML_EXCL_SEQ99") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "body_roundtrip") == 0);
            seen_excl_seq = 1;
        }
        if (strcmp(v->id, "DSB3_ML_EXCL_R_EVENT") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "body_roundtrip") == 0);
            seen_excl_r_event = 1;
        }
        if (strcmp(v->id, "DSB3_ML_EXCL_D_TIME") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "body_roundtrip") == 0);
            seen_excl_d_time = 1;
        }
        if (strcmp(v->id, "DSB3_ML_EXCL_D_EPOCH") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "body_roundtrip") == 0);
            seen_excl_d_epoch = 1;
        }
        if (strcmp(v->id, "DSB3_ML_EXCL_R_CYCLE") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "body_roundtrip") == 0);
            seen_excl_r_cycle = 1;
        }
        if (strcmp(v->id, "DSB3_ML_MUT_KIND") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
    ...
    REQUIRE(ml_count >= 96u);
    REQUIRE(ml_neg_count >= 41u);
    REQUIRE(ml_neg_replayed == ml_neg_count);
    REQUIRE(seen_resume == 1);
    REQUIRE(seen_discard == 1);
    REQUIRE(seen_r_rsn5 == 1);
    REQUIRE(seen_d_rsn4 == 1);
    REQUIRE(seen_aud128 == 1);
    REQUIRE(seen_time0 == 1);
    REQUIRE(seen_rev_max == 1);
    REQUIRE(seen_typed == 1);
    REQUIRE(seen_len == 1);
    REQUIRE(seen_key == 1);
    REQUIRE(seen_digest_wrong == 1);
    REQUIRE(seen_preimage_mut == 1);
    REQUIRE(seen_r_mut_exp_rev == 1);
    REQUIRE(seen_r_mut_aud_len == 1);
    REQUIRE(seen_d_mut_exp_rev == 1);
    REQUIRE(seen_d_mut_aud_len == 1);
    REQUIRE(seen_excl_seq == 1);
    REQUIRE(seen_excl_r_event == 1);
    ...
    ninlil_dv_free(&file);
    (void)fprintf(stdout,
        "management_ledger contracts ok ml_vectors=%u neg_replayed=%u\n",
        ml_count, ml_neg_replayed);
    return 0;
}

/*
 * D1-B3n RETENTION_BASIS: 90+N (106/170), subject kind+raw, KEY_DIGEST,
 * pending/trusted/eligible matrix, every negative production-replayed,
 * pre-B3n 1391 prefix preserved; base 1456 prefix preserved after coverage
 * append (CLEANUP epoch/delete/overflow + DLV single-segment zero).
 */
static int test_retention_basis_contracts(const char *path)
{
    ninlil_dv_file_t file;
    char err[256];
    size_t i;
    uint32_t rb_count = 0u;
    uint32_t rb_neg_count = 0u;
    uint32_t rb_neg_replayed = 0u;
    int seen_tx = 0;
    int seen_dlv = 0;
    int seen_pending = 0;
    int seen_overflow = 0;
    ...
    int seen_add_ovf = 0;
    int seen_len = 0;
    int seen_clean_epoch0 = 0;
    int seen_clean_del_wrong = 0;
    int seen_clean_overflow = 0;
    int seen_dlv_zero_seg = 0;
    ...
    REQUIRE(path != NULL);
    REQUIRE(ninlil_dv_load_file(path, &file, err, sizeof(err)) == 0);
    REQUIRE(file.vector_count > 1456u);
    REQUIRE(strcmp(file.vectors[0].id, "DSK1_SHA256_EMPTY") == 0);
    REQUIRE(strcmp(file.vectors[1390].id, "DSB3_ML_KEY_COMP_SUBTYPE") == 0);
    REQUIRE(strcmp(file.vectors[1455].id, "DSB3_RB_EMPTY") == 0);
    for (i = 0u; i < 1391u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_RB_", 8) != 0);
    }
    /* coverage-append ids live only after the 1456 base prefix */
    for (i = 0u; i < 1456u; ++i) {
        REQUIRE(strcmp(file.vectors[i].id, "DSB3_RB_CLEAN_EPOCH_ZERO") != 0);
        REQUIRE(strcmp(file.vectors[i].id, "DSB3_RB_CLEAN_DELETE_WRONG") != 0);
        REQUIRE(strcmp(file.vectors[i].id, "DSB3_RB_CLEAN_OVERFLOW") != 0);
        REQUIRE(
            strcmp(file.vectors[i].id, "DSB3_RB_DLV_ZERO_SEG_SRC_APP") != 0);
    }

    for (i = 0u; i < file.vector_count; ++i) {
        const ninlil_dv_vector_t *v = &file.vectors[i];
        if (strncmp(v->id, "DSB3_RB_", 8) != 0) {
            continue;
        }
        rb_count++;
        ...
        if (strcmp(v->expected_status, "CORRUPT") == 0
            ...
            if (strcmp(v->op, "body_decode") == 0
                ...
                REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                    ...
                    == 0);
                (void)memset(&body, 0xA5, sizeof(body));
                REQUIRE(ninlil_model_domain_decode_body_retention_basis(
                        (ninlil_bytes_view_t){body_buf, (uint32_t)bn},
                        &body)
                    == NINLIL_E_STORAGE_CORRUPT);
                REQUIRE(zeros(&body, sizeof(body)));
        ...
        if (v->subtype == 0x61u && strcmp(v->expected_status, "OK") == 0
            && strcmp(v->op, "body_roundtrip") == 0) {
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 106u || bn == 170u);
            REQUIRE((uint32_t)bn == v->body_length);
            (void)memset(&body, 0, sizeof(body));
            REQUIRE(ninlil_model_domain_decode_body_retention_basis(
                    (ninlil_bytes_view_t){body_buf, (uint32_t)bn}, &body)
                == NINLIL_OK);
            REQUIRE(body.subject_key_raw != NULL);
            REQUIRE(body.required_window_ms > 0u);
            olen = 0u;
            REQUIRE(ninlil_model_domain_encode_body_retention_basis(
                    &body, out, (uint32_t)sizeof(out), &olen)
                == NINLIL_OK);
            REQUIRE(olen == (uint32_t)bn);
            REQUIRE(memcmp(out, body_buf, bn) == 0);
            if (body.subject_kind
                == NINLIL_MODEL_DOMAIN_RETENTION_SUBJECT_TRANSACTION) {
                seen_tx = 1;
                REQUIRE(bn == 106u);
            }
            ...
            if (body.retention_state
                    == NINLIL_MODEL_DOMAIN_RETENTION_STATE_ACTIVE
                && body.basis_pending == 1u) {
                seen_pending = 1;
                REQUIRE(body.retention_overflow == 0u);
                REQUIRE(body.basis_at_ms == 0u);
                REQUIRE(body.exclusive_cleanup_at_ms == 0u);
            }
            if (body.retention_state
                    == NINLIL_MODEL_DOMAIN_RETENTION_STATE_ACTIVE
                && body.retention_overflow == 1u) {
                seen_overflow = 1;
                REQUIRE(body.basis_pending == 0u);
                REQUIRE(body.exclusive_cleanup_at_ms == 0u);
                REQUIRE(!zeros(body.basis_clock_epoch, 16u));
            }
            if (body.retention_state
                    == NINLIL_MODEL_DOMAIN_RETENTION_STATE_ACTIVE
                && body.basis_pending == 0u
                && body.retention_overflow == 0u) {
                seen_trusted = 1;
                REQUIRE(!zeros(body.basis_clock_epoch, 16u));
                REQUIRE(body.exclusive_cleanup_at_ms
                    == body.basis_at_ms + body.required_window_ms);
            }
            if (body.retention_state
                == NINLIL_MODEL_DOMAIN_RETENTION_STATE_ELIGIBLE) {
                seen_eligible = 1;
            ...
            if (body.basis_at_ms == 0u
                && body.basis_pending == 0u
                && body.retention_overflow == 0u
                && body.retention_state
                    != NINLIL_MODEL_DOMAIN_RETENTION_STATE_ACTIVE) {
                /* ELIGIBLE/CLEANUP with basis0, or trusted with basis0 */
                seen_basis0 = 1;
            }
            if (body.basis_at_ms == 0u && body.basis_pending == 0u
                && (body.retention_overflow == 1u
                    || (body.retention_overflow == 0u
                        && body.retention_state
                            == NINLIL_MODEL_DOMAIN_RETENTION_STATE_ACTIVE))) {
            ...
            if (body.required_window_ms == 1u) {
                seen_window1 = 1;
            }
            if (body.basis_at_ms == UINT64_MAX - 1u
                && body.required_window_ms == 1u
                && body.exclusive_cleanup_at_ms == UINT64_MAX) {
                seen_maxsum = 1;
            }
        }
        if (v->subtype == 0x61u && strcmp(v->op, "typed_record") == 0
            && strcmp(v->expected_status, "OK") == 0) {
            uint8_t kbuf[64];
            uint8_t vbuf[4096];
            size_t kn = 0u;
            ...
            REQUIRE(rec.key.identity_kind
                == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
            REQUIRE(rec.envelope.body.length == 106u
                || rec.envelope.body.length == 170u);
            seen_typed = 1;
        }
        if (strcmp(v->id, "DSB3_RB_DLV_BARE_COMPOSITE_DIGEST") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_bare = 1;
        }
        if (strcmp(v->id, "DSB3_RB_ADD_OVERFLOW") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_add_ovf = 1;
        }
        if (strcmp(v->id, "DSB3_RB_RAW15") == 0
            || strcmp(v->id, "DSB3_RB_RAW81") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_len = 1;
        }
        if (strcmp(v->id, "DSB3_RB_CLEAN_EPOCH_ZERO") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1456u);
            seen_clean_epoch0 = 1;
        }
        if (strcmp(v->id, "DSB3_RB_CLEAN_DELETE_WRONG") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1456u);
            seen_clean_del_wrong = 1;
        }
        if (strcmp(v->id, "DSB3_RB_CLEAN_OVERFLOW") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1456u);
            seen_clean_overflow = 1;
        }
        if (strcmp(v->id, "DSB3_RB_DLV_ZERO_SEG_SRC_APP") == 0) {
            uint8_t raw80[80];
            uint8_t dig_stored[32];
            uint8_t dig_expect[32];
            ninlil_model_domain_digest_t dig;
            ninlil_model_domain_key_t built;
            ninlil_bytes_view_t components;
            ninlil_bytes_view_t idv;
            uint8_t raw16_buf[2u + 80u];
            ...
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                ...
                == 0);
            REQUIRE(bn == 170u);
            REQUIRE(body_buf[0] == 0u && body_buf[1] == 3u); /* kind DELIVERY */
            REQUIRE(body_buf[4] == 0u && body_buf[5] == 80u);
            (void)memcpy(raw80, &body_buf[6], 80u);
            /* exactly one of five 16-byte ID segments is all-zero (src.app) */
            REQUIRE(zeros(&raw80[16], 16u));
            REQUIRE(!zeros(&raw80[0], 16u));
            REQUIRE(!zeros(&raw80[32], 16u));
            REQUIRE(!zeros(&raw80[48], 16u));
            REQUIRE(!zeros(&raw80[64], 16u));
            (void)memcpy(dig_stored, &body_buf[86], 32u);
            /* KEY_DIGEST(complete DELIVERY primary) for this raw must match */
            ninlil_model_domain_encode_u16_be(raw16_buf, 80u);
            (void)memcpy(&raw16_buf[2], raw80, 80u);
            ...
            REQUIRE(ninlil_model_domain_key_digest(
                    (ninlil_bytes_view_t){built.bytes, built.length}, &dig)
                == NINLIL_OK);
            (void)memcpy(dig_expect, dig.bytes, 32u);
            REQUIRE(memcmp(dig_stored, dig_expect, 32u) == 0);
            /* production rejects (zero segment only; digest is correct) */
            (void)memset(&body, 0xA5, sizeof(body));
            REQUIRE(ninlil_model_domain_decode_body_retention_basis(
                    (ninlil_bytes_view_t){body_buf, (uint32_t)bn}, &body)
                == NINLIL_E_STORAGE_CORRUPT);
    ...
    REQUIRE(rb_count >= 54u);
    REQUIRE(rb_neg_count >= 39u);
    REQUIRE(rb_neg_replayed == rb_neg_count);
    REQUIRE(seen_tx == 1);
    REQUIRE(seen_dlv == 1);
    REQUIRE(seen_pending == 1);
    REQUIRE(seen_overflow == 1);
    REQUIRE(seen_trusted == 1);
    REQUIRE(seen_eligible == 1);
    REQUIRE(seen_cleanup == 1);
    REQUIRE(seen_basis0 == 1);
    REQUIRE(seen_window1 == 1);
    REQUIRE(seen_maxsum == 1);
    REQUIRE(seen_typed == 1);
    REQUIRE(seen_bare == 1);
    REQUIRE(seen_add_ovf == 1);
    REQUIRE(seen_len == 1);
    REQUIRE(seen_clean_epoch0 == 1);
    REQUIRE(seen_clean_del_wrong == 1);
    REQUIRE(seen_clean_overflow == 1);
    REQUIRE(seen_dlv_zero_seg == 1);
    ninlil_dv_free(&file);
    (void)fprintf(stdout,
        "retention_basis contracts ok rb_vectors=%u neg_replayed=%u\n",
        rb_count, rb_neg_replayed);
    return 0;
}

/*
 * D1-B3o W6 known-answer pins (docs domain tags + complete key bytes).
 * Hardcoded independently of generator helpers so a shared preimage mistake
 * cannot self-validate. Fixture raw bytes match domain_store_vector_gen.
 */
static const uint8_t k_cp_w6_tx_raw[16] = {
    0x71, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x99
};
static const uint8_t k_cp_w6_dlv_raw[80] = {
    0xb1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x02, 0xa1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x71, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x99,
    0xc1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x03, 0xd1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04
};
/* KEY_DIGEST(complete TRANSACTION_ANCHOR ID128 key) */
static const uint8_t k_cp_w6_tx_primary_key_digest[32] = {
    0xd0, 0x5f, 0xc7, 0x81, 0x7b, 0xcd, 0xf0, 0x56, 0x71, 0xef, 0xb6, 0x5f,
    0x43, 0xe5, 0x4e, 0x5a, 0xe9, 0x6a, 0x58, 0x09, 0x06, 0x7d, 0x99, 0x74,
    0xac, 0xce, 0xe2, 0xd5, 0xe3, 0x89, 0x4c, 0xfb
};
/* KEY_DIGEST(complete DELIVERY COMPOSITE(40, RAW16 raw80) key) */
static const uint8_t k_cp_w6_dlv_primary_key_digest[32] = {
    0xb1, 0xf2, 0x96, 0x94, 0x6e, 0xc8, 0x4c, 0x2a, 0x50, 0x2f, 0x96, 0xda,
    0xfe, 0x52, 0xd8, 0xa2, 0x15, 0x4a, 0x70, 0x26, 0x3b, 0x1c, 0xaa, 0xf4,
    0x10, 0x0b, 0x23, 0x88, 0x38, 0x56, 0x97, 0x91
};
/* COMPOSITE(63, subject_kind=TX || TX primary KEY_DIGEST) identity */
static const uint8_t k_cp_w6_cleanup_composite_identity_tx[32] = {
    0x9a, 0x3b, 0x3a, 0xb4, 0xf4, 0x36, 0x68, 0x22, 0x07, 0xa0, 0x49, 0xd1,
    0xbf, 0xd7, 0x6d, 0xf8, 0x79, 0x09, 0xcf, 0x3f, 0x1f, 0xc7, 0xc1, 0xb9,
    0xe5, 0x86, 0x4d, 0xfb, 0x41, 0x3a, 0xc7, 0x6b
};
/* DELIVERY composite identity first 16 (not raw[:16]) */
static const uint8_t k_cp_w6_dlv_primary_id[16] = {
    0x84, 0x34, 0xdb, 0xf6, 0xd6, 0xf1, 0x4c, 0x86, 0xd8, 0x58, 0x36, 0xe7,
    0x2b, 0xba, 0xc4, 0xe8
};

/*
 * D1-B3o CLEANUP_PLAN: independent oracle prefix pin (1460), wide phase
 * matrix positives, production API replay of every negative, generator
 * independence (no shared production error codes), and audit W1–W7
 * regression assets after the 1537 baseline (format d1b3o append-only).
 */
static int test_cleanup_plan_contracts(const char *path)
{
    ninlil_dv_file_t file;
    char err[256];
    size_t i;
    uint32_t cp_count = 0u;
    uint32_t cp_neg_count = 0u;
    uint32_t cp_neg_replayed = 0u;
    int seen_tx = 0;
    int seen_dlv = 0;
    int seen_p1 = 0;
    int seen_p2 = 0;
    ...
    int seen_w6 = 0;
    int seen_w7 = 0;
    ninlil_model_domain_body_cleanup_plan_t body;
    ninlil_model_domain_typed_record_t rec;
    ninlil_model_domain_digest_t dig;
    ninlil_model_domain_key_t built;
    uint8_t body_buf[512];
    ...
    REQUIRE(path != NULL);
    REQUIRE(ninlil_dv_load_file(path, &file, err, sizeof(err)) == 0);
    REQUIRE(file.vector_count > 1537u);
    REQUIRE(strcmp(file.vectors[0].id, "DSK1_SHA256_EMPTY") == 0);
    REQUIRE(strcmp(file.vectors[1459].id, "DSB3_RB_DLV_ZERO_SEG_SRC_APP")
        == 0);
    REQUIRE(strcmp(file.vectors[1536].id, "DSB3_CP_KEY_COMP_SUBTYPE") == 0);
    for (i = 0u; i < 1460u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_CP_", 8) != 0);
    }
    /* Audit assets are strictly after the 1537 baseline prefix. */
    for (i = 0u; i < 1537u; ++i) {
        REQUIRE(strncmp(file.vectors[i].id, "DSB3_CP_W", 9) != 0);
    }
    REQUIRE(strncmp(file.vectors[1537].id, "DSB3_CP_W", 9) == 0);

    /* W6: production recompute of known-answer constants (not helper mirror). */
    idv.data = k_cp_w6_tx_raw;
    idv.length = 16u;
    REQUIRE(ninlil_model_domain_build_key(
            NINLIL_MODEL_DOMAIN_FAMILY_DOMAIN,
            NINLIL_MODEL_DOMAIN_SUBTYPE_TRANSACTION_ANCHOR,
    ...
    REQUIRE(ninlil_model_domain_key_digest(
            (ninlil_bytes_view_t){built.bytes, built.length}, &dig)
        == NINLIL_OK);
    REQUIRE(memcmp(dig.bytes, k_cp_w6_tx_primary_key_digest, 32u) == 0);

    ninlil_model_domain_encode_u16_be(raw16_buf, 80u);
    (void)memcpy(&raw16_buf[2], k_cp_w6_dlv_raw, 80u);
    cv.data = raw16_buf;
    cv.length = 82u;
    REQUIRE(ninlil_model_domain_composite_digest(
            NINLIL_MODEL_DOMAIN_SUBTYPE_DELIVERY, cv, &dig)
    ...
    {
        uint8_t dlv_comp[32];
        (void)memcpy(dlv_comp, dig.bytes, 32u);
        /* primary_id is composite identity first 16, not raw[:16] */
        REQUIRE(memcmp(dlv_comp, k_cp_w6_dlv_primary_id, 16u) == 0);
        REQUIRE(memcmp(k_cp_w6_dlv_raw, k_cp_w6_dlv_primary_id, 16u) != 0);
        idv.data = dlv_comp;
        idv.length = 32u;
        REQUIRE(ninlil_model_domain_build_key(
                NINLIL_MODEL_DOMAIN_FAMILY_DOMAIN,
        ...
        REQUIRE(ninlil_model_domain_key_digest(
                (ninlil_bytes_view_t){built.bytes, built.length}, &dig)
            == NINLIL_OK);
        REQUIRE(memcmp(dig.bytes, k_cp_w6_dlv_primary_key_digest, 32u) == 0);
    }

    /* COMPOSITE(63, subject_kind:u16=TX || subject primary KEY_DIGEST) */
    comp_in[0] = 0u;
    comp_in[1] = 2u;
    (void)memcpy(&comp_in[2], k_cp_w6_tx_primary_key_digest, 32u);
    cv.data = comp_in;
    cv.length = 34u;
    REQUIRE(ninlil_model_domain_composite_digest(
            NINLIL_MODEL_DOMAIN_SUBTYPE_CLEANUP_PLAN, cv, &dig)
        == NINLIL_OK);
    REQUIRE(memcmp(dig.bytes, k_cp_w6_cleanup_composite_identity_tx, 32u)
        == 0);
    seen_w6 = 1;

    for (i = 0u; i < file.vector_count; ++i) {
        const ninlil_dv_vector_t *v = &file.vectors[i];
        if (strncmp(v->id, "DSB3_CP_", 8) != 0) {
            continue;
        }
        cp_count++;
        ...
        if (strcmp(v->expected_status, "CORRUPT") == 0
            || strcmp(v->expected_status, "INVALID") == 0) {
            cp_neg_count++;
            /* Independent confirmation: oracle never embeds production E_*. */
            REQUIRE(strstr(v->id, "NINLIL_E_") == NULL);
            if (strcmp(v->op, "body_decode") == 0
                || strcmp(v->op, "body_roundtrip") == 0) {
                REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                            sizeof(body_buf), &bn)
                    == 0);
                (void)memset(&body, 0xA5, sizeof(body));
                REQUIRE(ninlil_model_domain_decode_body_cleanup_plan(
                        (ninlil_bytes_view_t){body_buf, (uint32_t)bn},
                        &body)
                    == NINLIL_E_STORAGE_CORRUPT);
                REQUIRE(zeros(&body, sizeof(body)));
        ...
        if (v->subtype == 0x63u && strcmp(v->expected_status, "OK") == 0
            && strcmp(v->op, "body_roundtrip") == 0) {
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 142u || bn == 206u);
            REQUIRE((uint32_t)bn == v->body_length);
            (void)memset(&body, 0, sizeof(body));
            REQUIRE(ninlil_model_domain_decode_body_cleanup_plan(
                    (ninlil_bytes_view_t){body_buf, (uint32_t)bn}, &body)
                == NINLIL_OK);
            REQUIRE(body.subject_key_raw != NULL);
            REQUIRE(body.cleanup_generation >= 1u);
            REQUIRE(body.batch_generation >= 1u);
            olen = 0u;
            REQUIRE(ninlil_model_domain_encode_body_cleanup_plan(
                    &body, out, (uint32_t)sizeof(out), &olen)
                == NINLIL_OK);
            REQUIRE(olen == (uint32_t)bn);
            REQUIRE(memcmp(out, body_buf, bn) == 0);
            ...
            if (body.cleanup_phase
                == NINLIL_MODEL_DOMAIN_CLEANUP_PHASE_DELETE_NON_INDEX) {
                seen_p1 = 1;
                REQUIRE(body.attempt_reuse_fenced == 0u);
                REQUIRE(body.remaining_attempt_index_count
                    == body.initial_attempt_index_count);
                REQUIRE(body.remaining_attempt_count
                    >= body.remaining_attempt_index_count);
            }
            if (body.cleanup_phase
                == NINLIL_MODEL_DOMAIN_CLEANUP_PHASE_DELETE_ATTEMPT_INDEX) {
                seen_p2 = 1;
                REQUIRE(body.attempt_reuse_fenced == 1u);
                REQUIRE(body.remaining_attempt_count
                    == body.remaining_attempt_index_count);
                REQUIRE(body.remaining_attempt_count >= 1u);
                REQUIRE(body.initial_attempt_index_count >= 1u);
            }
            if (body.cleanup_phase
                == NINLIL_MODEL_DOMAIN_CLEANUP_PHASE_FINALIZE) {
                seen_p3 = 1;
                REQUIRE(body.remaining_attempt_count == 0u);
                REQUIRE(body.remaining_attempt_index_count == 0u);
            ...
            if (body.cleanup_generation == 1u
                && body.batch_generation == 1u) {
                seen_gen1 = 1;
            }
            if (body.cleanup_generation == UINT64_MAX
                || body.batch_generation == UINT64_MAX) {
                seen_genmax = 1;
            }
        }
        if (v->subtype == 0x63u && strcmp(v->op, "typed_record") == 0
            && strcmp(v->expected_status, "OK") == 0) {
            uint8_t kbuf[64];
            uint8_t vbuf[4096];
            size_t kn = 0u;
            ...
            REQUIRE(rec.key.identity_kind
                == NINLIL_MODEL_DOMAIN_ID_KIND_SHA256_COMPOSITE);
            REQUIRE(rec.envelope.body.length == 142u
                || rec.envelope.body.length == 206u);
            seen_typed = 1;
        }
        if (strcmp(v->id, "DSB3_CP_DLV_BARE_COMPOSITE") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_bare = 1;
        }
        if (strcmp(v->id, "DSB3_CP_RAW15") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_raw15 = 1;
        }
        if (strcmp(v->id, "DSB3_CP_PVD_MISMATCH") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_pvd_mm = 1;
        }
        if (strcmp(v->id, "DSB3_CP_REV_MISMATCH") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            seen_rev_mm = 1;
        }
        if (strcmp(v->id, "DSB3_CP_DLV_ZERO_SEG") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(i >= 1460u);
            seen_dlv_zero = 1;
        }

        /* --- audit W1–W7 regression assets (after 1537 baseline) --- */
        if (strcmp(v->id, "DSB3_CP_W1_KEY_RAW_CONTENTS_TX") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "typed_record") == 0);
            REQUIRE(i >= 1537u);
            seen_w1_tx = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W1_KEY_RAW_CONTENTS_DLV") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "typed_record") == 0);
            REQUIRE(i >= 1537u);
            seen_w1_dlv = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W2_SELF_KEY_DIGEST_TX") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1537u);
            seen_w2_tx = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W2_SELF_KEY_DIGEST_DLV") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1537u);
            seen_w2_dlv = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W2_BARE_CLEANUP_COMPOSITE_TX") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1537u);
            seen_w2_bare = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W3_KIND2_RAW80") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1537u);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                ...
                == 0);
            REQUIRE(bn == 206u);
            REQUIRE(body_buf[0] == 0u && body_buf[1] == 2u); /* kind TX */
            REQUIRE(body_buf[4] == 0u && body_buf[5] == 80u); /* raw80 */
            seen_w3_k2 = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W3_KIND3_RAW16") == 0) {
            REQUIRE(strcmp(v->expected_status, "CORRUPT") == 0);
            REQUIRE(strcmp(v->op, "body_decode") == 0);
            REQUIRE(i >= 1537u);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                ...
                == 0);
            REQUIRE(bn == 142u);
            REQUIRE(body_buf[0] == 0u && body_buf[1] == 3u); /* kind DLV */
            REQUIRE(body_buf[4] == 0u && body_buf[5] == 16u); /* raw16 */
            seen_w3_k3 = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W4_P1_ALL0") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "body_roundtrip") == 0);
            REQUIRE(i >= 1537u);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
            ...
            REQUIRE(ninlil_model_domain_decode_body_cleanup_plan(
                    (ninlil_bytes_view_t){body_buf, (uint32_t)bn}, &body)
                == NINLIL_OK);
            REQUIRE(body.cleanup_phase
                == NINLIL_MODEL_DOMAIN_CLEANUP_PHASE_DELETE_NON_INDEX);
            REQUIRE(body.initial_attempt_count == 0u);
            REQUIRE(body.remaining_attempt_count == 0u);
            REQUIRE(body.initial_attempt_index_count == 0u);
            REQUIRE(body.remaining_attempt_index_count == 0u);
            REQUIRE(body.attempt_reuse_fenced == 0u);
            seen_w4 = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W4_P1_ALL0_TYPED") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "typed_record") == 0);
            REQUIRE(i >= 1537u);
            seen_w4_typed = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W5_P2_MID") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "body_roundtrip") == 0);
            REQUIRE(i >= 1537u);
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
            ...
            REQUIRE(body.cleanup_phase
                == NINLIL_MODEL_DOMAIN_CLEANUP_PHASE_DELETE_ATTEMPT_INDEX);
            REQUIRE(body.initial_attempt_count == 10u);
            REQUIRE(body.initial_attempt_index_count == 7u);
            REQUIRE(body.remaining_attempt_count == 2u);
            REQUIRE(body.remaining_attempt_index_count == 2u);
            REQUIRE(body.attempt_reuse_fenced == 1u);
            seen_w5 = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W5_P2_MID_TYPED") == 0) {
            REQUIRE(strcmp(v->expected_status, "OK") == 0);
            REQUIRE(strcmp(v->op, "typed_record") == 0);
            REQUIRE(i >= 1537u);
            seen_w5_typed = 1;
        }
        if (strcmp(v->id, "DSB3_CP_W7_DLV_PRIMARY_ID_RAW16") == 0) {
            uint8_t kbuf[64];
            uint8_t vbuf[4096];
            size_t kn = 0u;
            size_t vn = 0u;
            ...
            REQUIRE(hex_to(ninlil_dv_str(v->key_hex), kbuf, sizeof(kbuf), &kn)
                == 0);
            REQUIRE(hex_to(ninlil_dv_str(v->value_hex), vbuf, sizeof(vbuf),
                        &vn)
                == 0);
            /* NLR1(4)+rtype(2)+ver(2)+payload_len(4)+hdr_ver(2)+st/flags(2)
             * +rev(8) = 24; primary_id follows. */
            REQUIRE(vn >= 24u + 16u);
            (void)memcpy(pid, &vbuf[24], 16u);
            REQUIRE(memcmp(pid, k_cp_w6_dlv_raw, 16u) == 0);
            REQUIRE(memcmp(pid, k_cp_w6_dlv_primary_id, 16u) != 0);
            (void)memset(&rec, 0xA5, sizeof(rec));
            REQUIRE(ninlil_model_domain_validate_typed_record(
                    (ninlil_bytes_view_t){kbuf, (uint32_t)kn},
                    (ninlil_bytes_view_t){vbuf, (uint32_t)vn},
        ...
        /* W6 cross-check: baseline positive body digests match known answers */
        if (strcmp(v->id, "DSB3_CP_TX_P1_FULL") == 0) {
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 142u);
            REQUIRE(memcmp(&body_buf[6], k_cp_w6_tx_raw, 16u) == 0);
            REQUIRE(memcmp(&body_buf[22], k_cp_w6_tx_primary_key_digest, 32u)
                == 0);
        }
        if (strcmp(v->id, "DSB3_CP_DLV_P1") == 0) {
            REQUIRE(hex_to(ninlil_dv_str(v->body_hex), body_buf,
                        sizeof(body_buf), &bn)
                == 0);
            REQUIRE(bn == 206u);
            REQUIRE(memcmp(&body_buf[6], k_cp_w6_dlv_raw, 80u) == 0);
            REQUIRE(memcmp(&body_buf[86], k_cp_w6_dlv_primary_key_digest, 32u)
                == 0);
        }
        if (strcmp(v->id, "DSB3_CP_TX_P1_FULL_TYPED") == 0) {
            uint8_t kbuf[64];
            size_t kn = 0u;
            REQUIRE(hex_to(ninlil_dv_str(v->key_hex), kbuf, sizeof(kbuf), &kn)
                == 0);
            /* ROOT(8)+fam/st/ver/kind/len(5) = 13; identity is composite. */
            REQUIRE(kn >= 13u + 32u);
            REQUIRE(memcmp(&kbuf[13], k_cp_w6_cleanup_composite_identity_tx, 32u)
                == 0);
        }
    }

    REQUIRE(cp_count >= 70u);
    REQUIRE(cp_neg_count >= 48u);
    REQUIRE(cp_neg_replayed == cp_neg_count);
    REQUIRE(seen_tx == 1);
    REQUIRE(seen_dlv == 1);
    REQUIRE(seen_p1 == 1);
    REQUIRE(seen_p2 == 1);
    REQUIRE(seen_p3 == 1);
    REQUIRE(seen_idx0 == 1);
    REQUIRE(seen_idx_pos == 1);
    REQUIRE(seen_gen1 == 1);
    REQUIRE(seen_genmax == 1);
    REQUIRE(seen_typed == 1);
    REQUIRE(seen_bare == 1);
    REQUIRE(seen_raw15 == 1);
    REQUIRE(seen_pvd_mm == 1);
    REQUIRE(seen_rev_mm == 1);
    REQUIRE(seen_dlv_zero == 1);
    REQUIRE(seen_w1_tx == 1);
    REQUIRE(seen_w1_dlv == 1);
    REQUIRE(seen_w2_tx == 1);
    REQUIRE(seen_w2_dlv == 1);
    REQUIRE(seen_w2_bare == 1);
    REQUIRE(seen_w3_k2 == 1);
    REQUIRE(seen_w3_k3 == 1);
    REQUIRE(seen_w4 == 1);
    REQUIRE(seen_w4_typed == 1);
    REQUIRE(seen_w5 == 1);
    REQUIRE(seen_w5_typed == 1);
    REQUIRE(seen_w6 == 1);
    REQUIRE(seen_w7 == 1);
    ninlil_dv_free(&file);
    (void)fprintf(stdout,
        "cleanup_plan contracts ok cp_vectors=%u neg_replayed=%u "
        "audit_w1_w7=1\n",
        cp_count, cp_neg_replayed);
    return 0;
}
...
int main(int argc, char **argv)
{
    ...
    if (test_unit_sha_and_null_key() != 0
        ...
        || test_management_ledger_contracts(path) != 0
        || test_retention_basis_contracts(path) != 0
        || test_cleanup_plan_contracts(path) != 0
