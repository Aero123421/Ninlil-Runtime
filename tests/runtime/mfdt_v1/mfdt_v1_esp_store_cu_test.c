...
static mock_store_t *mock_for_user(void *user)
{
    return user != NULL ? (mock_store_t *)user : &g_mock;
}

static uint8_t mock_occ_count(const mock_store_t *store)
{
    uint8_t i;
    uint8_t n = 0u;
    for (i = 0; i < MOCK_MAX_ROWS; ++i) {
...
static void mock_apply_staged(mock_store_t *store)
{
    uint8_t i;
    for (i = 0; i < store->staged_n; ++i) {
        int idx = mock_find(store->rows, store->staged[i].key);
        if (store->staged[i].is_erase) {
            if (store->staged[i].corrupt_on_apply) {
                ...
                if (idx >= 0) {
                    memcpy(store->rows[idx].key, store->staged[i].key, 20);
                    store->rows[idx].val[0] =
                        (uint8_t)(store->rows[idx].val[0] ^ 0xA5u);
                    if (store->rows[idx].len == 0u) {
                        store->rows[idx].len = 1u;
                        store->rows[idx].val[0] = 0xA5u;
                    }
                    store->rows[idx].occ = 1u;
                }
            } else if (idx >= 0) {
                memset(&store->rows[idx], 0, sizeof(store->rows[idx]));
            }
            continue;
        }
        if (idx < 0) {
        ...
        if (idx >= 0) {
            store->rows[idx] = store->staged[i];
            store->rows[idx].is_erase = 0u;
            store->rows[idx].corrupt_on_apply = 0u;
        }
    }
    store->staged_n = 0u;
}
                                         ...
                                         ninlil_storage_handle_t *out)
{
    (void)ns;
    (void)schema;
    *out = (ninlil_storage_handle_t)(void *)mock_for_user(user);
...
{
    mock_store_t *store = mock_for_user(user);
    (void)h;
    (void)mode;
    store->begin_count += 1u;
    store->txn_open = 1u;
    store->staged_n = 0u;
    *out_txn = g_txn_handle;
    store->last_txn_token = (uintptr_t)(void *)g_txn_handle;
    return NINLIL_STORAGE_OK;
}
...
#endif
    if (store->staged_n >= MOCK_MAX_ROWS || key.length != 20u ||
        value.length > MOCK_VAL_MAX) {
        return NINLIL_STORAGE_IO_ERROR;
    }
    r = &store->staged[store->staged_n++];
    memset(r, 0, sizeof(*r));
                                        ...
                                        ninlil_mut_bytes_t *value)
{
    ...
    idx = mock_find(store->rows, key.data);
    if (idx < 0) {
        return NINLIL_STORAGE_NOT_FOUND;
    }
    if (value->capacity < store->rows[idx].len) {
        value->length = store->rows[idx].len;
        return NINLIL_STORAGE_BUFFER_TOO_SMALL;
    }
    if (store->rows[idx].len > 0u) {
        memcpy(value->data, store->rows[idx].val, store->rows[idx].len);
    }
    value->length = store->rows[idx].len;
    return NINLIL_STORAGE_OK;
}
...
static void test_two_owner_isolation_reentry_and_fini(void)
{
    ninlil_mfdt_v1_lab_store_t a;
    ninlil_mfdt_v1_lab_store_t b;
    ninlil_mfdt_v1_lab_store_t *before;
    mock_store_t media_a;
    mock_store_t media_b;
    ninlil_storage_ops_t ops_a;
    ninlil_storage_ops_t ops_b;
    uint8_t key[20];
    uint8_t value_a[4] = {1u, 2u, 3u, 4u};
    uint8_t value_b[4] = {5u, 6u, 7u, 8u};
    uint8_t out[4];
    uint32_t out_len = 0u;

    mock_reset();
    memset(&media_a, 0, sizeof(media_a));
    memset(&media_b, 0, sizeof(media_b));
    ops_a = g_ops;
    ops_b = g_ops;
    ops_a.user = &media_a;
    ops_b.user = &media_b;
    ninlil_mfdt_v1_lab_store_init(&a);
    ninlil_mfdt_v1_lab_store_init(&b);
    before = (ninlil_mfdt_v1_lab_store_t *)malloc(sizeof(*before));
    expect(before != NULL, "owner snapshot allocation");
    if (before == NULL) {
        return;
    }

    expect(ninlil_mfdt_v1_esp_store_bind(&a, &ops_a, &media_a) == 0,
           "owner A bind");
    expect(ninlil_mfdt_v1_esp_store_bind(&b, &ops_b, &media_b) == 0,
           "owner B bind");
    expect(a.esp.readback != b.esp.readback &&
               a.esp.old_pool != b.esp.old_pool,
           "owner bulk disjoint");

    memcpy(before, &a, sizeof(a));
    expect(ninlil_mfdt_v1_esp_store_bind(&a, &ops_a, &media_a) ==
               NINLIL_MFDT_V1_ERR_BUSY,
           "same owner rebind busy");
    expect(memcmp(before, &a, sizeof(a)) == 0,
           "same owner rebind no mutation");

    expect(ninlil_mfdt_v1_lab_full_begin(&a) == 0, "owner A begin");
    memcpy(before, &a, sizeof(a));
    expect(ninlil_mfdt_v1_lab_full_begin(&a) == NINLIL_MFDT_V1_ERR_BUSY,
           "same owner begin busy");
    expect(memcmp(before, &a, sizeof(a)) == 0,
           "same owner begin no mutation");
    expect(ninlil_mfdt_v1_lab_full_begin(&b) == 0,
           "different owner begins independently");

    memset(key, 0x31, sizeof(key));
    expect(ninlil_mfdt_v1_lab_put(&a, key, value_a, sizeof(value_a)) == 0,
           "owner A put");
    expect(ninlil_mfdt_v1_lab_put(&b, key, value_b, sizeof(value_b)) == 0,
           "owner B put");
    expect(ninlil_mfdt_v1_lab_full_commit(&a) == 0, "owner A commit");
    expect(ninlil_mfdt_v1_lab_full_commit(&b) == 0, "owner B commit");
    expect(media_a.commit_count == 1u && media_b.commit_count == 1u,
           "media commits isolated");
    expect(ninlil_mfdt_v1_lab_get(&a, key, out, sizeof(out), &out_len) == 0 &&
               out_len == sizeof(value_a) &&
               memcmp(out, value_a, sizeof(value_a)) == 0,
           "owner A reads A media");
    out_len = 0u;
    expect(ninlil_mfdt_v1_lab_get(&b, key, out, sizeof(out), &out_len) == 0 &&
               out_len == sizeof(value_b) &&
               memcmp(out, value_b, sizeof(value_b)) == 0,
           "owner B reads B media");

    memcpy(before, &b, sizeof(b));
    ninlil_mfdt_v1_lab_store_fini(&a);
    expect(bytes_are_zero(&a, sizeof(a)), "owner A fini all bytes zero");
    expect(memcmp(before, &b, sizeof(b)) == 0,
           "owner A fini leaves owner B unchanged");
    ninlil_mfdt_v1_lab_store_fini(&b);
    expect(bytes_are_zero(&b, sizeof(b)), "owner B fini all bytes zero");
    free(before);
}
...
static void test_bind_preallocates_all_operation_bulk(void)
{
    ...
    expect(ninlil_mfdt_v1_esp_store_bind(&st, &g_ops, &g_mock) == 0,
           "allocation bind");
    ...
    ninlil_mfdt_v1_lab_store_fini(&st);
}
...
static void test_64bit_txn_handle_width(void)
{
    ...
    expect(ninlil_mfdt_v1_esp_store_bind(&st, &g_ops, &g_mock) == 0, "bind");
...
static void test_cu_resolve_old_retry_cold_restart(void)
{
    ...
    ninlil_mfdt_v1_lab_store_init(&st);
    expect(ninlil_mfdt_v1_esp_store_bind(&st, &g_ops, &g_mock) == 0, "rebind");
    len = 0;
    ...
    expect(mock_occ_count(&g_mock) == 1u, "no duplicate row");
    len = 0;
...
static void test_retry_no_duplicate(void)
{
    ...
    expect(ninlil_mfdt_v1_lab_full_commit(&st) ==
           ...
           "raw NEW");
    expect(mock_occ_count(&g_mock) == 1u, "one row");
...
static void test_bind_requires_erase_ops(void)
{
    ...
    bad = g_ops;
    bad.erase = NULL;
    expect(ninlil_mfdt_v1_esp_store_bind(&st, &bad, &g_mock) ==
...
static void test_delete_cu_new_absent(void)
{
    ...
    expect(ninlil_mfdt_v1_esp_last_cu_class(&st) ==
               (int)NINLIL_MFDT_V1_CU_NEW,
           "class NEW");
    len = 0;
...
int main(void)
{
    g_fail = 0;
    ops_init();
    test_two_owner_isolation_reentry_and_fini();
