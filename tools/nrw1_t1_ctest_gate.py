#!/usr/bin/env python3
"""Single authority for R7 T1 NRW1 CTest registration + focused run (docs/32 §9).

Uses ``ctest --show-only=json-v1`` (not name-only -N text) so registration is
exact multiset **and** free of skip/DISABLED properties.

CTest names use nrw1_t1_* prefix so T0 r7_* exact 16/15 authority is untouched.

Profiles:
  normal     — full T1 set including production stack gate
  sanitizer  — same set minus production stack gate
...
PASS ≠ product GO / R7 complete / ESP HIL / T1 Accepted.
"""
...
NRW1_T1_CORE_REQUIRED: tuple[str, ...] = (
    "nrw1_t1_wire_portable_strict",
    "nrw1_t1_vectors_bridge",
    "nrw1_t1_oracle_self_test",
    "nrw1_t1_oracle_verify",
    "nrw1_t1_kat_pin",
    "nrw1_t1_kat_pin_self_test",
    "nrw1_t1_platform_split_gate",
    "nrw1_t1_platform_split_gate_self_test",
    "nrw1_t1_tests_off_packaging_gate",
    "nrw1_t1_tests_off_packaging_gate_self_test",
    "nrw1_t1_stack_gate_self_test",
    "nrw1_t1_ctest_gate_self_test",
)

NRW1_T1_PRODUCTION_STACK = "nrw1_t1_stack_gate"
NRW1_T1_FOCUSED_REGEX = r"nrw1_t1_"

# CTest properties that would let a required test be skipped, inverted, or
# declared green from output despite a non-zero command exit. Presence on a
# required nrw1_t1_* test is fail-closed (false-green otherwise).
FORBIDDEN_SKIP_PROPERTIES: frozenset[str] = frozenset(
    {
        ...
        # Historical / alternate spellings seen across CMake versions:
        "SKIP_RETURN_CODE_MATCH",
        # WILL_FAIL turns a failing required test green — fail-closed.
        "WILL_FAIL",
        # PASS_REGULAR_EXPRESSION can turn exit(1) green when output matches.
...
def required_for_profile(profile: str) -> tuple[str, ...]:
    if profile == "normal":
        return NRW1_T1_CORE_REQUIRED + (NRW1_T1_PRODUCTION_STACK,)
    if profile == "sanitizer":
        return NRW1_T1_CORE_REQUIRED
    raise ValueError(f"unknown profile: {profile}")


def _truthy_disabled(value: Any) -> bool:
    """CMake/CTest truthy property values (DISABLED/WILL_FAIL/etc.).
...
def properties_map(test: dict[str, Any]) -> dict[str, Any]:
    """Flatten ctest json-v1 properties list to name→value."""
    out: dict[str, Any] = {}
...
def test_skip_property_errors(name: str, props: dict[str, Any]) -> list[str]:
    ...
    for key, value in props.items():
        ...
        if upper in FORBIDDEN_SKIP_PROPERTIES or key in FORBIDDEN_SKIP_PROPERTIES:
            # WILL_FAIL without truthy is listed in the set but only truthy matters;
            # non-truthy DISABLED/WILL_FAIL already handled above.
            ...
            continue
        # Any property whose name starts with SKIP_ is skip-equivalent.
...
def registration_errors_from_tests(
    ...
    """Exact multiset + no skip/DISABLED properties on required T1 tests."""
    errors: list[str] = []
    ...
    registered = [n for n in names if n.startswith("nrw1_t1_")]
    ...
    if len(registered) != len(required) or sorted(registered) != sorted(required):
        ...
        for name, cnt in sorted(reg_counts.items()):
            ...
            if cnt != exp and name in required_set and cnt > 1:
                errors.append(
                    f"false-green: nrw1_t1_* multiset mismatch for '{name}': "
                    f"registered={cnt}, expected={exp}"
                )

    prod = sum(1 for n in names if n == NRW1_T1_PRODUCTION_STACK)
    if profile == "normal" and prod != 1:
        errors.append(
            f"false-green: production {NRW1_T1_PRODUCTION_STACK} count={prod} "
    ...
    # Skip / DISABLED property gate on every required test instance.
    for name in required:
...
def load_ctest_json(build_dir: Path) -> dict[str, Any]:
    ...
    # Some ctest versions print progress on stderr; JSON is on stdout.
...
def run_check_registration(build_dir: Path, profile: str) -> int:
    try:
        ...
        print(f"nrw1_t1_ctest_gate FAIL: {exc}", file=sys.stderr)
        return 1
    tests = data["tests"]
    ...
    if errors:
        for e in errors:
            print(f"nrw1_t1_ctest_gate FAIL: {e}", file=sys.stderr)
        return 1
    required = required_for_profile(profile)
    registered = [
        t.get("name")
        ...
        and t["name"].startswith("nrw1_t1_")
    ]
    prod = sum(1 for n in registered if n == NRW1_T1_PRODUCTION_STACK)
    print(
        f"nrw1_t1_ctest_gate: registration PASS "
        f"(profile={profile}, required={len(required)}, "
        f"nrw1_registered={len(registered)}, exact_set=yes, "
...
def run_focused(build_dir: Path, profile: str) -> int:
    ...
    if proc.returncode != 0:
        print(
            f"nrw1_t1_ctest_gate FAIL: focused CTest rc={proc.returncode}",
            file=sys.stderr,
        )
        return proc.returncode
    print(
        f"nrw1_t1_ctest_gate: focused run PASS "
        f"(profile={profile}, -R {NRW1_T1_FOCUSED_REGEX!r})"
    )
    return 0
...
def run_self_test() -> int:
    ...
    with_unexpected = list(normal_list) + ["nrw1_t1_unexpected_test"]
    if not registration_errors_from_tests(
    ...
    if registration_errors_from_tests(_synthetic_tests(with_noise), "normal"):
        failures.append("non-nrw1 noise incorrectly red")

    # DISABLED=true on a required test must red (json-v1 property path).
    disabled_tests = _synthetic_tests(normal_list)
    for t in disabled_tests:
        if t["name"] == "nrw1_t1_vectors_bridge":
    ...
    # SKIP_RETURN_CODE property must red.
    skip_rc_tests = _synthetic_tests(normal_list)
    for t in skip_rc_tests:
        if t["name"] == "nrw1_t1_oracle_self_test":
    ...
    if not any("SKIP_RETURN_CODE" in e for e in errs_src):
        failures.append(
            f"SKIP_RETURN_CODE mutation did not go red: {errs_src}"
        )

    # SKIP_REGULAR_EXPRESSION must red.
    skip_re_tests = _synthetic_tests(normal_list)
    for t in skip_re_tests:
        if t["name"] == "nrw1_t1_kat_pin":
            t["properties"].append(
    ...
    # PASS_REGULAR_EXPRESSION must red: CTest can otherwise turn exit(1)
    # green solely because command output matches this property.
    pass_re_tests = _synthetic_tests(normal_list)
    for t in pass_re_tests:
        if t["name"] == "nrw1_t1_wire_portable_strict":
            t["properties"].append(
    ...
    # String form DISABLED=true must red.
    disabled_str = _synthetic_tests(normal_list)
    for t in disabled_str:
        if t["name"] == "nrw1_t1_wire_portable_strict":
            t["properties"].append({"name": "DISABLED", "value": "true"})
    errs_ds = registration_errors_from_tests(disabled_str, "normal")
    if not any("DISABLED" in e for e in errs_ds):
        failures.append(f"DISABLED='true' string mutation did not go red: {errs_ds}")

    # WILL_FAIL=true must red (would invert required-test failure to green).
    will_fail_tests = _synthetic_tests(normal_list)
    for t in will_fail_tests:
        if t["name"] == "nrw1_t1_platform_split_gate":
    ...
    # Integration: real temporary CMake project + ctest --show-only=json-v1.
    ...
    if not cmake or not ctest:
        ...
        for prop_name, prop_value in prop_cases:
            try:
                with tempfile.TemporaryDirectory(prefix="nrw1-t1-ctest-mut-") as td:
                    root = Path(td)
                    src = root / "src"
                    build = root / "build"
                    src.mkdir()
                    ...
                    (src / "CMakeLists.txt").write_text(
                        textwrap.dedent(
                            ...
                            project(nrw1_t1_gate_mutation LANGUAGES NONE)
                            enable_testing()
                            set(_names
                            {names_cmake}
                            )
                            ...
                            endforeach()
                            set_tests_properties(nrw1_t1_vectors_bridge PROPERTIES
    ...
    if failures:
        for f in failures:
            print(f"nrw1_t1_ctest_gate self-test FAIL: {f}", file=sys.stderr)
        return 1
    print(
        f"nrw1_t1_ctest_gate self-test: PASS "
        f"(core={len(NRW1_T1_CORE_REQUIRED)}, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not build_dir.is_dir():
        print(
            f"nrw1_t1_ctest_gate FAIL: build dir missing: {build_dir}",
            file=sys.stderr,
        )
        return 1
